...
ATTR_NONINIT_DATA_IN_RAM static st_instance_a        g_instList_a[SAMPLE_A_INSTANCE_COUNT];
ATTR_NONINIT_DATA_IN_RAM static st_instance_b        g_instList_b[SAMPLE_B_INSTANCE_COUNT];

static cis_callback_t callback;
static cis_time_t g_lifetime = 100;
        ...
        case 0:
            {
                ...
                for (instIndex = 0;instIndex < instCount;instIndex++)
                {
                    ...
                    }else
                    {
                        ...
                        g_instList_a[instIndex].instance.floatValue = (float)cissys_rand() * 10.0 / 0xFFFFFFFF;
        ...
        **/
        ...
        if(nowtime - g_notifyLast > 30*1000){
            ...
            while(observe_node != NULL){
                if(observe_node->mid == 0)continue;
                if(observe_node->uri.flag == 0)continue;
                cis_uri_t uriLocal;
                uriLocal = observe_node->uri;
                prv_observeNotify(g_context,&uriLocal,observe_node->mid);//订阅资源发送接口
...
#define SAMPLE_DEFAULT_CONFIG 0
    ...
    if(cis_init(&g_context,NULL,0,NULL,NULL) != CIS_RET_OK){
        printf("cis entry init failed.\n");
        return -1;
    }
#else
    if(cis_init(&g_context,(void *)config_bin,config_size,NULL,NULL) != CIS_RET_OK){
        printf("cis entry init failed.\n");
        return -1;
    }
#endif
        ...
        case SAMPLE_OID_B:
            {
                ...
                {                   
                    ...
                    if(inst != NULL &&  inst->enabled == true)
                    {
                        ...
                        tmpdata[2].asBuffer.buffer = (uint8_t*)strdup(inst->instance.strValue);
    ...
    case SAMPLE_OID_A:
        {
            ...
            for (int i=0;i<count;i++)
            {
                ...
                case  attributeA_stringValue:
                    {
                        cis_memset(inst->instance.strValue,0,sizeof(inst->instance.strValue));
