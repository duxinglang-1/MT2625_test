                         ...
                         st_object_t * firmwareObj)
{
    ...
    if(firmwareObj->instBitmapBytes < instBytes){
        ...
        {
            cis_free(firmwareObj->instBitmapPtr);//yyl 2018/5/31
        }
        firmwareObj->instBitmapPtr = (uint8_t*)cis_malloc(instBytes);//yyl 2018/5/31
                          ...
                          st_object_t * objectP)
{
    ...
    if (instanceId != 0||targetP==NULL)//yyl 2018/5/20
    {
                           ...
                           st_object_t * objectP)
{
    ...
    do
    {
        ...
        case RES_M_PACKAGE:
			{				
				...
				{
				   	contextP->fw_request.single_packet_write = false;//yyl 2018/5/18
					contextP->fw_request.need_async_ack = true;////yyl 2018/5/18
					contextP->fw_request.write_state=PACKAGE_WRITING;//yyl 2018/5/28
	...
	//				contextP->fw_request.need_async_ack = true;
				}else
				{
					...
					{		
     					contextP->fw_request.single_packet_write = true;////yyl 2018/5/18
                    	contextP->fw_request.need_async_ack = true;//yyl 2018/5/18
						contextP->fw_request.write_state=PACKAGE_WRITING;////yyl 2018/5/18
                                   ...
                                   st_object_t * objectP)
{
    ...
    case RES_M_UPDATE:
        if (targetP->fw_info->state == FOTA_STATE_DOWNLOADED)
        {
				...
				||targetP->fw_info->result == FOTA_RESULT_UPDATEFAIL)//bad request
			{
				...
				fw_state_change(contextP,FOTA_STATE_UPDATING);//yyl 2018/5/17
				cissys_setFwState(FOTA_STATE_UPDATING);//yyl 2018/5/17
				object_asynAckNodata(contextP,contextP->fw_request.response_ack,COAP_204_CHANGED);//yyl 2018/5/24
	...
	case RES_M_SWITCH_TO_DOWNLOAD://change to downloading
		if (targetP->fw_info->state == FOTA_STATE_IDIL)  
		{
			...
			cissys_setFwState(FOTA_STATE_DOWNLOADING);//yyl 2018/5/17
			object_asynAckNodata(contextP,contextP->fw_request.response_ack,COAP_204_CHANGED);
		...
		{
			object_asynAckNodata(contextP,contextP->fw_request.response_ack,COAP_400_BAD_REQUEST);
			return COAP_400_BAD_REQUEST;
		}
    default:
...
void std_firmware_clean(st_context_t * contextP)
{
	...
	cissys_setFwState(fw_info->state);//yyl 2018/5/17
	...
	while (firmwareInstance != NULL)
	{
		...
		if(deleteInst->fw_info!=NULL)		
			cis_free(deleteInst->fw_info);//yyl 2018/5/30
		cis_free(deleteInst);
	}
}
