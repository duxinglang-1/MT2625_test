    ...
    do {
        ...
        if (ret < 0) {
            ...
            if (rc > 0) {
				...
				m2m_mqtt_protocol_receive(buffer,len);
            } else {
                MQTT_DBG("mqtt read fail: ret=%d, rc = %d, recvlen = %d", ret, rc, recvlen);
				ctiot_lwm2m_client_send_response("mqtt read recv fail");
        ...
        if (recvlen < len) vTaskDelay(10 / portTICK_RATE_MS);
    } while (recvlen < len);
    return recvlen;
}
...
int ConnectNetwork(Network *n, char *addr,  char *port)
{
    ...
    if (rc == 0) {
        ...
        lwip_setsockopt(n->my_socket, SOL_SOCKET, SO_SNDTIMEO, &send_timeout, sizeof(send_timeout));
