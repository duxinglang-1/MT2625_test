...
#define NB_DEFAULT_FIRST_DNS_SERVER  ("218.4.4.4")
#define NB_DEFAULT_SECOND_DNS_SERVER ("114.114.114.114")

#define NB_NETIF_BACKUP_TOTAL_ITEM 2
typedef struct _nbiot_bearer_info_backup_struct
{
    u8_t is_activated; //activated (1) and deactivated(0)
    u8_t cid;
    u8_t type; //ipv4(0) or ipv6(1)
    u8_t ip6_ready; /*true: 1, false: 0*/
    u32_t mtu;
    u32_t channel_id;
    union {
...
{
    memset(backup_item, 0, sizeof(nbiot_bearer_info_backup_struct));
    ...
    if (bearer_info->type == 0) {
        ip4addr_aton(bearer_info->ip_addr, &(backup_item->ip_addr.ip4_address));
        if (bearer_info->pri_dns[0] != 0) {
            ip4addr_aton(bearer_info->pri_dns, &(backup_item->pri_dns.ip4_address));
        } else {
            ip4addr_aton(NB_DEFAULT_FIRST_DNS_SERVER, &(backup_item->pri_dns.ip4_address));
        }
        if (bearer_info->snd_dns[0] != 0) {
            ip4addr_aton(bearer_info->snd_dns, &(backup_item->sec_dns.ip4_address));
        } else {
            ip4addr_aton(NB_DEFAULT_SECOND_DNS_SERVER, &(backup_item->sec_dns.ip4_address));
        }
    } else if (bearer_info->type == 1) {
        NBNETIF_LOGI("IPV6 info will backup later\r\n");
        if (bearer_info->pri_dns[0] != 0) {
            ip6addr_aton(bearer_info->pri_dns, &(backup_item->pri_dns.ip6_address));
        }
    }
...
void nb_clear_backup_item_by_cid(u32_t cid)
{
    int i;
    for(i = 0; i < NB_NETIF_BACKUP_TOTAL_ITEM; i++) {
        if(g_lwip_nbiot_bearer_retention[i].cid == cid) {
            NBNETIF_LOGI("clear backup item %d, cid %d, cid %d\n", i, g_lwip_nbiot_bearer_retention[i].cid, cid);
            memset(g_lwip_nbiot_bearer_retention + i, 0, sizeof(nbiot_bearer_info_backup_struct));
        }
    }
}
...
void nb_retention_backup_item()
{
    ...
    for(i = 0; i < NB_NETIF_BACKUP_TOTAL_ITEM; i++) {
        ...
        if(g_lwip_nbiot_bearer_retention[i].is_activated == 1) {
            ...
            if (netinfo->type == 0) {
                memcpy(&(dnsserver.u_addr.ip4), &(netinfo->pri_dns.ip4_address), sizeof(netinfo->pri_dns.ip4_address));
                dnsserver.type = 0;
                memcpy(&(sec_dnsserver.u_addr.ip4), &(netinfo->sec_dns.ip4_address), sizeof(netinfo->sec_dns.ip4_address));
                sec_dnsserver.type = 0;
            } else {
                memcpy(&(dnsserver.u_addr.ip6), &(netinfo->pri_dns.ip6_address), sizeof(netinfo->pri_dns.ip6_address));
                dnsserver.type = 1;
                memcpy(&(sec_dnsserver.u_addr.ip6), &(netinfo->sec_dns.ip6_address), sizeof(netinfo->sec_dns.ip6_address));
                sec_dnsserver.type = 1;
            }
            dns_setserver(0, &dnsserver);
            dns_setserver(1, &sec_dnsserver);
            nbnetif->type = netinfo->type;
            if (netinfo->type == 0) {
                memcpy(&(nbnetif->ipaddr), &(netinfo->ip_addr.ip4_address), sizeof(netinfo->ip_addr.ip4_address));
            } else if (netinfo->type == 1) {
                nb_net_info_add_t *link_local_netif = NULL;
                nbnetif->ip6_ready = netinfo->ip6_ready;
                memcpy(&(nbnetif->ip6_address), &(netinfo->ip_addr.ip6_address), sizeof(netinfo->ip_addr.ip6_address));
                if (!ip6_addr_islinklocal(&(netinfo->ip_addr.ip6_address))) {
                    link_local_netif = pvPortMalloc(sizeof(nb_net_info_add_t));
                    if (link_local_netif != NULL) {
                        memset(link_local_netif, 0, sizeof(nb_net_info_add_t));
                        link_local_netif->type = netinfo->type;
                        link_local_netif->cid = netinfo->cid;
                        link_local_netif->channel_id = netinfo->channel_id;
                        link_local_netif->mtu = netinfo->mtu;
                        link_local_netif->ip6_ready = netinfo->ip6_ready;
                        /*restore link local address.*/
                        link_local_netif->ip6_address.addr[0] = PP_HTONL(0xfe800000ul);
                        link_local_netif->ip6_address.addr[1] = 0;
                        link_local_netif->ip6_address.addr[2] = netinfo->ip_addr.ip6_address.addr[2];
                        link_local_netif->ip6_address.addr[3] = netinfo->ip_addr.ip6_address.addr[3];
                        NBNETIF_LOGI("restore link local IP: %x:%x:%x:%x:%x:%x:%x:%x", \
                            IP6_ADDR_BLOCK1(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK2(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK3(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK4(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK5(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK6(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK7(&(link_local_netif->ip6_address)),         \
                            IP6_ADDR_BLOCK8(&(link_local_netif->ip6_address)));
                        nb_netif_send_message(MSG_ID_BEARER_INFO_ADD, (void *)link_local_netif);
                    } else {
                        NBNETIF_LOGI("allocate mem failed\r\n");
                    }
                }
            }
...
void nb_netif_backup_ip6_address(ip6_addr_t* ip6_address)
{
  ...
  for(i = 0; i < NB_NETIF_BACKUP_TOTAL_ITEM; i++) {
      if(g_lwip_nbiot_bearer_retention[i].is_activated == 1) {
          ...
          netinfo = &g_lwip_nbiot_bearer_retention[i];
          if (netinfo->type == 1) {
              NBNETIF_LOGI("backup ipv6 address:%d\r\n", i);
              memcpy(&(netinfo->ip_addr.ip6_address), ip6_address, sizeof(ip6_addr_t));
              netinfo->ip6_ready = 1;
              break;
          }
      }
...
void nb_netif_update_ip6_mtu(uint32_t mtu_size)
{
  ...
  for(i = 0; i < NB_NETIF_BACKUP_TOTAL_ITEM; i++) {
      if(g_lwip_nbiot_bearer_retention[i].is_activated == 1) {
          ...
          netinfo = &g_lwip_nbiot_bearer_retention[i];
          if (netinfo->type == 1) {
              NBNETIF_LOGI("update IP6 MTU size:%d, index:%d\r\n",mtu_size, i);
              netinfo->mtu = mtu_size;
          }
      }
  }
}
...
int nb_netif_bearer_info_ind(nbiot_bearer_info_struct *netinfo)
{
    ...
    if(netinfo->is_activated == 0) {
        ...
        ril_channel_forbid_data_receiving(netinfo->channel_id, false);
        nbnetif = pvPortMalloc(sizeof(nb_net_info_del_t));
        nbnetif->channel_id = channel_id;
        nbnetif->cid = cid;
        nb_clear_backup_item_by_cid(cid);
        ...
        memset(nbnetif, 0, sizeof(nb_net_info_add_t));
        if (netinfo->type == 0) {
            if (netinfo->pri_dns[LWIP_DNS_SERVER_IPV4_PRI_IDX] != 0) {
                NBNETIF_LOGI("DNS server:%s\r\n", netinfo->pri_dns);
                if(ipaddr_aton(netinfo->pri_dns, &dnsserver) != 0)
                    dns_setserver(0, &dnsserver);
            } else {
                NBNETIF_LOGI("used default DNS server\r\n");
                if(ipaddr_aton(NB_DEFAULT_FIRST_DNS_SERVER, &dnsserver) != 0)
                    dns_setserver(0, &dnsserver);
            }
            if (netinfo->snd_dns[LWIP_DNS_SERVER_IPV4_SEC_IDX] != 0) {
                NBNETIF_LOGI("Sec DNS server:%s\r\n", netinfo->snd_dns);
                if(ipaddr_aton(netinfo->snd_dns, &dnsserver) != 0)
                    dns_setserver(1, &dnsserver);
            } else {
                if(ipaddr_aton(NB_DEFAULT_SECOND_DNS_SERVER, &dnsserver) != 0)
                    dns_setserver(1, &dnsserver);
            }
        } else {
            if (netinfo->pri_dns[0] != 0) {
                u8_t ip6addr[16] = {0};
                char ipv6_format[48] = {0};
                nb_parse_ipv6_addr(netinfo->pri_dns, ip6addr);

                snprintf(ipv6_format, sizeof(ipv6_format), "%x:%x:%x:%x:%x:%x:%x:%x",
                         (ip6addr[0]<<8) | ip6addr[1], (ip6addr[2]<<8) | ip6addr[3], (ip6addr[4]<<8) | ip6addr[5],
                         (ip6addr[6]<<8) | ip6addr[7], (ip6addr[8]<<8) | ip6addr[9], (ip6addr[10]<<8) | ip6addr[11],
                         (ip6addr[12]<<8) | ip6addr[13], (ip6addr[14]<<8) | ip6addr[15]);
                NBNETIF_LOGI("IPV6 DNS server:%s\r\n", ipv6_format);
                if(ipaddr_aton(ipv6_format, &dnsserver) != 0)
                    dns_setserver(LWIP_DNS_SERVER_IPV6_PRI_IDX, &dnsserver);
            } else {
                NBNETIF_LOGI("used default IPV6 DNS server\r\n");
                //if(ipaddr_aton(NB_DEFAULT_FIRST_DNS_SERVER, &dnsserver) != 0)
                //    dns_setserver(0, &dnsserver);
            }
            if (netinfo->snd_dns[0] != 0) {
                u8_t ip6addr[16] = {0};
                char ipv6_format[48] = {0};
                nb_parse_ipv6_addr(netinfo->snd_dns, ip6addr);
                snprintf(ipv6_format, sizeof(ipv6_format), "%x:%x:%x:%x:%x:%x:%x:%x",
                        (ip6addr[0]<<8) | ip6addr[1], (ip6addr[2]<<8) | ip6addr[3], (ip6addr[4]<<8) | ip6addr[5],
                        (ip6addr[6]<<8) | ip6addr[7], (ip6addr[8]<<8) | ip6addr[9], (ip6addr[10]<<8) | ip6addr[11],
                        (ip6addr[12]<<8) | ip6addr[13], (ip6addr[14]<<8) | ip6addr[15]);
                NBNETIF_LOGI("Sec IPV6 DNS server:%s\r\n", ipv6_format);
                if(ipaddr_aton(ipv6_format, &dnsserver) != 0)
                    dns_setserver(LWIP_DNS_SERVER_IPV6_SEC_IDX, &dnsserver);
            } else {
                NBNETIF_LOGI("used default sec IPV6 DNS server\r\n");
                //if(ipaddr_aton(NB_DEFAULT_SECOND_DNS_SERVER, &dnsserver) != 0)
                //    dns_setserver(1, &dnsserver);
            }
        }
        ...
        if (netinfo->ip_addr[0] != 0) {
            ...
            #if LWIP_IPV6
                u8_t ip6addr[16] = {0};
                struct netif ipv6_netif;
                memset(&ipv6_netif, 0, sizeof(struct netif));
                ret = nb_parse_ipv6_addr(netinfo->ip_addr, ip6addr);
                netif_create_ip6_linklocal_address(&ipv6_netif, 0, ip6addr);
                memcpy(&(nbnetif->ip6_address), ipv6_netif.ip6_addr, sizeof(nbnetif->ip6_address));
                nbnetif->ip6_ready = 0;
            #endif
            } else {
            ...
            if(ret > 0) {
                ...
                mux_ap_change_callback(netinfo->channel_id, nb_low_level_mux_callback);
                ril_channel_forbid_data_receiving(netinfo->channel_id, false);
...
static int nb_netif_add_netinfo(nb_net_info_add_t *netinfo)
{
    ...
    if (netinfo->type == 1) {
        ...
        memcpy(netif->ip6_addr, &(netinfo->ip6_address), sizeof(netinfo->ip6_address));
        netif->ip6_addr_state[0] = IP6_ADDR_PREFERRED;
        netif->be_ip6 = 1;
        NBNETIF_LOGI("IPv6: %x:%x:%x:%x:%x:%x:%x:%x", \
            IP6_ADDR_BLOCK1(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK2(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK3(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK4(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK5(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK6(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK7(&(netinfo->ip6_address)),         \
            IP6_ADDR_BLOCK8(&(netinfo->ip6_address)));
    }
#endif
...
static int nb_netif_del_netinfo(u8_t context_id)
{
    ...
    while(netif != NULL) {
        result = 0;
        netif_remove(netif);
        NBNETIF_LOGI("del netinfo:0x%x, cid:%d\r\n", netif, context_id);
        mem_free(netif);
        netif = netif_find_by_context_id(context_id);
    }
    return result;
...
static int nb_cid_del(u8_t context_id, u32_t channel_id)
{
    ...
    nb_netif_cid_list_t *p = g_netif_context->cid_list;
    NBNETIF_LOGI("nb del cid, conext_d:%d, channel_id:%d \r\n", context_id, channel_id);
    while(p != NULL) {
...
static err_t nb_low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    ret = mux_ap_send_data_ex(channel_id, p->payload, p->len, (void *)p, true);
    ...
    switch (event_id) {
        ...
        case MUX_AP_EVENT_PREPARE_TO_RECEIVE: {
            ...
            if(p != NULL) {
                ...
                int *channel_id = NULL;
                NBNETIF_LOGI("Channel:%d stopped\r\n", info->channel_id);
    ...
    for (;;) {
        if (xQueueReceive(g_netif_context->qid, &msg_queue_data, portMAX_DELAY) == pdPASS) {
            ...
            if (MSG_ID_BEARER_INFO_DEL == msg_queue_data.msg_id) {
                ...
                #if LWIP_IPV6_SEND_ROUTER_SOLICIT
                if ((data->type == 1) && (data->ip6_ready == 0)) {
                ...
                while(q != NULL) {
                    ...
                    NBNETIF_LOGI("Channel:%d resumed\r\n", q->channel_id);
