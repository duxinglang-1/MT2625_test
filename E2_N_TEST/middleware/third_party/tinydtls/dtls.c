		...
		size_t buf_len_array[], size_t buf_array_len)
{
  ...
  res = CALL(ctx, dtlswrite, session, sendbuf, len);
...
handle_alert(dtls_context_t *ctx, dtls_peer_t *peer,
  ...
  (void)CALL(ctx, dtlsevent, &peer->session,
...
dtls_handle_message(dtls_context_t *ctx,
  ...
  while ((rlen = is_record(msg,msglen))) {
	...
	dtls_stop_retransmission(ctx, peer);
	CALL(ctx, dtlsevent, &peer->session, 0, DTLS_EVENT_CONNECTED);
      }
      break;
    ...
    case DTLS_CT_APPLICATION_DATA:
      ...
      dtls_stop_retransmission(ctx, peer);
      CALL(ctx, dtlsread, &peer->session, data, data_length);
      break;
    default:
...
dtls_connect(dtls_context_t *ctx, const session_t *dst) {
   ...
   * re-negotiation. */
  if (res > 0) {
    CALL(ctx, dtlsevent, &peer->session, 0, DTLS_EVENT_CONNECT);
  } else if (res == 0) {
    CALL(ctx, dtlsevent, &peer->session, 0, DTLS_EVENT_RENEGOTIATE);
  }
...
dtls_retransmit(dtls_context_t *context, netq_t *node) {
  ...
  if (node->retransmit_cnt < DTLS_DEFAULT_MAX_RETRANSMIT) {
	...
	return;
      ...
      (void)CALL(context, dtlswrite, &node->peer->session, sendbuf, len);
      return;
  }
