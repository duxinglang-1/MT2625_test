                                 ...
                                 lwm2m_server_t * bootstrapServer)
{
    ...
    query_length = lwm2m_utils_stringCopy(query, PRV_QUERY_BUFFER_LENGTH, QUERY_STARTER QUERY_NAME);
    if (query_length < 0)
    {
        bootstrapServer->status = STATE_BS_FAILING;
        return;
    }
    res = lwm2m_utils_stringCopy(query + query_length, PRV_QUERY_BUFFER_LENGTH - query_length, context->endpointName);
    if (res < 0)
    {
        bootstrapServer->status = STATE_BS_FAILING;
        return;
    ...
    {
        if (context->connect_server_callback) {
            bootstrapServer->sessionH = context->connect_server_callback(bootstrapServer->secObjInstID, context->userData);
        }
    }
    ...
    if (bootstrapServer->sessionH != NULL)
    {
        ...
        transaction = lwm2m_transaction_new(bootstrapServer->sessionH, COAP_POST, NULL, NULL, context->nextMID++, 4, NULL);
        if (transaction == NULL)
        {
            bootstrapServer->status = STATE_BS_FAILING;
            return;
        ...
        if (lwm2m_transaction_send(context, transaction) == 0)
        {
...
void lwm2m_bootstrap_step(lwm2m_context_t * contextP,
...
coap_status_t lwm2m_bootstrap_handleFinish(lwm2m_context_t * context,
                                     void * fromSessionH)
{
    lwm2m_server_t * bootstrapServer;

    LOG("Entering");
    bootstrapServer = lwm2m_utils_findBootstrapServer(context, fromSessionH);
    ...
    while (targetP != NULL)
    {
        ...
        if (targetP->sessionH == NULL)
        {
            if (contextP->connect_server_callback) {
                targetP->sessionH = contextP->connect_server_callback(targetP->secObjInstID, contextP->userData);
            }
        }
        targetP = targetP->next;
    }
...
coap_status_t lwm2m_bootstrap_handleCommand(lwm2m_context_t * contextP,
                                      lwm2m_uri_t * uriP,
                                      lwm2m_server_t * serverP,
                                      coap_packet_t * message,
    ...
    case COAP_PUT:
        {
            ...
            {
                if (lwm2m_object_isInstanceNew(contextP, uriP->objectId, uriP->instanceId))
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (i = 0 ; i < size ; i++)
                    {
                        ...
                        {
                            if (lwm2m_object_isInstanceNew(contextP, uriP->objectId, dataP[i].id))
                            {
                                        ...
                                        void * fromSessionH)
{
    ...
    LOG("Entering");
    serverP = lwm2m_utils_findBootstrapServer(contextP, fromSessionH);
                           ...
                           lwm2m_uri_t * uriP)
{
    ...
    LOG_URI(uriP);
    transaction = lwm2m_transaction_new(sessionH, COAP_DELETE, NULL, uriP, contextP->nextMID++, 4, NULL);
                          ...
                          size_t length)
{
    ...
    transaction = lwm2m_transaction_new(sessionH, COAP_PUT, NULL, uriP, contextP->nextMID++, 4, NULL);
    if (transaction == NULL) return COAP_500_INTERNAL_SERVER_ERROR;
                           ...
                           void * sessionH)
{
    ...
    LOG("Entering");
    transaction = lwm2m_transaction_new(sessionH, COAP_POST, NULL, NULL, contextP->nextMID++, 4, NULL);
    if (transaction == NULL) return COAP_500_INTERNAL_SERVER_ERROR;
