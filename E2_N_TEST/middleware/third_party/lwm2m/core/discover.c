                                               ...
                                               lwm2m_server_t * serverP)
{
    ...
    observedP = lwm2m_observe_findByUri(contextP, uriP);
                                   ...
                                   size_t bufferLen)
{
    ...
    if (paramP != NULL)
    {
        ...
        if (paramP->toSet & LWM2M_ATTR_FLAG_MIN_PERIOD)
        {
            ...
            res = lwm2m_utils_intToText(paramP->minPeriod, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
        else if (objectParamP->toSet & LWM2M_ATTR_FLAG_MIN_PERIOD)
        {
            ...
            res = lwm2m_utils_intToText(objectParamP->minPeriod, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
        ...
        if (paramP->toSet & LWM2M_ATTR_FLAG_MAX_PERIOD)
        {
            ...
            res = lwm2m_utils_intToText(paramP->maxPeriod, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
        else if (objectParamP->toSet & LWM2M_ATTR_FLAG_MAX_PERIOD)
        {
            ...
            res = lwm2m_utils_intToText(objectParamP->maxPeriod, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
        ...
        if (paramP->toSet & LWM2M_ATTR_FLAG_GREATER_THAN)
        {
            ...
            res = lwm2m_utils_floatToText(paramP->greaterThan, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
        if (paramP->toSet & LWM2M_ATTR_FLAG_LESS_THAN)
        {
            ...
            res = lwm2m_utils_floatToText(paramP->lessThan, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
        if (paramP->toSet & LWM2M_ATTR_FLAG_STEP)
        {
            ...
            res = lwm2m_utils_floatToText(paramP->step, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
        }
                                 ...
                                 size_t bufferLen)
{
    ...
    case LWM2M_TYPE_MULTIPLE_RESOURCE:
        ...
        res = lwm2m_utils_intToText(tlvP->id, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
        ...
        if (tlvP->type == LWM2M_TYPE_MULTIPLE_RESOURCE)
        {
            ...
            res = lwm2m_utils_intToText(tlvP->value.asChildren.count, buffer + head, bufferLen - head);
            if (res <= 0) return -1;
            head += res;
    ...
    case LWM2M_TYPE_OBJECT_INSTANCE:
    {
        ...
        res = lwm2m_utils_intToText(tlvP->id, uriStr + uriLen, URI_MAX_STRING_LEN - uriLen);
        if (res <= 0) return -1;
        uriLen += res;
...
int lwm2m_discover_serialize(lwm2m_context_t * contextP,
                       lwm2m_uri_t * uriP,
                       lwm2m_server_t * serverP,
                       int size,
                       lwm2m_data_t * dataP,
                       uint8_t ** bufferP)
{
    ...
    else
    {
        ...
        if (LWM2M_URI_IS_SET_INSTANCE(uriP))
        {
            ...
            res = lwm2m_utils_intToText(uriP->objectId, bufferLink + head, PRV_LINK_BUFFER_SIZE - head);
            if (res <= 0) return -1;
            head += res;
            PRV_CONCAT_STR(bufferLink, PRV_LINK_BUFFER_SIZE, head, LINK_URI_SEPARATOR, LINK_URI_SEPARATOR_SIZE);
            res = lwm2m_utils_intToText(uriP->instanceId, bufferLink + head, PRV_LINK_BUFFER_SIZE - head);
            if (res <= 0) return -1;
            head += res;
    ...
    baseUriLen = lwm2m_uri_toString(uriP, baseUriStr, URI_MAX_STRING_LEN, NULL);
