...
static int get_psk_info(struct dtls_context_t *ctx,
    ...
    switch (type) {
        ...
        case DTLS_PSK_KEY:
        {
            ...
            {
                LOG_ARG("result_length=%d, keyLen=%d\n", result_length, keyLen);
...
static dtls_handler_t cb = {
  .dtlswrite = send_to_peer,
  .dtlsread  = read_from_peer,
  .dtlsevent = NULL,
//#ifdef DTLS_PSK
...
void connection_free(dtls_connection_t * connList)
{
    while (connList != NULL)
    {
        ...
        nextP = connList->next;
        if (connList->dtlsSession != NULL) free(connList->dtlsSession);
        free(connList);
