                             ...
                             uint8_t ** bufferP)
{
    ...
    case LWM2M_TYPE_INTEGER:
        res = lwm2m_utils_int64ToPlainText(dataP->value.asInteger, bufferP);
        if (res == 0) return -1;
        return (int)res;

    case LWM2M_TYPE_FLOAT:
        res = lwm2m_utils_float64ToPlainText(dataP->value.asFloat, bufferP);
        if (res == 0) return -1;
        return (int)res;

    case LWM2M_TYPE_BOOLEAN:
        res = lwm2m_utils_boolToPlainText(dataP->value.asBoolean, bufferP);
        if (res == 0) return -1;
        return (int)res;
                              ...
                              lwm2m_data_t * dataP)
{
    ...
    LOG_ARG("lwm2m_data_encode_string \"%s\"", string);
    if (string == NULL)
    {
        len = 0;
    }
                          ...
                          int64_t * valueP)
{
    ...
    case LWM2M_TYPE_STRING:
        result = lwm2m_utils_plainTextToInt64(dataP->value.asBuffer.buffer, dataP->value.asBuffer.length, valueP);
        break;

    case LWM2M_TYPE_OPAQUE:
        result = lwm2m_utils_opaqueToInt(dataP->value.asBuffer.buffer, dataP->value.asBuffer.length, valueP);
                            ...
                            double * valueP)
{
    ...
    case LWM2M_TYPE_STRING:
        result = lwm2m_utils_plainTextToFloat64(dataP->value.asBuffer.buffer, dataP->value.asBuffer.length, valueP);
        break;

    case LWM2M_TYPE_OPAQUE:
        result = lwm2m_utils_opaqueToFloat(dataP->value.asBuffer.buffer, dataP->value.asBuffer.length, valueP);
        if (result == (int)dataP->value.asBuffer.length)
        {
            result = 1;
        }
...
void lwm2m_data_include(lwm2m_data_t * subDataP,
                        ...
                        lwm2m_data_t * dataP)
{
    LOG_ARG("lwm2m_data_include count: %d", count);
...
void lwm2m_data_encode_instances(lwm2m_data_t * subDataP,
                                 ...
                                 lwm2m_data_t * dataP)
{
    LOG_ARG("lwm2m_data_encode_instances count: %d", count);
...
int lwm2m_data_parse(lwm2m_uri_t * uriP,
                     ...
                     lwm2m_data_t ** dataP)
{
    LOG_ARG("lwm2m_data_parse format: %s, bufferLen: %d", STR_MEDIA_TYPE(format), bufferLen);
    LOG_URI(uriP);
    switch (format)
    {
                         ...
                         uint8_t ** bufferP)
{
    ...
    LOG_ARG("lwm2m_data_serialize Final format: %s", STR_MEDIA_TYPE(*formatP));
    ...
    case LWM2M_CONTENT_TLV:
        {
            ...
            return lwm2m_tlv_serialize(isResourceInstance, size, dataP, bufferP);
        }

#ifdef LWM2M_CLIENT_MODE
    case LWM2M_CONTENT_LINK:
        return lwm2m_discover_serialize(NULL, uriP, NULL, size, dataP, bufferP);
#endif
#ifdef LWM2M_SUPPORT_JSON
    case LWM2M_CONTENT_JSON:
