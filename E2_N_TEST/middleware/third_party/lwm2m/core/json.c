                             ...
                             lwm2m_data_t * targetP)
{
    ...
    case _TYPE_FLOAT:
    {
        ...
        if (i == recordP->valueLen)
        {
            ...
            if ( 1 != lwm2m_utils_plainTextToInt64(recordP->value,
                                             recordP->valueLen,
                                             &value))
            {
                return false;
        ...
        else
        {
            ...
            if ( 1 != lwm2m_utils_plainTextToFloat64(recordP->value,
                                               recordP->valueLen,
                                               &value))
            {
                return false;
                             ...
                             lwm2m_data_t ** dataP)
{
    ...
    for (index = 0 ; index < count ; index++)
    {
        ...
        if (targetP == NULL)
        {
            ...
            targetP->type = lwm2m_utils_depthToDatatype(rootLevel);
        }
        if (recordArray[index].ids[1] != LWM2M_MAX_ID)
        {
            ...
            for (i = 1 ; i <= resSegmentIndex ; i++)
            {
                ...
                if (targetP == NULL)
                {
                    ...
                    targetP->type = lwm2m_utils_depthToDatatype(level);
                }
               ...
               lwm2m_data_t ** dataP)
{
    ...
    LOG_ARG("json_parse bufferLen: %d, buffer: \"%s\"", bufferLen, (char *)buffer);
    LOG_URI(uriP);
    *dataP = NULL;
    recordArray = NULL;
    parsedP = NULL;
                              ...
                              size_t bufferLen)
{
    ...
    case LWM2M_TYPE_INTEGER:
    {
        ...
        res = lwm2m_utils_intToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
    ...
    case LWM2M_TYPE_FLOAT:
    {
        ...
        res = lwm2m_utils_floatToText(value, buffer + head, bufferLen - head);
        if (res <= 0) return -1;
        head += res;
    ...
    case LWM2M_TYPE_OPAQUE:
        ...
        res = lwm2m_utils_base64Encode(tlvP->value.asBuffer.buffer, tlvP->value.asBuffer.length, buffer+head, bufferLen - head);
        if (res == 0) return -1;
        head += res;
                   ...
                   uint8_t ** bufferP)
{
    ...
    LOG_ARG("json_serialize size: %d", size);
    LOG_URI(uriP);
    if (size != 0 && tlvP == NULL) return -1;

    baseUriLen = lwm2m_uri_toString(uriP, baseUriStr, URI_MAX_STRING_LEN, &rootLevel);
         ...
         || targetP->type == LWM2M_TYPE_MULTIPLE_RESOURCE))
    {
        ...
        res = lwm2m_utils_intToText(targetP->id, baseUriStr + baseUriLen, URI_MAX_STRING_LEN - baseUriLen);
        if (res <= 0) return 0;
        baseUriLen += res;
