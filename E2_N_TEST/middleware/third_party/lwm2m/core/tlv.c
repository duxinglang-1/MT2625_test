                    ...
                    size_t * oDataLen)
{
    ...
    LOG_ARG("lwm2m_decode_TLV buffer_len: %d", buffer_len);
              ...
              lwm2m_data_t ** dataP)
{
    ...
    LOG_ARG("lwm2m_tlv_parse bufferLen: %d", bufferLen);
    ...
    *dataP = NULL;
    ...
    while (0 != (result = lwm2m_decode_TLV((uint8_t*)buffer + index, bufferLen - index, &type, &id, &dataIndex, &dataLen)))
    {
        ...
        *dataP = newTlvP;
        ...
        {
            (*dataP)[size].value.asChildren.count = lwm2m_tlv_parse(buffer + index + dataIndex,
                                                          dataLen,
        ...
        case LWM2M_TYPE_INTEGER:
            {
                ...
                data_len = lwm2m_utils_encodeInt(dataP[i].value.asInteger, unused_buffer);
...
int lwm2m_tlv_serialize(bool isResourceInstance, 
                  int size,
                  lwm2m_data_t * dataP,
                  uint8_t ** bufferP)
{
    ...
    LOG_ARG("lwm2m_tlv_serialize isResourceInstance: %s, size: %d", isResourceInstance?"true":"false", size);
        ...
        case LWM2M_TYPE_OBJECT_INSTANCE:
            {
                ...
                res = lwm2m_tlv_serialize(isInstance, dataP[i].value.asChildren.count, dataP[i].value.asChildren.array, &tmpBuffer);
                if (res < 0)
                {
                    length = -1;
                }
        ...
        case LWM2M_TYPE_INTEGER:
            {
                ...
                data_len = lwm2m_utils_encodeInt(dataP[i].value.asInteger, data_buffer);
        ...
        case LWM2M_TYPE_FLOAT:
            {
                ...
                data_len = lwm2m_utils_encodeFloat(dataP[i].value.asFloat, data_buffer);
                headerLen = prv_createHeader(*bufferP + index, isInstance, dataP[i].type, dataP[i].id, data_len);
                index += headerLen;
                memcpy(*bufferP + index, data_buffer, data_len);
                index += data_len;
