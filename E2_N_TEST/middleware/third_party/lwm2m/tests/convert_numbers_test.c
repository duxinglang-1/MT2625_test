...
static void test_lwm2m_PlainTextToInt64(void)
{
    ...
    for (i = 0 ; tests[i] != NULL ; i++)
    {
        ...
        if (lwm2m_utils_plainTextToInt64((unsigned char*)tests[i], strlen(tests[i]), &res) != 1)
            res = -1;
...
static void test_lwm2m_PlainTextToFloat64(void)
{
    ...
    for (i = 0 ; tests[i] != NULL ; i++)
    {
        ...
        if (lwm2m_utils_plainTextToFloat64((unsigned char*)tests[i], strlen(tests[i]), &res) != 1)
            res = -1;
...
static void test_lwm2m_int64ToPlainText(void)
{
    ...
    for (i = 0 ; i < sizeof(ints)/sizeof(int64_t); i++)
    {
        ...
        len = lwm2m_utils_int64ToPlainText(ints[i], (uint8_t**)&res);
...
static void test_lwm2m_float64ToPlainText(void)
{
    ...
    for (i = 0 ; i < sizeof(floats)/sizeof(floats[0]); i++)
    {
        ...
        len = lwm2m_utils_float64ToPlainText(floats[i], (uint8_t**)&res);
...
static struct TestTable table[] = {
        { "test of lwm2m_utils_plainTextToInt64()", test_lwm2m_PlainTextToInt64 },
        { "test of lwm2m_utils_plainTextToFloat64()", test_lwm2m_PlainTextToFloat64 },
        { "test of lwm2m_utils_int64ToPlainText()", test_lwm2m_int64ToPlainText },
        { "test of lwm2m_utils_float64ToPlainText()", test_lwm2m_float64ToPlainText },
        { NULL, NULL },
};
