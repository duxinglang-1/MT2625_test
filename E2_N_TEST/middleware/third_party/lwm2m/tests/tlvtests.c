...
static void test_opaqueToInt()
{
    ...
    length = lwm2m_utils_opaqueToInt(data1, sizeof(data1), &value);
    CU_ASSERT_EQUAL(length, 1);
    CU_ASSERT_EQUAL(value, 1);

    length = lwm2m_utils_opaqueToInt(data2, sizeof(data2), &value);
    CU_ASSERT_EQUAL(length, 2);
    CU_ASSERT_EQUAL(value, 0x102);

    length = lwm2m_utils_opaqueToInt(data3, sizeof(data3), &value);
    CU_ASSERT_EQUAL(length, 4);
    CU_ASSERT_EQUAL(value, -0x1020304);

    length = lwm2m_utils_opaqueToInt(data4, sizeof(data4), &value);
