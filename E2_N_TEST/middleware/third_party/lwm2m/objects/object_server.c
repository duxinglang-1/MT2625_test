...
static uint8_t prv_server_write(void * contextP,
                                uint16_t instanceId,
                                int numData,
                                ...
                                lwm2m_object_t * objectP)
{
    ...
    LOG_ARG("prv_server_write instanceId:%d", instanceId);
    ...
    do
    {
        ...
        case LWM2M_SERVER_SHORT_ID_ID:
            {
                ...
                if (COAP_204_CHANGED == result)
                {
                    if (0 < value && 0xFFFF >= value)
                    {
                        ...
                        {
                            ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_SERVER_INSTANCE_ID, LWM2M_NOTIFY_CODE_SUCCESS, instanceId);
                            ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_SHORT_SERVER_ID, LWM2M_NOTIFY_CODE_SUCCESS, targetP->shortServerId);
                        }
                    }
                    else
                    {
        ...
        case LWM2M_SERVER_LIFETIME_ID:             
            ...
            if (COAP_204_CHANGED == result)
            {
                if (((lwm2m_context_t *)contextP)->notify_callback != NULL)
                {
                    ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_LIFETIME_CHANGED, LWM2M_NOTIFY_CODE_SUCCESS, targetP->lifetime);
                }
            }
            break;
        ...
        case LWM2M_SERVER_MIN_PERIOD_ID:
            ...
            if (COAP_204_CHANGED == result)
            {
                if (((lwm2m_context_t *)contextP)->notify_callback != NULL)
                {
                    ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_MIN_PERIOD_CHANGED, LWM2M_NOTIFY_CODE_SUCCESS, targetP->defaultMinPeriod);
                }
            }
            break;
        ...
        case LWM2M_SERVER_MAX_PERIOD_ID:
            ...
            if (COAP_204_CHANGED == result)
            {
                if (((lwm2m_context_t *)contextP)->notify_callback != NULL)
                {
                    ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_MAX_PERIOD_CHANGED, LWM2M_NOTIFY_CODE_SUCCESS, targetP->defaultMaxPeriod);
                }
            }
            break;
        ...
        case LWM2M_SERVER_STORING_ID:
        {
            ...
            if (1 == lwm2m_data_decode_bool(dataArray + i, &value))
            {
                ...
                targetP->storing = false; // should set false since we didn't support it??
                result = COAP_204_CHANGED;
            }
            else
            {
        ...
        case LWM2M_SERVER_BINDING_ID:
              ...
              || strncmp((char*)dataArray[i].value.asBuffer.buffer, "UQS", dataArray[i].value.asBuffer.length) == 0))
            {
                ...
                if (((lwm2m_context_t *)contextP)->notify_callback != NULL)
                {
                    ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_BINDING_CHANGED, LWM2M_NOTIFY_CODE_SUCCESS, (uint32_t)targetP->binding);
                }
                result = COAP_204_CHANGED;
            }
            else
...
static uint8_t prv_server_execute(void * contextP,
                                  uint16_t instanceId,
                                  uint16_t resourceId,
                                  uint8_t * buffer,
                                  int length,
...

{
    ...
    case LWM2M_SERVER_DISABLE_ID:
        ...
        if (0 < targetP->disableTimeout){
            if (((lwm2m_context_t *)contextP)->notify_callback != NULL)
            {
                ((lwm2m_context_t *)contextP)->notify_callback(LWM2M_NOTIFY_TYPE_DISABLE, LWM2M_NOTIFY_CODE_SUCCESS, targetP->disableTimeout);
            }
    ...
    case LWM2M_SERVER_UPDATE_ID:
        // executed in core, if COAP_204_CHANGED is returned
        lwm2m_update_registration((lwm2m_context_t *)contextP, 0, false);
        return COAP_204_CHANGED;
    default:
        return COAP_405_METHOD_NOT_ALLOWED;
    }
                                 ...
                                 lwm2m_object_t * objectP)
{
    ...
    LOG_ARG("prv_server_delete id:%d", id);
...
static uint8_t prv_server_create(void * contextP,
                                 uint16_t instanceId,
                                 int numData,
                                 lwm2m_data_t * dataArray,
                                 lwm2m_object_t * objectP)
{
    ...
    LOG_ARG("prv_server_create instanceId:%d", instanceId);
    ...
    result = prv_server_write(contextP, instanceId, numData, dataArray, objectP);
...
void set_server_binding(lwm2m_context_t * contextP, lwm2m_binding_t binding, uint16_t instanceId)
{
    ...
    else
    {
        ...
        {
        case BINDING_U:
            memcpy(targetP->binding, "U", sizeof(targetP->binding));
            break;
        case BINDING_UQ:
            memcpy(targetP->binding, "UQ", sizeof(targetP->binding));
            break;
        case BINDING_S:
            memcpy(targetP->binding, "S", sizeof(targetP->binding));
            break;
        case BINDING_SQ:
            memcpy(targetP->binding, "SQ", sizeof(targetP->binding));
            break;
        case BINDING_US:
            memcpy(targetP->binding, "US", sizeof(targetP->binding));
            break;
        case BINDING_UQS:
            memcpy(targetP->binding, "UQS", sizeof(targetP->binding));
            break;
        default:
            break;
        }
...
lwm2m_object_t * get_server_object_ex(int instanceId,
                                      int serverId,
                                      const char* binding,
                                      int lifetime,
                                      bool storing)
{
    ...
    if (NULL != serverObj)
    {
        ...
        serverInstance->defaultMinPeriod = LWM2M_SERVER_DEFAULT_MIN_PERIOD;
        serverInstance->defaultMaxPeriod = LWM2M_SERVER_DEFAULT_MAX_PERIOD;
        serverInstance->disableTimeout = LWM2M_SERVER_DISABLE_TIMEOUT;
...
{
    return get_server_object_ex(0, serverId, binding, lifetime, storing);
}
