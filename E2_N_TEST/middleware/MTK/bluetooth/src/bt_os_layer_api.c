...
#if defined(__GNUC__)
ATTR_RWDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t plain_value[16] = {0};
ATTR_RWDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t key_value[16] = {0};
ATTR_RWDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t encrypted_buffer[32] = {0};
#endif
...
{
    #if defined(__GNUC__)
    hal_aes_buffer_t temp_plain_text = {
        .buffer = plain_value,
        .length = sizeof(plain_value)
    };
    hal_aes_buffer_t temp_key = {
        .buffer = key_value,
        .length = sizeof(key_value)
    };
    hal_aes_buffer_t temp_encrypted_text = {
        .buffer = encrypted_buffer,
        .length = sizeof(encrypted_buffer)
    };
    BT_LOGI("BT", "bt_os_layer_aes_encrypt use hal version");
    memcpy(temp_plain_text.buffer, plain_text->buffer, sizeof(plain_value));
    temp_plain_text.length = plain_text->length;
    memcpy(temp_key.buffer, key->buffer, sizeof(key_value));
    temp_key.length = key->length;
    hal_aes_ecb_encrypt((hal_aes_buffer_t*)&temp_encrypted_text, (hal_aes_buffer_t*)&temp_plain_text, (hal_aes_buffer_t*)&temp_key);
    memcpy(encrypted_data->buffer, temp_encrypted_text.buffer, sizeof(encrypted_buffer));
    encrypted_data->length = temp_encrypted_text.length;
    #else
...
{
#ifdef HAL_SLEEP_MANAGER_ENABLED
    return (uint8_t)(hal_sleep_manager_acquire_sleeplock(handle_index, (hal_sleep_lock_t)level));
#else
    return 0;
#endif
}
...
{
#ifdef HAL_SLEEP_MANAGER_ENABLED
    return (uint8_t)(hal_sleep_manager_release_sleeplock(handle_index, (hal_sleep_lock_t)level));
#else
    return 0;
#endif
}
...
{
#ifdef HAL_SLEEP_MANAGER_ENABLED
    return rtc_power_on_result_external();
#else
    return 0;
#endif
}
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
    sleep_management_register_suspend_callback((sleep_management_backup_restore_module_t)module, callback, data, mode);
#else
#endif
}

uint8_t bt_os_layer_get_sleep_management_backup_restore_module_id(void) {
#ifdef HAL_SLEEP_MANAGER_ENABLED
    sleep_management_backup_restore_module_t module;
    module = SLEEP_BACKUP_RESTORE_USER;
    return module;
#else
    return 0;
#endif
...
#ifdef MTK_NVDM_MODEM_ENABLE
    return (int16_t)(nvdm_modem_init());
#else
#ifdef MTK_NVDM_ENABLE
    return (int16_t)(nvdm_init());
#else
    return -1;
#endif  // MTK_NVDM_ENABLE
#endif  // MTK_NVDM_MODEM_ENABLE
...
int16_t bt_os_layer_nvdm_write_data_item(const char *group_name,
                                   ...
                                   uint8_t attr) {
#ifdef MTK_NVDM_MODEM_ENABLE
    return (int16_t)(nvdm_modem_write_normal_data_item(group_name,
                                                 data_item_name,
...
#else
#ifdef MTK_NVDM_ENABLE
    return (int16_t)(nvdm_write_data_item(group_name,
                                    data_item_name,
...
int16_t bt_os_layer_nvdm_read_data_item(const char *group_name,
                                  ...
                                  uint8_t *buffer,
                                  uint32_t *size) {
    nvdm_modem_data_item_type_t nvdm_modem_type = (nvdm_modem_data_item_type_t)type;
#ifdef MTK_NVDM_MODEM_ENABLE
    return (int16_t)(nvdm_modem_read_normal_data_item(group_name,
                                                data_item_name,
                                                &nvdm_modem_type,
                                                buffer,
                                                size));
#else
#ifdef MTK_NVDM_ENABLE
    return (int16_t)(nvdm_read_data_item(group_name,
                                   data_item_name,
                                   buffer,
                                   size));
#else
...
int16_t bt_os_layer_nvdm_modem_reset_data_item(const char *group_name,
                                  const char *data_item_name) {
#ifdef MTK_NVDM_MODEM_ENABLE
    return (int16_t)(nvdm_modem_reset_normal_data_item(group_name,
                                                 data_item_name));
#else
    return -1;
#endif  // MTK_NVDM_MODEM_ENABLE
