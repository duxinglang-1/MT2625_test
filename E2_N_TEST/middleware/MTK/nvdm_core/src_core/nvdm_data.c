...
#ifdef MTK_NVDM_OTA_SUPPORT
        p_data_item_header->verno = nvdm_get_ap_item_verno(group_name, data_item_name);
#endif
...
void data_item_scan(int32_t pnum)
{
    ...
    while (offset < (g_nvdm_peb_size - PEB_HEADER_SIZE - DATA_ITEM_HEADER_SIZE)) {
        ...
        peb_read_data(pnum, data_item_header.offset + DATA_ITEM_HEADER_SIZE, &group_name[0], data_item_header.group_name_size);
        peb_read_data(pnum, data_item_header.offset + DATA_ITEM_HEADER_SIZE + data_item_header.group_name_size, &data_item_name[0], data_item_header.data_item_name_size);

        if (data_item_header.verno != nvdm_get_ap_item_verno((char *)group_name, (char *)data_item_name)) {
            /* change the status of data item in flash from valid to delete */
            status = DATA_ITEM_STATUS_DELETE;
