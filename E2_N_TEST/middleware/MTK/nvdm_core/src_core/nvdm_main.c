...
{
    int32_t i, sum_valid, merge_cnt, merge_start, hard_merge, merge_loop;
...
static void garbage_collection_peb(void)
{
    ...
    for (i = 0; i < g_nvdm_peb_count; i++) {
        ...
        cur_valid = (g_nvdm_peb_size - PEB_HEADER_SIZE) - g_pebs_info[i].dirty - g_pebs_info[i].free;
        if (cur_valid == 0) {
            peb_list[found_blocks++] = i;
        }
    }
    if (found_blocks) {
        nvdm_port_log_info("reclaim blocks select by empty PEBs = %d", found_blocks);
        nvdm_port_log_info("reclaim peb_list(empty PEBs): ");
...
void space_sub_valid(int32_t size)
{
    ...
    if (g_valid_data_size < 0) {
        nvdm_port_log_error("detect g_valid_data_size abnormal");
    }
}

static bool peb_check_peb_integrity(int32_t pnum, peb_header_t *peb_hdr)
{
    if ((peb_hdr->magic == 0xFFFFFFFF) && (peb_hdr->status == PEB_STATUS_VIRGIN)) {
        ...
        nvdm_port_log_notice("PEB%d:integrity check failed, erase again", pnum);
        peb_erase(pnum);
        return false;
    }
...
static void peb_scan(void)
{
    ...
    for (i = 0; i < g_nvdm_peb_count; i++) {
        ...
        if (peb_check_peb_integrity(i, &peb_hdr) == false) {
            peb_read_header(i, &peb_hdr);
        }
