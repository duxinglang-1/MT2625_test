...
#define IPERF_WDT_TIMER 10 /*10 seconds.*/
...
typedef struct _iperf_context
{
    ...
    uint32_t end_count;
    int is_tradeoff_test_client;
    int is_tradeoff_test_server;
}iperf_context_t;

// Private variables -----------------------------------------------------------
uint32_t g_iperf_debug_feature = 0;
static iperf_context_t g_iperf_context[IPERF_TYPE_MAX] = {0};

// Private function prototypes -------------------------------------------------
static void iperf_calculate_result(int pkt_size, count_t *pkt_count);
static void iperf_display_report(iperf_context_t* iperf_context, char *report_title, unsigned time, unsigned h_ms_time, count_t *pkt_count);
...
static void iperf_get_current_time(iperf_context_t* iperf_context, uint32_t *s, uint32_t *ms);
...
void iperf_udp_run_server(char *parameters[])
{
...
#endif
    char *buffer = NULL;
    uint32_t curr_t, curr_h_ms, t2_h_ms, t1_h_ms, tmp_t, tmp_h_ms, offset_t1, offset_t2, offset_time;
    uint32_t t1 = 0;
    uint32_t t2 = 0;
    UDP_datagram *udp_h;
    client_hdr *client_h;
    ...
    iperf_context_t* iperf_context_p = g_iperf_context + IPERF_UDP_SERVER;
    uint32_t* start_count_p = &(iperf_context_p->start_count);
    ...
    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_32K, start_count_p);
    server_port = 0;
    t1 = 0;
    t2 = 0;
    ...
    if (iperf_context_p->is_tradeoff_test_client == 0) {
        for (i = 0; i < 13; i++) {
            if (strcmp((char *)&parameters[i * offset], "-p") == 0) {
                ...
                i++;
                timeout.tv_sec = atoi((char *)&parameters[i * offset]);
                IPERF_LOGI("Set timeout = %d (secs)\n", atoi((char *)&parameters[i * offset]));
            }
        }
    }
    ...
    if (sockfd < 0) {
        ...
        iperf_context_p->is_iperf_running = false;
        vTaskDelete(NULL);
    }
    ...
    do {
        // Handles request
        do {
            iperf_get_current_time(iperf_context_p, &offset_t1, 0);
            if (ipv6_flag == 0) {
...
#endif
            iperf_get_current_time(iperf_context_p, &offset_t2, 0);
            ...
            if (pkt_count.times == 1) {
                iperf_get_current_time(iperf_context_p, &t1, &t1_h_ms);
                t1_h_ms = (t1_h_ms / 100) % 10;
            }

            // Report by second
            if ((pkt_count.times >= 1 && interval_tag > 0)) {
                iperf_get_current_time(iperf_context_p, &curr_t, &curr_h_ms);
                ...
                if ((((curr_t - t1) / 10) == interval_tag) && ((curr_h_ms >= t1_h_ms) || ((curr_t - t1) % 10) >= 1)) {
                    ...
                    iperf_display_report(iperf_context_p, "UDP Server", 10, 0, &result_count);
                    ...
                    iperf_diff_count(&result_count, &pkt_count, &tmp_count);
                    iperf_display_report(iperf_context_p, "UDP Server", (tmp_t - ((curr_t - t1 + 1) / 10 * 10 - 10)), tmp_h_ms, &result_count);
                    iperf_copy_count(&pkt_count, &tmp_count);
                    interval_tag++;
                }
            }
            ...
            if ((is_test_started == 0) && (udp_h_id > 0) && (nbytes > 0)) {
                ...
                if (pkt_count.times >= 1) {
                    ...
                    if (interval_tag > 0) {
                        ...
                        iperf_get_current_time(iperf_context_p, &t2, &t2_h_ms);
                ...
                // print out result
                iperf_display_report(iperf_context_p, "[Total]UDP Server", t2, t2_h_ms, &pkt_count);
                ...
                if (udp_h_id < 0) {
                    ...
                    if (ipv6_flag == 0) {
                        ...
                        send_bytes = sendto(sockfd, buffer, nbytes, 0, (struct sockaddr *)&cliaddr6, cli_len);
                    }
#endif
                    (void)send_bytes;
                ...
                // Tradeoff mode
                if (ipv6_flag == 0 && IPERF_HEADER_VERSION1 & client_h_trans.flags) {
                    IPERF_LOGI("Tradeoff mode, client-side start.\n");

                    iperf_context_t* iperf_udp_client_context_p = g_iperf_context + IPERF_UDP_CLIENT;
                    iperf_udp_client_context_p->is_tradeoff_test_server = 1;
                    iperf_udp_client_context_p->server_addr = cliaddr.sin_addr.s_addr;
                    iperf_udp_client_context_p->port = ntohl(client_h->port);
                    iperf_udp_client_context_p->buffer_len = ntohl(client_h->buffer_len);
                    iperf_udp_client_context_p->win_band = ntohl(client_h->win_band);
                    iperf_udp_client_context_p->amount = ntohl(client_h->amount);
                    iperf_udp_run_client(NULL);
                    iperf_udp_client_context_p->is_tradeoff_test_server = 0;
    ...
    iperf_context_p->result_t.data_size = 0;
    iperf_context_p->result_t.send_time = 0;
    if (iperf_context_p->callback) {
        iperf_context_p->callback(&(iperf_context_p->result_t));
    }
...
void iperf_tcp_run_server(char *parameters[])
{
    ...
    int offset = IPERF_COMMAND_BUFFER_SIZE / sizeof(char *);
    int ret = 0;
    iperf_context_t* iperf_context_p = g_iperf_context + IPERF_TCP_SERVER;
    uint32_t* start_count_p = &(iperf_context_p->start_count);

    struct timeval timeout;
#ifdef MTK_TCPIP_FOR_NB_MODULE_ENABLE
    timeout.tv_sec = 120; //set recvive timeout = 20(sec)
#else
        ...
        do {
            ...
            if (connfd != -1) {
                ...
                do {
                    ...
                    iperf_calculate_result(nbytes, &pkt_count);
                    if (pkt_count.times == 1) {
                        iperf_get_current_time(iperf_context_p, &t1, 0);
                    }
#if defined(MTK_IPERF_DEBUG_ENABLE)
                    if (tmp != nbytes) {
                        DBGPRINT_IPERF(IPERF_DEBUG_RECEIVE, ("\n[%s:%d] nbytes=%d \n", __FUNCTION__, __LINE__, nbytes));
                    ...
                    if (pkt_count.times >= 1 && interval_tag > 0) {
                        iperf_get_current_time(iperf_context_p, &curr_t, 0);
                        if (((curr_t - t1) / 10) == interval_tag) {
                            count_t result_count;
                            IPERF_LOGI("\nInterval: %d - %d sec   ", (int)(curr_t - t1) / 10 * 10 - 10, (int)(curr_t - t1) / 10 * 10);
                            iperf_diff_count(&result_count, &pkt_count, &tmp_count);
                            iperf_display_report(iperf_context_p, "TCP Server", 10, 0, &result_count);
                            iperf_copy_count(&pkt_count, &tmp_count);
                            interval_tag++;
                        }
                    }
                ...
                if (pkt_count.times >= 1) {
                    iperf_get_current_time(iperf_context_p, &t2, 0);
                }


                IPERF_LOGI("\nClose socket!\n");
                //Get report
                iperf_display_report(iperf_context_p, "[Total]TCP Server", t2 - t1, 0, &pkt_count);
...
void iperf_tcp_run_client(char *parameters[])
{
    ...
    uint32_t t1, t2, curr_t, wdt_time;
    int offset = IPERF_COMMAND_BUFFER_SIZE / sizeof(char *);
    int ret = 0;
    iperf_context_t* iperf_context_p = g_iperf_context + IPERF_TCP_CLIENT;
    uint32_t* start_count_p = &(iperf_context_p->start_count);
    ...
    do {
        ...
        if (interval_tag > 0) {
            ...
            if (((curr_t - t1) / 10) == interval_tag) {
                ...
                IPERF_LOGI("\nInterval: %d - %d sec   ", (int)(curr_t - t1) / 10 * 10 - 10, (int)(curr_t - t1) / 10 * 10);
                iperf_diff_count(&result_count, &pkt_count, &tmp_count);
                iperf_display_report(iperf_context_p, "TCP Client", 10, 0, &result_count);
                iperf_copy_count(&pkt_count, &tmp_count);
                interval_tag++;
            }
        }
        ...
        iperf_get_current_time(iperf_context_p, &curr_t, 0);
        if ((curr_t - wdt_time) >= IPERF_WDT_TIMER){
            wdt_time = curr_t;
    ...
    iperf_display_report(iperf_context_p, "[Total]TCP Client", t2 - t1, 0, &pkt_count);

    if (parameters) {
        vPortFree(parameters);
    }

    iperf_context_p->result_t.data_size = win_size;
    iperf_context_p->result_t.send_time = send_time;
    if (iperf_context_p->callback)
        iperf_context_p->callback(&(iperf_context_p->result_t));
...
void iperf_udp_run_client(char *parameters[])
{
    ...
    uint32_t t1, t2, curr_t, t1_ms, t2_ms, last_tick, current_tick, last_sleep, current_sleep, wdt_time;
    UDP_datagram *udp_h;
    client_hdr *client_h;
    int udp_h_id = 0;
    int offset = IPERF_COMMAND_BUFFER_SIZE / sizeof(char *);
    int ret = 0;
    iperf_context_t* iperf_context_p = g_iperf_context + IPERF_UDP_CLIENT;
    uint32_t* start_count_p = &(iperf_context_p->start_count);
    //Statistics init
    iperf_reset_count(&pkt_count);
    iperf_reset_count(&tmp_count);
    ...
    //Handle input parameters
    if (iperf_context_p->is_tradeoff_test_server == 0) {
        Server_IP = (char *)&parameters[0];
        for (i = 1; i < 18; i++) {
            if (strcmp((char *)&parameters[i * offset], "-l") == 0) {
                i++;
                ...
                total_send = iperf_format_transform((char *)&parameters[i * offset]);
                num_tag = 1;
                IPERF_LOGI("Set number to transmit = %d packet\n", total_send);
            } else if (strcmp((char *)&parameters[i * offset], "-S") == 0) {
                i++;
                tos = atoi((char *)&parameters[i * offset]);
                IPERF_LOGI("Set TOS = %d \n", atoi((char *)&parameters[i * offset]));
    ...
    if (iperf_context_p->is_tradeoff_test_server == 0) {
        ...
        if (ipv6_flag == 0) {
            servaddr.sin_addr.s_addr = iperf_context_p->server_addr;
        }
#if LWIP_IPV6
        else {
            ip6_addr_t ip6addr;
...
#endif
        server_port = iperf_context_p->port;
        bw = iperf_context_p->win_band / 8;
        total_send = iperf_context_p->amount;
        num_tag = 1;
    }
    if (ipv6_flag == 0) {
    ...
    iperf_context_p->result_t.data_size = data_size;
    ...
    if (bw > 0) {
        ...
        IPERF_LOGI("pkt_delay=%d, pkt_delay_offset=%d\r\n", pkt_delay, pkt_delay_offset);
    }
    ...
    iperf_get_current_time(iperf_context_p, &t1, &t1_ms);
    last_tick = t1_ms;
    last_sleep = 0;
    wdt_time = t1;
    ...
    do {
        ...
        iperf_get_current_time(iperf_context_p, &curr_t, &current_tick);
...
#endif
        //IPERF_LOGI("nbytes = %d, current_tick = %d, send_time = %d, total_send = %d", nbytes, current_tick, send_time, total_send);
        if (num_tag == 1 && nbytes > 0) {
            total_send--;
        }
        ...
        if (interval_tag > 0) {
            if (((current_tick - t1_ms) / 10000) == interval_tag) {
                count_t result_count;
                IPERF_LOGI("Interval: %d - %d sec   ", (int)(current_tick - t1_ms) / 10000 * 10 - 10, (int)(current_tick - t1_ms) / 10000 * 10);
                iperf_diff_count(&result_count, &pkt_count, &tmp_count);
                iperf_display_report(iperf_context_p, "UDP Client", 10, 0, &result_count);
                iperf_copy_count(&pkt_count, &tmp_count);
                interval_tag++;
            }
            iperf_get_current_time(iperf_context_p, &curr_t, &current_tick);
    ...
    iperf_get_current_time(iperf_context_p, &t2, &t2_ms);
    if ((t2 - t1) == 0) {
        iperf_display_report(iperf_context_p, "[Total]UDP Client", 0, t2_ms - t1_ms, &pkt_count);
    } else {
        iperf_display_report(iperf_context_p, "[Total]UDP Client", t2 - t1, 0, &pkt_count);
    }
    ...
    if (tradeoff_tag == 1) {
        ...
        iperf_context_t* iperf_udp_server_context_p = g_iperf_context + IPERF_UDP_SERVER;
        iperf_udp_server_context_p->is_tradeoff_test_client = 1;
        iperf_udp_run_server(NULL);
        iperf_udp_server_context_p->is_tradeoff_test_client = 0;
    }
...
static void iperf_display_report(iperf_context_t* iperf_context, char *report_title, unsigned time, unsigned h_ms_time, count_t *pkt_count)
{
    double tmp_time = time + (double)h_ms_time/(double)1000.0;
    char s[9] = {0};
    double tput = 0.0;
    int conv;
    if (iperf_context == NULL) {
        return;
    }
    memcpy(iperf_context->result_t.report_title, report_title, strlen(report_title));
#if defined(MTK_IPERF_DEBUG_ENABLE)
    ...
    sprintf(iperf_context->result_t.total_len, "%s %s", s, kLabel_Byte[conv]);
    ...
    sprintf(iperf_context->result_t.result, "%s %s/sec.", s, kLabel_bit[conv]);
...
static void iperf_get_current_time(iperf_context_t* iperf_context, uint32_t *s, uint32_t *ms)
{
    ...
    hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_32K, &(iperf_context->end_count));
    hal_gpt_get_duration_count(iperf_context->start_count, iperf_context->end_count, &count);
    if (s) {
        *s = count / 32768;
    }
...
{
    iperf_context_t* iperf_context_p = NULL;
    if (type >= IPERF_TYPE_MAX) {
        return false;
    }
    iperf_context_p = g_iperf_context + type;
    return iperf_context_p->is_iperf_running;
}
...
void iperf_register_callback(iperf_type_t type, iperf_callback_t callback)
{
    ...
    iperf_context_p = g_iperf_context + type;
    iperf_context_p->callback = callback;
    iperf_context_p->is_iperf_running = true;
}
