...
void wifi_host_get_scan_result(uint8_t *payload)
{
    uint32_t i = 0;
    wh_scan_list_t *scan_list_temp = (wh_scan_list_t *)payload;
    wifi_scan_list_item_t* scan_ap_list_temp = scan_list_temp->scan_list;
    uint32_t scan_ap_size_temp = scan_list_temp->list_size;

    if((g_scan_list == NULL) || (g_scan_list_size == 0)) {
        return;
    }
    if(scan_ap_size_temp > g_scan_list_size) {
        printf("receive the error number of scan table.\r\n");
        return;
    }
    
    for (i = 0; i < scan_ap_size_temp; i++) {
        os_memcpy(&g_scan_list[i], &scan_ap_list_temp[i], sizeof(wifi_scan_list_item_t));
    }
