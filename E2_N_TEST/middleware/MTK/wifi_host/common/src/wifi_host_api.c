...
static uint8_t g_wifi_start_flag = 1;
int32_t wifi_host_config_set_wifi_start(void)
{
    int32_t ret = 0;
    if(g_wifi_start_flag == 0) {
#if (PRODUCT_VERSION == 2523)
        hal_gpio_set_output(HAL_GPIO_48, HAL_GPIO_DATA_HIGH); 
#elif (PRODUCT_VERSION == 2625)
        hal_gpio_set_output(HAL_GPIO_11, HAL_GPIO_DATA_HIGH); //pull down chip_en
#endif        
        wfcm_stub_init();
//#if (PRODUCT_VERSION == 2523)
        wifi_host_main_init();
...
#if (PRODUCT_VERSION == 2523)
        hal_gpio_set_output(HAL_GPIO_48, HAL_GPIO_DATA_LOW); //pull down chip_en
#elif (PRODUCT_VERSION == 2625)
        hal_gpio_set_output(HAL_GPIO_11, HAL_GPIO_DATA_LOW); //pull down chip_en
#endif        
        wfcm_stub_deinit();
        g_wifi_start_flag = 0;
    }else {
