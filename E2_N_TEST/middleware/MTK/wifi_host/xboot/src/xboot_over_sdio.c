...
/*----------------------------------------------------------------------------*/
/* Temp porting: not formal. */
#if !defined(HAL_LEGACY_METHOD)
#include "hal.h"
void wifi_gpt_delay(uint32_t ms)
{
   hal_gpt_delay_ms(ms);
}
#endif
...
#else
    ret = wifi_write_bus(u4Register, Length, pBuffer);
#endif
    if (!ret) {
...
#else
    ret = wifi_read_bus(u4Register, Length, pBuffer);
#endif
...
bool xboot_over_sdio_fw_download(uint8_t *address, uint32_t length)
{
  ...
  #endif
#else
    //Move wifi_chip_hw_power_on & wifi_chip_hw_power_off to caller
#endif
...
#else
    //Lack: hal_sdio_get_block_size
    //Defined in wifi_bus_init under sdio_hal.c
#endif
