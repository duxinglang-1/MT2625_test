...
bool wifi_read_bus_spi(uint32_t offset, uint32_t length, uint8_t *read_buffer)
{
    ...
    memcpy(read_buffer, &(g_transfer_rx_buffer[1]), length);
#endif
    return true;
}
...
bool wifi_write_bus_spi(uint32_t offset, uint32_t length, uint8_t *write_buffer)
{
    ...
    if(remain_length > 1023)
    {
        ...
        {
            g_spi_transaction_finish = false;
            memcpy(&(g_transfer_tx_buffer[0]), &(write_buffer[count*1024]), 1024);
        ...
        g_spi_transaction_finish = false;
        memcpy(&(g_transfer_tx_buffer[0]), &(write_buffer[count*1024]), remain_length);
        ...
        g_transfer_tx_buffer[0] = SPIS_WR_CMD;
        memcpy(&(g_transfer_tx_buffer[1]), write_buffer, length);
