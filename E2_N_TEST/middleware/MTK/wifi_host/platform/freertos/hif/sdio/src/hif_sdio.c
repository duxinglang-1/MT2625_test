...
bool wifi_bus_init(void)
{
    ...
    wifi_write_bus(SDIO_IP_WHCR,4,&whcr);
...
//wppt: the first packet length(less than 0xffff). wpto: the second packet address
bool wifi_write_bus_ex(uint32_t offset, uint16_t length, uint8_t *write_buffer,uint32_t wppt, uint8_t *wpto)
{
        uint32_t wpto_addr = (uint32_t)wpto;
    hal_sdio_status_t status;
	uint8_t block_number = 0;
	uint32_t func1_block_size;
	hal_sdio_command53_config_t cmd_cfg;
    ...
    if (block_number > 0) {
   	    status = hal_sdio_execute_command53_dma_ex(sdio_port, &cmd_cfg,wppt,wpto_addr);
    } else {
   	    status = hal_sdio_execute_command53(sdio_port, &cmd_cfg);
	}
	if (HAL_SDIO_STATUS_OK != status) {
