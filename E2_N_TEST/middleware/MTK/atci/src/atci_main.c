...
void atci_uart_irq(hal_uart_callback_event_t event, void *parameter)
{
    ...
    if (g_atci_port_handle >= HAL_UART_MAX){
        ...
        LOGW("atci_uart_irq, invaild uart port:%ld\r\n", g_atci_port_handle);
        return;
    }
...
atci_status_t atci_send_data_int(uint8_t* data, uint32_t data_len)
{
    ...
    LOGI("atci_send_data_int() send data len:%ld\r\n", sent_len);
    return ret;
}
...
atci_status_t atci_send_data(uint8_t* data, uint32_t data_len)
{
    ...
    LOGI("atci_send_data() enter function, cache_left_len:%ld \r\n", g_send_data_cache_left_len);
    if(atci_vaild_port_check(g_atci_port_handle) == ATCI_STATUS_ERROR)
    {
        LOGE("atci_send_data() send data fail,because g_atci_port_handle is invalid:%ld \r\n", g_atci_port_handle);
        return ATCI_STATUS_ERROR;
    }
    ...
    LOGI("atci_send_data() sending data(len:%ld), sent data(len:%ld)\r\n", send_len, sent_len);
    ...
    if(sent_len >= send_len) {
        ...
        if(g_send_data_cache_len == 0) {
            ...
            if(g_send_data_cache == NULL)
            {
                ...
                if(g_send_data_cache == NULL)
                {
                    LOGW("atci_send_data() alloc memory fail, drop the cached data(data_len:%ld)\r\n", g_send_data_cache_left_len);
                    g_send_data_cache_len = 0;
                    g_send_data_cache_left_len = 0;
                    return ATCI_STATUS_ERROR;
                }
        ...
        LOGI("atci_send_data() cached data(data_len:%ld)\r\n", g_send_data_cache_left_len);
    }
...
atci_status_t atci_read_data(void)
{
    ...
    LOGI("atci_read_data, enter,g_atci_port_handle:%ld\r\n", g_atci_port_handle);
    if(atci_vaild_port_check(g_atci_port_handle) == ATCI_STATUS_ERROR)
        return ATCI_STATUS_ERROR;
...
void atci_processing(void)
{
    ...
    while (1) {
         ...
         */
            ...
            LOGI("atci_processing() commad_in_processing(%ld), input_queue_msg_num(%d)\r\n", atci_input_commad_in_processing, common_queue_msg_num);
        ...
        while (cached_queue_msg_num > 0) {
            ...
            if (msg_queue_data.msg_id == MSG_ID_ATCI_LOCAL_WRITE_CMD_IND) {
                ...
                LOGI("atci_processing() g_send_data_cache_left_len(%ld)\r\n", g_send_data_cache_left_len);
                if (g_send_data_cache_left_len > 0)
