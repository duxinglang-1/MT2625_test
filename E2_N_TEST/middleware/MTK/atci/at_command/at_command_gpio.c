...
/* --- Utilities ---*/
bool atci_gpio_is_number(char* strtok_result) {
    bool ret = true;
    int i;
    for(i=0 ;strtok_result[i]!='\0' && strtok_result[i]!='\r';i++) {
        if( !isdigit(strtok_result[i]))
        {
            ret = false;
            break;
        } 
...
atci_status_t atci_cmd_hdlr_gpio(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        ...
        case ATCI_CMD_MODE_EXECUTION:        
            ...
            cmd[255] = '\0'; //this is required for strtok
            ...
            if (strncmp(cmd, "AT+EGPIO=GPIO_GET:", strlen("AT+EGPIO=GPIO_GET:")) == 0) {

                param1 = strtok(cmd, ":");
                
                param2 = strtok(NULL, "~");     
                if( (!param2) || ( !atci_gpio_is_number(param2) ) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <gpio_sid>~<gpio_eid>\r\n", param2);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
                ...
                param3 = strtok(NULL, "~");
                if( (!param3) || (!atci_gpio_is_number(param3)) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <gpio_sid>~<gpio_eid>\r\n", param2);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
            ...
            if( (!param2) || ( !atci_gpio_is_number(param2) ) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <int>,<int>\r\n", param2);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
            ...
            /*  set GPIO pupd r0 r1 state. */
                ...
                param4 = strtok(NULL, ",");
                if( (!param4) || (!atci_gpio_is_number(param4)) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <int>\r\n", param4);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                    atci_send_response(&resonse);
                    break;
                ...
                param5 = strtok(NULL, ",");
                if( (!param5) || (!atci_gpio_is_number(param5)) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <int>\r\n", param5);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
            ...
            /* Set serveral configurations to one GPIO. */
                ...
                param6 = strtok(NULL, ",");
                if( (!param6) || (!atci_gpio_is_number(param6)) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <int>\r\n", param6);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                ...
                param7 = strtok(NULL, ",");
                if( (!param7) || (!atci_gpio_is_number(param7)) ) {
                  snprintf((char *)resonse.response_buf, ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                             "GPIO get sate input error parameter, your input argument is %s but expect <int>\r\n", param7);
                    resonse.response_len = strlen((char *)resonse.response_buf);
                    resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
