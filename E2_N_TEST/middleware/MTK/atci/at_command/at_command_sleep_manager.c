...
atci_status_t atci_cmd_hdlr_sleep_manager(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    if (strstr(cmd, "AT+SM=0") != NULL) {
        ...
        __asm volatile("cpsid i");
        rtc_set_sys_oper_mode(RTC_SYS_DS_AT);
...
//        hal_sleep_manager_enter_sleep_mode(HAL_SLEEP_MODE_DEEP_SLEEP);
        ...
        if (strstr(cmd, "AT+SM=LOCK_FOREVER") != NULL) {
        ...
        if (strstr(cmd, "AT+SM=UNLOCK_FOREVER") != NULL) {
            nvdm_write_data_item("sleep_manager", "sys_default_lock", NVDM_DATA_ITEM_TYPE_RAW_DATA, (uint8_t *)&sys_default_lock, sizeof(sys_default_lock));
        }
        ...
        response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
        atci_send_response(&response);
    } else if (strstr(cmd, "AT+SM=SHOWINFO") != NULL) {
        uint8_t sys_show_lock = false;
        uint32_t sys_show_freq = 0;
        char* token = NULL;

        if (strstr(cmd, "AT+SM=SHOWINFO,1") != NULL) {
            sys_show_lock = true;
            token = strchr(cmd, ',');
            token++;
            token = strchr(token, ',');
            token++;
            sscanf(token, "%u", (unsigned int*)&sys_show_freq);
        }
        nvdm_write_data_item("sleep_manager", "sys_show_lock", NVDM_DATA_ITEM_TYPE_RAW_DATA, &sys_show_lock, sizeof(sys_show_lock));
        nvdm_write_data_item("sleep_manager", "sys_show_freq", NVDM_DATA_ITEM_TYPE_RAW_DATA, (uint8_t *)&sys_show_freq, sizeof(sys_show_freq));

        response.response_len = strlen((char *) response.response_buf);
        response.response_flag |= ATCI_RESPONSE_FLAG_APPEND_OK;
        atci_send_response(&response);
#endif
    } else if (strstr(cmd, "AT+SM=POWEROFF") != NULL) {
        hal_rtc_gpio_control_t gpio_control;
        // Control RTC GPIO 0 output low by software.
        gpio_control.is_sw_control = true;
        gpio_control.is_sw_output_high = false;
