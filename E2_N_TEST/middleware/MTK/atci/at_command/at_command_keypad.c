...
ATTR_NONINIT_DATA_IN_RAM atci_response_t resonse = {{0}};
void atci_keypad_task_main(void *pvParameters)
{
...
atci_status_t atci_cmd_hdlr_keypad(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_TESTING:    // rec: AT+EKEYURC=?
            strcpy((char *)resonse.response_buf, "+EKEYURC=(0,1,2,3)\r\n");
        ...
        case ATCI_CMD_MODE_EXECUTION: // rec: AT+EKEYURC=<op>  the handler need to parse the parameters
            ...
            if (param == NULL || (param_len <= 1)) {
                LOGI("param is null after assign\r\n");
            ...
            if (param != NULL && ((param1_val >= 0) && (param1_val <= 3))) {
                /* valid parameter, update the data and return "OK"
                                param1_val = 0 :  Turn off powerkey+keypad URC information.
                                param1_val = 1 :  Turn on powerkey+keypad URC information.
                                param1_val = 2 :  Turn off keypad URC information.
                                param1_val = 3 :  Turn on keypad URC information.
                                param1_val = 4 :  Set to open all keypad pin, and select single key mode.
                                param1_val = 5 :  Set to open all keypad pin, and select double key mode.
                          */

                if (param1_val == 1) {
                    /*set to EPT default mode, and init keypad+powerkey*/
                    ...
                    /*set to EPT default mode, and init keypad only*/
                    if (keypad_custom_normal_init() == true) {
                        atci_keypad_task_init();
                        ret = hal_keypad_register_callback((hal_keypad_callback_t)atci_keypad_handler, NULL);
                        ret = hal_keypad_enable();
                        LOGI("keypad initilized OK...\r\n");
                    } else {
                        LOGI("keypad has been initilized...\r\n");
                        /* others are invalid command format */
                        strcpy((char *)resonse.response_buf, "\r\nKeypad init error\r\n");
                        resonse.response_flag = ATCI_RESPONSE_FLAG_URC_FORMAT;
                        resonse.response_len = strlen((char *)resonse.response_buf);
                    ...
                    /*deinit keypad and disable keypad module*/
                    if (keypad_custom_normal_deinit() == true) {
                        if (atci_keypad_queue_handle != NULL) {
                            atci_keypad_task_deinit();
                            ret = hal_keypad_disable();
                        }
                        LOGI("keypad deinitilized OK...\r\n");
                    } else {
                        LOGI("keypad deinitilized error...\r\n");
                    }
                } else if (param1_val == 4) {
                    /*set to single key and open all keypad GPTO pin*/
                    atci_keypad_set_key_mode(HAL_KEYPAD_MODE_SINGLE_KEY);
                    atci_keypad_gpio_init();
                    LOGI("keypad set to single key and all keypad GPIO pin is opened...\r\n");
                } else if (param1_val == 5) {
                    /*set to double key and open all keypad GPTO pin*/
                    atci_keypad_set_key_mode(HAL_KEYPAD_MODE_DOUBLE_KEY);
                    atci_keypad_gpio_init();
                    LOGI("keypad set to double key all keypad GPIO pin is opened...\r\n");
                } else if (param1_val == 0) {
                    /*deinit keypad+powerkey and disable keypad+powerkey module*/
