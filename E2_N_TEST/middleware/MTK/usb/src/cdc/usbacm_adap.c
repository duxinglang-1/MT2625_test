...
uint16_t USB2UART_GetBytes(uint8_t port, uint8_t *buffaddr, uint16_t length)
{
    ...
    if (USB2UART_Check_Config(usb_port) == false) {
        LOG_E(common, "USB2UART_GetBytes, enumeration not ready");
        return 0;
    }
...
uint16_t USB2UART_PutBytes(uint8_t port, uint8_t *buffaddr, uint16_t length)
{
    ...
    do{
        if(data_len > 64)
            hal_usb_write_endpoint_fifo(g_UsbACM[usb_port].txpipe->byEP, 64, buffaddr + i*64);
        else
            hal_usb_write_endpoint_fifo(g_UsbACM[usb_port].txpipe->byEP, data_len, buffaddr + i*64);
...
return length;
    ...
    if (setup_dma == true) {
        if (USB2UART_Check_Config(usb_port) == false) {
            ...
            if ((USB2UARTPort[usb_port].initialized == true) && (USB_Get_Device_State() != DEVSTATE_CONFIG)) {
                LOG_E(common, "USB2UART_PutBytes, enumeration not ready, stop dma transfer then return error\n");
                return 0;
            }
...
void USB2UART_PutData_Polling(uint8_t port, uint8_t *Buffaddr, uint16_t Length)
{
    ...
    hal_usb_wait_dma_idle(g_UsbACM[usb_port].txpipe->byEP);
...
/* This function is only used for retrive exception log*/
uint32_t USB2UART_GetData_Polling(uint8_t port, uint8_t *Buffaddr, uint16_t Length)
{
    uint16_t nCount;
    uint32_t usb_port = USB_PORT[port];
    uint8_t  ep_num = g_UsbACM[usb_port].rxpipe->byEP;

    if ((g_UsbACM[usb_port].rxpipe == NULL) || (USB2UARTPort[usb_port].initialized == false) || (USB_Get_Device_State() != DEVSTATE_CONFIG))
    {
        return 0;
    }
    
    //Read data count in FIFO.
    while((nCount = hal_usb_ep_pkt_len(ep_num)) == 0);

     //Read data from FIFO to data_buffer.
    hal_usb_read_endpoint_fifo(ep_num, nCount, Buffaddr);
