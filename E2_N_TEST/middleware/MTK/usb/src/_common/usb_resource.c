...
ATTR_NONINIT_DATA_IN_RAM static uint8_t USB_TEST_Buffer[200];
#else
ATTR_NONINIT_DATA_IN_RAM static uint8_t USB_TEST_Buffer[1024];
#endif
...
void USB_Software_Create(void)
{
    ...
    /* Get IP version */
    gUsbDevice.devdscr.bcdUSB = 0x0110; /* USB spec rev is 2.0 */
