...
#define BOOT_TIMEOUT_CNT 90000   /*jerry   充电头充电死机 */
...
void USB_Send_Message(usb_msg_type_t msg, void *data)
{
    ...
    if (ret != pdTRUE) {
        LOG_E(common, "[USB] Send Queue fail!! Queue size = %d ", USB_QUEUE_LENGTH);
    }
        ...
        if (usb_queue_handle != NULL) {
         ...
         case USB_MSC_RESET_IND_MSG:
                ...
                if (stack_cur > stack_max) {
                    stack_max = stack_cur;
                    LOG_I(common, "USB task max-usage:%d \r\n", stack_max);
                }
...
bool usb_task_init(void)
{
    ...
    if (usb_queue_handle == NULL) {
        LOG_E(common, "usb_queue_handle create fail!");
        return false;
    }
    ...
    if (xReturn == pdFALSE) {
        ...
        LOG_E(common, "usb_task_main task create fail!");
        return false;
    }
...
void usb_cable_detect(void)
{
    ...
    if (is_vusb_ready()) {
        ...
        init_check = ap_usb_init(USB_MASS_STORAGE);
#else //defined(MTK_USB_BOOT_ACM)
        init_check = ap_usb_init(USB_CDC_ACM);  
#endif
        if(init_check == false)
            return;

    //dbg_print("enumeration start",0,i);
    do {
	    vTaskDelay( xDelay );
	    if (i++ >= 30) {
		    break;
    ...
    //dbg_print("enumeration end",0,i);
        ...
        if (g_serial_port_usb_callback[0] != NULL) {
            ...
            LOG_I(common, "USB1 CONNECTION callback\n");
        }
        if (g_serial_port_usb_callback[1] != NULL) {
            ...
            LOG_I(common, "USB2 CONNECTION callback\n");
        }
#endif
        ...
        if (g_msc_usb_callback != NULL) {
            ...
            LOG_I(common, "msc CONNECTION callback\n");
        }
#endif
        ...
        if (g_serial_port_usb_callback[1] != NULL) {
            ...
            if (g_msc_usb_callback != NULL) {
                ...
                LOG_I(common, "msc DISCONNECTION callback\n");
            }
#endif
            ap_usb_deinit();
            LOG_I(common, "USB out\n");
#ifdef USB_DETECT_LED
...
void usb_boot_init(void)
{
    ...
    if (usb_sleep_handler == 0) {
        usb_sleep_handler = hal_sleep_manager_set_sleep_handle("USBDemo");
		LOG_I(common, "usb_sleep_handler=%d\n", usb_sleep_handler);
    }

    if (is_vusb_ready()) {
		LOG_I(common, "Boot-USB_in\n");

#if defined(MTK_USB_BOOT_MSC)
        ap_usb_init(USB_MASS_STORAGE);
#else //defined(MTK_USB_BOOT_MSC)
        ap_usb_init(USB_CDC_ACM);
        LOG_I(common, "ap_usb_init(USB_CDC_ACM)\n");
#endif
        ...
        LOG_I(common, "USBCOM init, hisr cnt%d\n\r", i);
    }
...
void usb_reset(void)
{
	...
	//LOG_I(common, "usb_reset, hisr cnt%d\n\r", i);
...
void ap_usb_register_msc_callback(msc_register_callback_t callback)
{
	...
	LOG_I(common, "register MSC connection callback\n");

    if (g_msc_usb_callback != NULL) {
        if (is_vusb_ready()) {
            g_msc_usb_callback(MSC_EVENT_USB_CONNECTION);
            LOG_I(common, "MSC_CONNECTION callback\n");
        } else {
            g_msc_usb_callback(MSC_EVENT_USB_DISCONNECTION);
            LOG_I(common, "MSC_DISCONNECTION callback\n");
        }
    }
}
#endif
