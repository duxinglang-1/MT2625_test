...
ATTR_NONINIT_DATA_IN_NONCACHED_RAM_4BYTE_ALIGN unsigned char USB_BUFFER[USB_MAX_MEMORY_SIZE]; // USB total buffer
...
void USB_Init(USB_DEVICE_TYPE type)
{
    ...
    switch (type) {
        case USB_CDC_ACM:
			//LOG_I(common, "USB_Init_Acm_Status\n");
            USB_Init_Acm_Status();
            break;
    ...
    if (pcmd->bmRequestType != USB_CMD_STDDEVIN) {
        ...
        switch (pcmd->wValue & USB_CMD_DESCMASK) {
            ...
            case USB_CMD_DEVICE_QUALIFIER:
                //LOG_I(common, "----USB_CMD_DEVICE_QUALIFIER\n");
...
static void USB_Stdcmd(Usb_Ep0_Status *pep0state, Usb_Command *pcmd)
{
    ...
    switch (pcmd->bRequest) {
        ...
        default:
            bError = true;			
            //LOG_I(common, "bRequest out of case:%d\n", pcmd->bRequest);
            break;
    }
...
static void USB_Endpoint0_Idle (void)
{
    ...
    //LOG_I(common, "USB_Endpoint0_Idle\n");
    //LOG_I(common, "    -bmRequestType:%x\n", gUsbDevice.cmd.bmRequestType);
    //LOG_I(common, "    -bRequest     :%x\n", gUsbDevice.cmd.bRequest);
    //LOG_I(common, "    -wValue       :%x\n", gUsbDevice.cmd.wValue);
    //LOG_I(common, "    -wIndex       :%x\n", gUsbDevice.cmd.wIndex);
    //LOG_I(common, "    -wLength      :%x\n", gUsbDevice.cmd.wLength);
...
static void USB_Endpoint0_Hdlr(void)
{
    ...
    //LOG_I(common, "USB_Endpoint0_Hdlr\n");
