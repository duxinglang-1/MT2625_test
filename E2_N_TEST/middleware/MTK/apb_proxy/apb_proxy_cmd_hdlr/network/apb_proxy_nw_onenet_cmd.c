...
#else
#define ONENET_AT_LOGI(fmt, args...)
#define ONENET_AT_LOGW(fmt, args...)
#define ONENET_AT_LOGE(fmt, args...)
#endif
...
} onenet_at_nvdm_data_t;

static ATTR_ZIDATA_IN_RETSRAM onenet_at_context_t g_onenet_at_context[ONENET_AT_INSTANCE_NUM];
static onenet_at_nvdm_data_t g_onenet_at_nvdm_data[ONENET_AT_INSTANCE_NUM];
...
static void *g_onenet_at_mutex_handle;
...
static onenet_at_context_t *onenet_at_create_instance(void)
{
    ...
    for (i = 0; i < ONENET_AT_INSTANCE_NUM; i++) {
        if (g_onenet_at_context[i].is_used == false) {
            ...
            g_onenet_at_context[i].config_bin = g_onenet_at_nvdm_data[i].config_bin;
            g_onenet_at_context[i].object_info = g_onenet_at_nvdm_data[i].object_info;
...
/* Group Name */
#define ONENET_AT_NVDM_GROUP_NAME_CONFIG     "onenet_cfg%d"
#define ONENET_AT_NVDM_GROUP_NAME_OBJECT     "onenet_obj%d:%d"

/* Item Name */
#define ONENET_AT_NVDM_ITEM_NAME_CONFIG      "onenet_config"
#define ONENET_AT_NVDM_ITEM_NAME_OBJECT      "onenet_object"

static void onenet_at_read_config(uint32_t onenet_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (onenet_id >= ONENET_AT_INSTANCE_NUM) return;

    sprintf(group_name, ONENET_AT_NVDM_GROUP_NAME_CONFIG, (int)onenet_id);
    len = ONENET_AT_CONFIG_MAX_BUFFER_LEN;
    status = nvdm_read_data_item(group_name, ONENET_AT_NVDM_ITEM_NAME_CONFIG, (uint8_t*)&g_onenet_at_nvdm_data[onenet_id].config_bin, &len);
    ONENET_AT_LOGI("read nvdm [%s: %d] = %d", ONENET_AT_NVDM_ITEM_NAME_CONFIG, g_onenet_at_nvdm_data[onenet_id].config_bin, status);
}

static void onenet_at_write_config(uint32_t onenet_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    sprintf(group_name, ONENET_AT_NVDM_GROUP_NAME_CONFIG, (int)onenet_id);
    len = ONENET_AT_CONFIG_MAX_BUFFER_LEN;
    status = nvdm_write_data_item(group_name, ONENET_AT_NVDM_ITEM_NAME_CONFIG, NVDM_DATA_ITEM_TYPE_RAW_DATA, (uint8_t*)&g_onenet_at_nvdm_data[onenet_id].config_bin, len);
    ONENET_AT_LOGI("write nvdm [%s: %d] = %d", ONENET_AT_NVDM_ITEM_NAME_CONFIG, g_onenet_at_nvdm_data[onenet_id].config_bin, status);
}

static void onenet_at_retention_load_config(onenet_at_context_t *onenet)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    onenet_at_read_config(onenet->onenet_id);
    onenet->config_bin = g_onenet_at_nvdm_data[onenet->onenet_id].config_bin;
}

static void onenet_at_retention_save_config(onenet_at_context_t *onenet)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    onenet_at_write_config(onenet->onenet_id);
}

static void onenet_at_read_object(uint32_t onenet_id, uint32_t index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    if (onenet_id >= ONENET_AT_INSTANCE_NUM) return;
    if (index >= ONENET_AT_OBJECT_MAX_COUNT) return;

    sprintf(group_name, ONENET_AT_NVDM_GROUP_NAME_OBJECT, (int)onenet_id, (int)index);
    len = sizeof(onenet_at_object_info_t);
    status = nvdm_read_data_item(group_name, ONENET_AT_NVDM_ITEM_NAME_OBJECT, (uint8_t*)&g_onenet_at_nvdm_data[onenet_id].object_info[index], &len);
    ONENET_AT_LOGI("read nvdm [%s: %d] = %d", ONENET_AT_NVDM_ITEM_NAME_OBJECT, index, status);
}

static void onenet_at_write_object(uint32_t onenet_id, uint32_t index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    sprintf(group_name, ONENET_AT_NVDM_GROUP_NAME_OBJECT, (int)onenet_id, (int)index);
    len = sizeof(onenet_at_object_info_t);
    status = nvdm_write_data_item(group_name, ONENET_AT_NVDM_ITEM_NAME_OBJECT, NVDM_DATA_ITEM_TYPE_RAW_DATA, (uint8_t*)&g_onenet_at_nvdm_data[onenet_id].object_info[index], len);
    ONENET_AT_LOGI("write nvdm [%s: %d] = %d", ONENET_AT_NVDM_ITEM_NAME_OBJECT, index, status);
}

static void onenet_at_retention_load_object(onenet_at_context_t *onenet)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    for (i = 0; i < ONENET_AT_OBJECT_MAX_COUNT; i++) {
        onenet_at_read_object(onenet->onenet_id, i);
    }
    onenet->object_info = g_onenet_at_nvdm_data[onenet->onenet_id].object_info;
}
...
static void onenet_at_retention_save_object(onenet_at_context_t *onenet, cis_oid_t objectid, const cis_inst_bitmap_t *bitmap, const cis_res_count_t *resource)
{
    ...
    for (i = 0; i < ONENET_AT_OBJECT_MAX_COUNT; i++) {
        ...
        if (object_info->is_used == 0) {
            ...
            onenet_at_write_object(onenet->onenet_id, i);
            return;
        }
    }
}
        ...
        if (onenet == NULL) {
            ...
            if (currentsize / 2 > ONENET_AT_CONFIG_MAX_BUFFER_LEN) {
                ...
                if (config != NULL) {
                    ...
                    onenet_at_retention_save_config(onenet);
                } else {
                ...
                if (onenet->use_default_config) {
                    cis_ret = cis_init(&onenet->cis_context, NULL, 0, NULL, NULL);
                } else {
...
static int32_t onenet_at_delete(uint32_t onenet_id)
{
    ...
    if (onenet == NULL) {
        ...
        xSemaphoreTake(g_onenet_at_mutex_handle, portMAX_DELAY);
        ...
        xSemaphoreGive(g_onenet_at_mutex_handle);
    }
        ...
        for (i = 0; i < attrcount; i++) {
            switch (value[i].type) {
                case cis_data_type_string:
                    ...
                    string = (char *)pvPortMalloc(length + 1);
                    ...
                    if (length <= 1024) {
                        strncpy(string, (char *)value[i].asBuffer.buffer, length);
                        string[length] = '\0';
                        ...
                        /* APB can not send long data more than 1400 bytes, so need to split into several AT commands */
                        uint32_t count = (length + 1023) / 1024;
                        uint32_t j, split_length;
                        for (j = 0; j < count - 1; j++) {
                            if (j == 0) {
                                flag = ONENET_AT_FLAG_FIRST;
                            } else {
                                flag = ONENET_AT_FLAG_MIDDLE;
                            }
                            split_length = 1024;
                            strncpy(string, (char *)value[i].asBuffer.buffer + j * 1024, split_length);
                            string[length] = '\0';
                            sprintf(response_data, "+MIPLWRITE: %d, %d, %d, %d, %d, %d, %d, %s, %d, %d",
                                    (int)onenet->onenet_id, (int)mid, (int)uri->objectId, (int)uri->instanceId, (int)value[i].id,
                                    (int)value[i].type, (int)split_length, string,
                                    (int)flag, (int)(attrcount - 1 - i));
                            response.pdata = response_data;
                            response.length = strlen(response.pdata);
                        ...
                        flag = ONENET_AT_FLAG_LAST;
                        split_length = length % 1024;
                        strncpy(string, (char *)value[i].asBuffer.buffer + j * 1024, split_length);
                        string[length] = '\0';
                        sprintf(response_data, "+MIPLWRITE: %d, %d, %d, %d, %d, %d, %d, %s, %d, %d",
                                (int)onenet->onenet_id, (int)mid, (int)uri->objectId, (int)uri->instanceId, (int)value[i].id,
                ...
                case cis_data_type_opaque:
                    ...
                    if (length <= 512) {
                        ...
                        for (j = 0; j < count - 1; j++) {
                            ...
                            split_length = 512;
                            onenet_at_bin_to_hex(string, value[i].asBuffer.buffer + j * 512, split_length * 2);
                            sprintf(response_data, "+MIPLWRITE: %d, %d, %d, %d, %d, %d, %d, %s, %d, %d",
                                    (int)onenet->onenet_id, (int)mid, (int)uri->objectId, (int)uri->instanceId, (int)value[i].id,
                                    (int)value[i].type, (int)split_length, string,
                                    (int)flag, (int)(attrcount - 1 - i));
...
static void onenet_at_event_callback(void *context, cis_evt_t id, void* param)
{
    ...
    if (onenet == NULL) {
        ...
        switch (id)
        {
            ...
            case CIS_EVENT_NOTIFY_SUCCESS_FROM_BS:
...
static void onenet_at_task_processing(void *arg)
{
    ...
    if (g_onenet_at_mutex_handle == NULL) {
        g_onenet_at_mutex_handle = xSemaphoreCreateMutex();
    }
    ...
    vSemaphoreDelete(g_onenet_at_mutex_handle);
    g_onenet_at_mutex_handle = NULL;
...
create_exit:
    ...
    if (cis_error == ONENET_AT_ERRID_OK) {
        sprintf(response_data, "+MIPLCREATE: %d", (int)ref);
        response.pdata = response_data;
        response.length = strlen(response.pdata);
            ...
            if (param_buffer == NULL) {
                ...
                if (!cmd_string || *(cmd_string + 1) == '\0') {
                    ...
                    if (param_num < 10) {
                        ...
                        char *new_value = (char *)pvPortMalloc(strlen(value) + 1);
                        onenet_at_remove_backslash(value, new_value);
                        if (param_num > 10) {
                            ackid = atoi(param_list[10]);
                        }
                        if (index > 0) {
                            cis_error = onenet_at_notify(ref, msgid, objectid, instanceid, resourceid, valuetype, len, new_value, ackid, CIS_NOTIFY_CONTINUE);
                        } else if (index == 0) {
                            cis_error = onenet_at_notify(ref, msgid, objectid, instanceid, resourceid, valuetype, len, new_value, ackid, CIS_NOTIFY_CONTENT);
                        } else {
                            ONENET_AT_LOGE("parameter error");
                            cis_error = ONENET_AT_ERRID_PARAMETER_ERROR;
                        }
                ...
                if (!cmd_string || *(cmd_string + 1) == '\0') {
                    ...
                    if (param_num < 4) {
                        ...
                        if (length == 0 || length != strlen(valuestring)) {
                            ...
                            field_buffer = (char *)pvPortMalloc(strlen(valuestring) + 1);
                            if (field_buffer == NULL) {
                                ONENET_AT_LOGE("memory error");
                                cis_error = ONENET_AT_ERRID_MEMORY_ERROR;
                            } else {
                                uint32_t filed_num = onenet_at_parse_param(valuestring, field_buffer, NULL, field_list, ONENET_AT_CMD_FILED_NUM + 1, ";");
...
apb_proxy_status_t apb_proxy_hdlr_onenet_version_cmd(apb_proxy_parse_cmd_param_t *parse_cmd)
{
    ...
    if (cis_error == ONENET_AT_ERRID_OK) {
        sprintf(response_data, "+MIPLVER: %d.%d.%d", (int)version.major, (int)version.minor, (int)version.micro);
        response.pdata = response_data;
        response.length = strlen(response.pdata);
    } else if (response.result_code == APB_PROXY_RESULT_CUSTOM_ERROR) {
        sprintf(response_data, "+CIS ERROR: %d", (int)cis_error);
...
void onenet_at_remove_backslash(const char *value, char *new_value)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    char *str1 = (char *)value, *str2 = new_value;
...
uint32_t onenet_at_parse_cmd(const char *cmd_string, const char *param_buffer, char *param_list[], uint32_t param_max_num)
{
    ...
    char *p = (char *)cmd_string, *p2, *p3, *p4;
