...
void nvdm_modem_peb_io_read(int32_t pnum, int32_t offset, uint8_t *buf, int32_t len)
{
    ...
    addr = nvdm_modem_port_get_peb_address(pnum, offset);

    nvdm_modem_port_flash_read(addr, buf, len);
}
...
void nvdm_modem_peb_io_write(int32_t pnum, int32_t offset, const uint8_t *buf, int32_t len)
{
    ...
    if (len % NVDM_BUFFER_SIZE) {
        memset(io_buffer, 0, NVDM_BUFFER_SIZE);
        nvdm_modem_port_flash_read(addr, io_buffer, len % NVDM_BUFFER_SIZE);
        ret = compare_buffer(io_buffer, &buf[NVDM_BUFFER_SIZE * fragment], len % NVDM_BUFFER_SIZE);
        if (ret) {
            nvdm_modem_port_log_error("addr=0x%x, pnum=%d, offset=0x%x, len=%d", addr, pnum, offset, len);
        }
    }
}
...
void nvdm_modem_peb_io_erase(int32_t pnum)
{
    ...
    addr = nvdm_modem_port_get_peb_address(pnum, 0);

    nvdm_modem_port_flash_erase(addr);

    io_buffer = g_io_buffer;

    /* peb size must be aligned to NVDM_BUFFER_SIZE boundary */
    for (i = 0; i < g_nvdm_modem_peb_size / NVDM_BUFFER_SIZE; i++) {
        nvdm_modem_port_flash_read(addr, io_buffer, NVDM_BUFFER_SIZE);
        ret = check_pattern(io_buffer, 0xFF, NVDM_BUFFER_SIZE);
        if (ret) {
            nvdm_modem_port_log_error("[io_erase]check pattern failed, pnum=%d", pnum);
            return;
        }
        addr += NVDM_BUFFER_SIZE;
    }
