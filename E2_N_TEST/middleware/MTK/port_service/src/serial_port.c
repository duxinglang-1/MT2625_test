...
serial_port_operation_t g_serial_port_handlers[SERIAL_PORT_TYPE_MAX] = {
    ...
    {
        serial_port_fake_init,
        serial_port_fake_control,
        serial_port_fake_deinit,
        NULL,
    },
};
...
serial_port_status_t serial_port_set_device_status(serial_port_dev_t device, bool busy)
{
    ...
    g_serial_port_device_is_busy[device] = busy;
...
serial_port_type_t serial_port_get_device_type(serial_port_dev_t device)
{
    ...
    } else if (device == SERIAL_PORT_DEV_FAKE) {
        device_type = SERIAL_PORT_TYPE_FAKE;
    } else {
        device_type = SERIAL_PORT_TYPE_UART;
    }
                                      ...
                                      serial_port_handle_t *handle)
{
    ...
    *handle = serial_port_return_handle(true, device);
    return status;

error:
    irq_status = save_and_set_interrupt_mask();
    *handle = serial_port_return_handle(false, SERIAL_PORT_DEV_UNDEFINED);
