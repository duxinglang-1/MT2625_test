...
void serial_port_uart_reset_para(serial_port_setting_t *setting)
{
    ...
    for (device = SERIAL_PORT_DEV_UART_TYPE_BEGIN; device <= SERIAL_PORT_DEV_UART_TYPE_END; device++) {
        ...
        if((device == DEFAULT_PORT_FOR_AT_CMD) || (device == DEFAULT_PORT_FOR_AT_CMD_2)) {
            port_setting_uart->is_baud_rate_set = 0;}
        else {
            port_setting_uart->is_baud_rate_set = 1;}
#endif
    }
}
...
static void serial_port_config_check_default_setting(serial_port_setting_t *serial_port_setting)
{
    ...
    if (status == NVDM_STATUS_ITEM_NOT_FOUND) {
        ...
        nvdm_write_data_item(PORT_SETTING_GROUP_NAME, PORT_SETTING_NAME_PORT_AUTO_BAUD_RATE, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)&isDone, sizeof(bool));
    } else if (status != NVDM_STATUS_OK) {
        SERIAL_PORT_ASSERT();
    }
}
...
serial_port_status_t serial_port_config_read_auto_baud_rate_done(serial_port_dev_t device, bool *isDone)
{
    uint32_t setting_size, setting;

    setting_size = sizeof(setting);

    nvdm_read_data_item(PORT_SETTING_GROUP_NAME, PORT_SETTING_NAME_PORT_AUTO_BAUD_RATE, &setting, &setting_size);
...
serial_port_status_t serial_port_config_write_auto_baud_rate_done(serial_port_dev_t device, bool isDone)
{
    nvdm_write_data_item(PORT_SETTING_GROUP_NAME, PORT_SETTING_NAME_PORT_AUTO_BAUD_RATE, NVDM_DATA_ITEM_TYPE_RAW_DATA, (const uint8_t *)&isDone, sizeof(bool));
