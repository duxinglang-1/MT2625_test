...
typedef struct {
...
#endif
#ifdef MTK_SENSOR_BIO_USE_MT6381
    ...
    sensor_descriptor_t sensio_descriptor;
    ...
    sensor_data_unit_t sensio_data;
...
uint32_t xSMLastExecutionTime, xSMLastExecutionTimeInUS;
...
#if defined(FUSION_SENSIO_USE)
    supported_algo |= (1U << SENSOR_TYPE_SENSIO);
#endif
...
static void sensor_fusion_task( void *pvParameters )
{
    ...
    while (1) {
        if( xQueueReceive(data_queue_handle, &sensor_data, xTicksToWait)) {
            ...
            if(sensor_data.data->sensor_type == SENSOR_TYPE_BIOSENSOR_PPG2){
                data_updated |= (1U<<SENSOR_TYPE_BIOSENSOR_PPG2);
            }
...
#if defined(FUSION_SENSIO_USE)
            if (is_sensor_enabled(SENSOR_TYPE_SENSIO)) {
                if (data_updated & (1U << SENSOR_TYPE_BIOSENSOR_PPG1)) {
                   smt.sensio_descriptor.process_data(&sensor_data, NULL);
                }
                if (data_updated & (1U << SENSOR_TYPE_BIOSENSOR_PPG2)) {
                   smt.sensio_descriptor.process_data(&sensor_data, NULL);
                }
                if (data_updated & (1U << SENSOR_TYPE_BIOSENSOR_EKG)) {
                   smt.sensio_descriptor.process_data(&sensor_data, NULL);
...
static void sensor_mgr_task( void *pvParameters )
{
    ...
    while (1) {
        if ( xQueueReceive(sm_queue_handle, &event_item, xTicksToWait)) {
            xSMLastExecutionTimeInUS = sensor_driver_get_us_tick();
            xSMLastExecutionTime = xSMLastExecutionTimeInUS/1000;
            ...
            #endif
#ifdef MTK_SENSOR_BIO_USE_MT6381
#ifndef MT6381_USE_IRQ
                    if (sensor_driver_get_enable(SENSOR_TYPE_BIOSENSOR_PPG1) == 1) {
                        ppg1_actual_out_size = 0;
                        sensor_type = SENSOR_TYPE_BIOSENSOR_PPG1;
                        sensor_driver_get_data(sensor_type, &(smt.ppg1_data), \
                                               BIO_MAX_BUF_CNT, &ppg1_actual_out_size);
                        if (ppg1_actual_out_size > 0) {
            ...
            if (smt.data_updated & (1U << SENSOR_TYPE_BIOSENSOR_PPG2)) {
                this_bio_ppg2_data.data_exist_count = ppg2_actual_out_size;
                this_bio_ppg2_data.data = smt.ppg1_data;
                (void)this_bio_ppg2_data;
            }
        ...
        if (is_sensor_enabled(SENSOR_TYPE_HEART_RATE_VARIABILITY_MONITOR)) {
            ...
            if (smt.data_updated & (1U << SENSOR_TYPE_BIOSENSOR_PPG1)) {
                if (ppg1_actual_out_size > 0) {
                    physical_sensor_data.data = &smt.ppg1_data[i];
                    physical_sensor_data.data_exist_count = ppg1_actual_out_size;
                    send_data_to_subscriber(physical_sensor_data, SENSOR_TYPE_BIOSENSOR_PPG1);
                }
            }
            if (smt.data_updated & (1U << SENSOR_TYPE_BIOSENSOR_PPG2)) {
                if (ppg2_actual_out_size > 0) {
                    physical_sensor_data.data = &smt.ppg2_data[i];
                    physical_sensor_data.data_exist_count = ppg2_actual_out_size;
                    send_data_to_subscriber(physical_sensor_data, SENSOR_TYPE_BIOSENSOR_PPG2);
                }
            }
            if (smt.data_updated & (1U << SENSOR_TYPE_BIOSENSOR_EKG)) {
                if (ekg_actual_out_size > 0) {
                    physical_sensor_data.data = &smt.ekg_data[i];
                    physical_sensor_data.data_exist_count = ekg_actual_out_size;
                    send_data_to_subscriber(physical_sensor_data, SENSOR_TYPE_BIOSENSOR_EKG);
                }
            }
#endif
...
int32_t sensor_fusion_algorithm_register_type(const sensor_descriptor_t *desp)
{
    ...
    } else if (SENSOR_TYPE_SENSIO == desp->sensor_type) {
        memcpy(&(smt.sensio_descriptor), desp, sizeof(sensor_descriptor_t));
    } else {
        return -1;
    }
    return 1;
...
int32_t sensor_fusion_algorithm_notify(uint32_t sensor_type)
{
    ...
    } else if (SENSOR_TYPE_SENSIO == sensor_type) {
        if (smt.sensio_descriptor.get_result != NULL) {
            algorithm_result.data = &(smt.sensio_data);
            smt.sensio_descriptor.get_result(&algorithm_result);
        }
    } else {
        ret = -1;
        return ret;
...
{
    send_data_to_subscriber(data, sensor_type);
...
sensor_input_list_t *get_algorithm_input_list(uint32_t sensor_type)
{
    ...
    switch (sensor_type) {
        ...
        case SENSOR_TYPE_ELECTRIC_HEART_RATE_MONITOR:
            ...
            break;
        case SENSOR_TYPE_SENSIO:
            tmp_input_list = smt.sensio_descriptor.input_list;
            break;
        default:
            tmp_input_list = NULL;
