...
bl_fota_status_t bl_fota_compress_update(uint32_t bin_number)
{
    ...
    BL_PRINT(LOG_DEBUG,"fota_head_buf.m_bin_info[%d].m_bin_type  = %x \n\r", bin_number, fota_head_buf.m_bin_info[bin_number].m_bin_type);
...
bl_fota_status_t bl_fota_write_update_info(void *buffer, int32_t buffer_size)
{
    ...
    if (BL_SEC_ENABLED == sboot_is_secure_boot_enable()){
        marker_addr -= BL_FOTA_SEC_HEADER_LEN;
    }
#endif
...
bl_fota_status_t bl_fota_do_update(uint32_t bin_number)
{
    hal_flash_status_t status = HAL_FLASH_STATUS_OK;
    uint32_t addr = fota_head_buf.m_bin_info[bin_number].m_bin_start_addr;
    uint32_t length = fota_head_buf.m_bin_info[bin_number].m_bin_length;
    uint32_t bin_offset = fota_head_buf.m_bin_info[bin_number].m_bin_offset;
    uint32_t p = addr;
    ...
    BL_PRINT(LOG_DEBUG, "Start Address=%x,length=%x\r\n", addr, length);
...
bl_fota_status_t bl_fota_integrity_check(uint32_t bin_number)
{
    uint8_t signature_in_fota_pkg[BL_FOTA_SIGNATURE_SIZE];
    hal_flash_status_t hal_status   = HAL_FLASH_STATUS_OK;
    uint32_t sig_len  = fota_head_buf.m_bin_info[bin_number].m_sig_length;
    uint32_t sig_addr = ( bl_custom_fota_start_address() + fota_head_buf.m_bin_info[bin_number].m_sig_offset ) - bl_custom_rom_baseaddr();
    uint32_t sig_size = sig_len;
    uint32_t bin_counter = fota_head_buf.m_bin_info[bin_number].m_bin_length;
    uint32_t bin_offset = fota_head_buf.m_bin_info[bin_number].m_bin_offset;
    uint32_t fota_bin_raw_seek;
...
bl_fota_status_t bl_fota_load_and_check_fota_header()
{
...
#if defined(MTK_SECURE_BOOT_ENABLE) && defined(__GNUC__)
    if (BL_SEC_ENABLED == sboot_is_secure_boot_enable()){
        BL_PRINT(LOG_DEBUG, "Security boot is enabled.\r\n");
        if (sboot_secure_boot_check(bl_custom_fota_start_address() - BL_FOTA_SEC_HEADER_LEN) == 0){
            BL_PRINT(LOG_DEBUG, "Security check passed.\r\n");
        }else{
            BL_PRINT(LOG_DEBUG, "Security check failed.\r\n");
            bl_fota_write_update_status(BL_FOTA_ERROR_SEC_CHECK_ERROR);
            return BL_FOTA_ERROR_SEC_CHECK_ERROR;
        }
    }else{
        BL_PRINT(LOG_DEBUG, "Security boot is not enabled.\r\n");
    }
#endif
    /* load header */
    ...
    BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_num = %x \n\r", fota_head_buf.m_bin_num);
    for(index = 0; index < fota_head_buf.m_bin_num; index ++){
        BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_info[%d].m_bin_length = %x \n\r", index, fota_head_buf.m_bin_info[index].m_bin_length);
        BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_info[%d].m_bin_offset = %x \n\r", index , fota_head_buf.m_bin_info[index].m_bin_offset);
        BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_info[%d].m_bin_start_addr = %x \n\r", index, fota_head_buf.m_bin_info[index].m_bin_start_addr);
        BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_info[%d].m_sig_length = %x \n\r", index, fota_head_buf.m_bin_info[index].m_sig_length);
        BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_info[%d].m_sig_offset = %x \n\r", index, fota_head_buf.m_bin_info[index].m_sig_offset);
        BL_PRINT(LOG_DEBUG, "fota_head_buf.m_bin_info[%d].m_is_compressed = %x \n\r", index, fota_head_buf.m_bin_info[index].m_is_compressed);
        BL_PRINT(LOG_DEBUG, "g_fota_header.m_bin_info[%d].m_version_info[0]  = %d \n\r", index, fota_head_buf.m_bin_info[index].m_version_info[0]);
        BL_PRINT(LOG_DEBUG, "g_fota_header.m_bin_info[%d].m_version_info[1]  = %d \n\r", index, fota_head_buf.m_bin_info[index].m_version_info[1]);
        BL_PRINT(LOG_DEBUG, "g_fota_header.m_bin_info[%d].m_bin_type  = %d \n\r", index, fota_head_buf.m_bin_info[index].m_bin_type);
    }
...
bl_fota_status_t bl_fota_update_main(void)
{
    ...
    if (status == BL_FOTA_ERROR_NONE) {
        ...
        for(index = 0; index < fota_head_buf.m_bin_num; index ++){
            status = bl_fota_integrity_check(index);
            /* error handling */
            if (status != BL_FOTA_ERROR_NONE) {
    ...
    if (status == BL_FOTA_ERROR_NONE) {
        ...
        for(index = 0; index < fota_head_buf.m_bin_num; index ++){
            if (fota_head_buf.m_bin_info[index].m_bin_type == BL_FOTA_GNSS_BIN_TYPE_VALUE){
                BL_PRINT(LOG_DEBUG, "skip GNSS bin type, bin_num=%d\n\r", index);
                continue;
            }
            if (fota_head_buf.m_bin_info[index].m_is_compressed == 1) {
                status = bl_fota_compress_update(index);
            } else {
                status = bl_fota_do_update(index);
            }
            if (status != BL_FOTA_ERROR_NONE) {
                BL_PRINT(LOG_DEBUG, "compressed value = %x, bin_num=%d\n\r",fota_head_buf.m_bin_info[index].m_is_compressed, index);
                break;
            }
        }
    }
...
bl_fota_status_t bl_rock_fota_update_main(void)
{
    bl_fota_status_t status = BL_FOTA_ERROR_NONE;
    int first_run = 1;
    /* set updating marker */
    if (status == BL_FOTA_ERROR_NONE) {
        if(bl_fota_is_updating() == FALSE){
            ...
            first_run = 0;
            BL_PRINT(LOG_INFO, "updating marker exist");
        }
    }

    /* run fota update */
    if (status == BL_FOTA_ERROR_NONE) {
        status = rock_main(first_run);
        if (status != BL_FOTA_ERROR_NONE) {
            BL_PRINT(LOG_INFO, "rock update error, status = %d\n\r", status);
        }else{
            BL_PRINT(LOG_INFO, "rock update successfully\n\r");
        }
...
#ifdef ROCK_FOTA_SUPPORT
        status = bl_rock_fota_update_main();
#else
