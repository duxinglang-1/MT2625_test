...
void bl_hardware_init()
{
    ...
    bl_print(LOG_DEBUG, "hal_rtc_init\n\r");
    hal_rtc_init();
    ...
    bl_print(LOG_DEBUG, "hal_emi_configure_advanced \n\r");
    ...
    bl_print(LOG_DEBUG, "hal_wdt_enable for 30s\n\r");
...
void bl_start_user_code()
{
    ...
    if(sboot_is_secure_boot_enable()) {
        if (sboot_secure_boot_check(hdrAddr) == -1) {
