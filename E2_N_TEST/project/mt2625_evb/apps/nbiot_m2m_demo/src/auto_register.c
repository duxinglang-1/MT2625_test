...
static int32_t auto_register_query_iccid_callback(ril_cmd_response_t *rsp)
{
    ...
    if (rsp->res_code == RIL_RESULT_CODE_OK && param != NULL) 
	{
        ...
        //89861118246009163822
        //read ICCID from NVDM
        
		memset(total_buffer_str,0,sizeof(total_buffer_str));
		strncat(total_buffer_str,g_auto_reg_cntx.iccid+9,10);	
        Iccid_Get_sim(total_buffer_str);
		memset(temp_info.iccid,0,sizeof(temp_info.iccid));
		strcat(temp_info.iccid,g_auto_reg_cntx.iccid);	
        AUTO_REG_LOGI("<<<<<<<<<<<<<<<<<<<<< temp_info.iccid >>>>>>>>>>>>>>>>>>:%s\r\n", temp_info.iccid);
...
static void auto_register_prompt_message(uint32_t result)
{
	...
	sbit_m2m_ct_send_massege(M2M_CT_SELF_REGISTER,&msg_result,4,0);
...
#ifndef __H10_TEMP__
static int32_t register_query_imei_callback(ril_cmd_response_t *rsp)
{
    ril_status_t ret = RIL_STATUS_SUCCESS;

    uint8_t imei_len = 0;
    ril_serial_number_rsp_t *param = (ril_serial_number_rsp_t*)rsp->cmd_param;
	AUTO_REG_LOGI("ZZZZ::register_query_imei_callback\r\n");		
    if (rsp->res_code == RIL_RESULT_CODE_OK && param!= NULL){
        
        imei_len = strlen(param->value.imei);

        AUTO_REG_LOGI("ZZZZ::[auto_reg]param->value.imei = %s,%d\r\n", param->value.imei, imei_len);        
        //memcpy(g_auto_reg_cntx.imei, param->value.imei, (imei_len > AUTO_REG_IMEI_LEN)? AUTO_REG_IMEI_LEN : imei_len);
		memset(temp_info.imei_num, 0, sizeof(temp_info.imei_num));
        memcpy(temp_info.imei_num, param->value.imei, (imei_len > AUTO_REG_IMEI_LEN)? AUTO_REG_IMEI_LEN : imei_len);
		if((strstr(temp_info.imei_num,"000000000000000"))!=NULL)
		{
			memset(temp_info.imei_num, 0, sizeof(temp_info.imei_num));
			//memcpy(temp_info.imei_num,"865352031620296",16);
			//memcpy(temp_info.imei_num,"111118681536649",16);
			memcpy(temp_info.imei_num,"000000000000000",16);
			//memcpy(temp_info.imei_num,"111100000010320",16);
			//memcpy(temp_info.imei_num,"111100000010664",16);
			//memcpy(temp_info.imei_num,"111100000010573",16);
			//memcpy(temp_info.imei_num,"111100000010482",16);
			//memcpy(temp_info.imei_num,"861797030390188",16);
...
static int32_t register_query_imsi_callback(ril_cmd_response_t *rsp)
{
    uint8_t imsi_len = 0;
    auto_reg_message_t *auto_reg_msg = NULL;
    ril_imsi_rsp_t *param = (ril_imsi_rsp_t*)rsp->cmd_param;
    ...
    if (rsp->res_code == RIL_RESULT_CODE_OK && param!= NULL) 
	{
        ...
        AUTO_REG_LOGI("[auto_reg]param->imsi = %s,%d\r\n", param->imsi, imsi_len);
        memcpy(g_auto_reg_cntx.imsi, param->imsi, (imsi_len > AUTO_REG_IMSI_LEN)? AUTO_REG_IMSI_LEN : imsi_len);
		memset(temp_info.imsi_num, 0, sizeof(temp_info.imsi_num));
		memcpy(temp_info.imsi_num, param->imsi, (imsi_len > AUTO_REG_IMSI_LEN)? AUTO_REG_IMSI_LEN : imsi_len);
    } 
	else 
    {
        AUTO_REG_LOGI("[auto_reg]Query imsi result fail!\r\n");
...
void ril_query_imei_number(void)
{
	ril_status_t ret = RIL_STATUS_SUCCESS;
	ret = ril_request_serial_number(RIL_EXECUTE_MODE, 1, register_query_imei_callback, NULL);
	AUTO_REG_LOGI("===ZZZZ::ril_query_imei_number\n");
	if (ret != RIL_STATUS_SUCCESS) {
		AUTO_REG_LOGI("===ZZZZ::[%s]%s, %d\n", __FILE__,__FUNCTION__, __LINE__);
	}
}
 
void ril_query_imsi_number(void)
{
	ril_status_t ret = RIL_STATUS_SUCCESS;
	
	auto_register_read_iccid();
	
	ret = ril_request_imsi(RIL_ACTIVE_MODE, register_query_imsi_callback, NULL);
	AUTO_REG_LOGI("===ZZZZ::ril_query_imsi_number\n");
	if (ret != RIL_STATUS_SUCCESS) {
		AUTO_REG_LOGI("===ZZZZ::[%s]%s, %d\n", __FILE__,__FUNCTION__, __LINE__);
	}
}
