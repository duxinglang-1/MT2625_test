...
#define MSDC_STOP_DMA(msdc_register_base) \
    do { \
        ...
        MSDC_TIMEOUT_WAIT(((!(msdc_register_base->DMA_CTRL & MSDC_DMA_CTRL_STOP)) || (!(msdc_register_base->DMA_CFG & MSDC_DMA_CFG_STS))), 500); \
    } while(0)
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
static void sdio_backup_all_register(hal_sdio_port_t sdio_port)
{
    ...
    sdio_backup_register.PATCH_BIT1   = msdc_register_base->PATCH_BIT1;
}


static void sdio_restore_all_register(hal_sdio_port_t sdio_port)
{
    msdc_register_t *msdc_register_base;
    ...
    msdc_register_base->PATCH_BIT1   = sdio_backup_register.PATCH_BIT1;
}

static void sdio_backup_all(void *data, uint32_t mode)
{
    log_hal_info("sdio_backup_all ++\r\n");
...
static void sdio_restore_all(void *data, uint32_t mode)
{
    log_hal_info("sdio_restore_all ++\r\n");
...
hal_sdio_status_t hal_sdio_init(hal_sdio_port_t sdio_port, hal_sdio_config_t *sdio_config)
{
    ...
    if (sdio_information.is_busy) {
        ...
        if (MSDC_OWNER_SD == msdc_get_owner((msdc_port_t)sdio_port)) {
            ...
            log_hal_error("msdc is used by SD\r\n");
            return HAL_SDIO_STATUS_ERROR;
        } else {
    ...
    msdc_init((msdc_port_t)sdio_port);


#ifdef HAL_SLEEP_MANAGER_ENABLED
    sdio_backup_parameter.sdio_port = sdio_port;
    ...
    /*enable SDIO mode*/
    msdc_register_base->SDC_CFG |= SDC_CFG_SDIO;

    /*enable 74 serial clock*/
    msdc_register_base->MSDC_CFG = msdc_register_base->MSDC_CFG | MSDC_CFG_CKPDN;

    /*enable SDIO interrupt*/
    msdc_register_base->SDC_CFG |= SDC_CFG_SDIOIDE;

    msdc_wait(10);

    /*disable serial clock*/
    msdc_register_base->MSDC_CFG = msdc_register_base->MSDC_CFG & (~MSDC_CFG_CKPDN); // it is a debug clk. in real application, it will disable, that will only have clk when sdio used
    ...
    msdc_nvic_set((msdc_port_t)sdio_port,true);     // TODO: may think to be false, we may just open nvic in none blocking mode
...
void msdc_dump_debug_reg(hal_sdio_port_t sdio_port)
{
    uint32_t i = 0;
    uint32_t reg;
    msdc_register_t *msdc_register_base;
    ...
    for (i = 0; i < 0x19; i++) {
        msdc_register_base->SW_DBG_DEL = i;
        msdc_wait(1);
        reg = msdc_register_base->SW_DBG_OUT;
        log_hal_info("i = %x, reg = %x \r\n", (unsigned int)i, (unsigned int)reg);
    }
}


void msdc_dump_all_reg(hal_sdio_port_t sdio_port)
{
    msdc_register_t *msdc_register_base;

    msdc_register_base = (HAL_SDIO_PORT_0 == sdio_port) ? MSDC0_REG : MSDC1_REG;

    log_hal_info("MSDC_CFG:0x%08x\r\n",msdc_register_base->MSDC_CFG);
    log_hal_info("MSDC_IOCON:0x%08x\r\n",msdc_register_base->MSDC_IOCON);
    log_hal_info("MSDC_PS:0x%08x\r\n",msdc_register_base->MSDC_PS);
    log_hal_info("MSDC_INT:0x%08x\r\n",msdc_register_base->MSDC_INT);
    log_hal_info("MSDC_INTEN:0x%08x\r\n",msdc_register_base->MSDC_INTEN);
    log_hal_info("MSDC_FIFOCS:0x%08x\r\n",msdc_register_base->MSDC_FIFOCS);
    log_hal_info("MSDC_TXDATA:0x%08x\r\n",msdc_register_base->MSDC_TXDATA);
    log_hal_info("MSDC_RXDATA:0x%08x\r\n",msdc_register_base->MSDC_RXDATA);
    log_hal_info("SDC_CFG:0x%08x\r\n",msdc_register_base->SDC_CFG);
    log_hal_info("SDC_CMD:0x%08x\r\n",msdc_register_base->SDC_CMD);
    log_hal_info("SDC_ARG:0x%08x\r\n",msdc_register_base->SDC_ARG);
    log_hal_info("SDC_STS:0x%08x\r\n",msdc_register_base->SDC_STS);
    log_hal_info("SDC_RESP0:0x%08x\r\n",msdc_register_base->SDC_RESP0);
    log_hal_info("SDC_RESP1:0x%08x\r\n",msdc_register_base->SDC_RESP1);
    log_hal_info("SDC_RESP2:0x%08x\r\n",msdc_register_base->SDC_RESP2);
    log_hal_info("SDC_RESP3:0x%08x\r\n",msdc_register_base->SDC_RESP3);
    log_hal_info("SDC_BLK_NUM:0x%08x\r\n",msdc_register_base->SDC_BLK_NUM);
    log_hal_info("SDC_CSTS:0x%08x\r\n",msdc_register_base->SDC_CSTS);
    log_hal_info("SDC_CSTS_EN:0x%08x\r\n",msdc_register_base->SDC_CSTS_EN);
    log_hal_info("SDC_DATCRC_STS:0x%08x\r\n",msdc_register_base->SDC_DATCRC_STS);
    log_hal_info("SDC_ACMD_RESP:0x%08x\r\n",msdc_register_base->SDC_ACMD_RESP);
    log_hal_info("DMA_SA:0x%08x\r\n",msdc_register_base->DMA_SA);
    log_hal_info("DMA_CA:0x%08x\r\n",msdc_register_base->DMA_CA);
    log_hal_info("DMA_CTRL:0x%08x\r\n",msdc_register_base->DMA_CTRL);
    log_hal_info("DMA_CFG:0x%08x\r\n",msdc_register_base->DMA_CFG);
    log_hal_info("SW_DBG_SEL:0x%08x\r\n",msdc_register_base->SW_DBG_DEL);
    log_hal_info("SW_DBG_OUT:0x%08x\r\n",msdc_register_base->SW_DBG_OUT);
    log_hal_info("DMA_LENGTH:0x%08x\r\n",msdc_register_base->DMA_LENGTH);
    log_hal_info("PATCH_BIT0:0x%08x\r\n",msdc_register_base->PATCH_BIT0);
    log_hal_info("PATCH_BIT1:0x%08x\r\n",msdc_register_base->PATCH_BIT1);
    log_hal_info("PAD_TUNE:0x%08x\r\n",msdc_register_base->PAD_TUNE);
    log_hal_info("DAT_RD_DLY0:0x%08x\r\n",msdc_register_base->DAT_RD_DLY0);
    log_hal_info("DAT_RD_DLY1:0x%08x\r\n",msdc_register_base->DAT_RD_DLY1);
    log_hal_info("MAIN_VER:0x%08x\r\n",msdc_register_base->MAIN_VER);
    log_hal_info("ECO_VER:0x%08x\r\n",msdc_register_base->ECO_VER);
}
...
hal_sdio_status_t hal_sdio_execute_command53(hal_sdio_port_t sdio_port, hal_sdio_command53_config_t *command53_config)
{
...
#ifdef MSDC_USE_INT
    msdc_register_base->MSDC_INTEN |= MSDC_DAT_INTS_EN | MSDC_INTEN_DXFER_DONE;
#endif
    ...
    if (command53_config->direction) {
        ...
        while (word_count) {    // word_count is the total send amount
            uint32_t msdc_fifo_space_in_word = (MSDC_FIFO_SIZE - MSDC_TXFIFOCNT(msdc_register_base)) >> 2;

            uint32_t send_word = MINIMUM (word_count, msdc_fifo_space_in_word);
            ...
            word_count = word_count - send_word;
        }
    } else {
        /*read data*/
        if (command53_config->block) {
        ...
        while (word_count) {
            ...
            word_count = word_count - want_comsume_word;
        }
    }
...
void msdc_dma_stop(hal_sdio_port_t sdio_port)
{
    ...
    log_hal_info("***dma status: 0x%08x", (unsigned int)msdc_register_base->DMA_CFG);
...
hal_sdio_status_t hal_sdio_execute_command53_dma_blocking(hal_sdio_port_t sdio_port, hal_sdio_command53_config_t *command53_config)
{
    ...
    /*use basic DMA mode.*/
    msdc_register_base->DMA_CTRL &= ~MSDC_DMA_CTRL_MODE;

    msdc_register_base->DMA_CTRL = (msdc_register_base->DMA_CTRL & (~MSDC_DMA_CTRL_BRUSTSZ)) | (MSDC_DMA_BURST_64_BYTES << 12);
    ...
    status = sdio_wait_data_ready(sdio_port);
    if (status != NO_ERROR) {
        msdc_dump_debug_reg(sdio_port);
        goto error;
    }

    msdc_dma_stop(sdio_port);
    msdc_reset((msdc_port_t)sdio_port);
...
hal_sdio_status_t hal_sdio_execute_command53_dma_gpd(hal_sdio_port_t sdio_port, hal_sdio_command53_config_t *command53_config)
{
    ...
    msdc_register_base->DMA_CFG |= MSDC_DMA_CFG_DECSEN;
