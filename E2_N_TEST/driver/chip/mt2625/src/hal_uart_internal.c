...
{
    uint32_t irq_status = save_and_set_interrupt_mask();
    ...
    uartx->FCR_UNION.FCR_CELLS.CLRR = 1;
    uartx->FCR_UNION.FCR_CELLS.CLRT = 1;
...
void uart_print_debug(void)
{
    ...
    for( i = 0; i < 4; i++) {
        printf("UART%d reg dump\n", i);
        for( j = 0; j < 32; j++) {
            printf("%x ", g_uart_reg_dump[i][j]);
        }
        printf("\n");
    }
}
...
void uart_purge_fifo(UART_REGISTER_T *uartx, int32_t is_rx)
{
    ...
    FCR = UART_FCR_RXTRIG_12 | UART_FCR_FIFOE_MASK;
...
void uart_set_auto_baudrate(UART_REGISTER_T *uartx, bool is_enable, uint32_t current_baudrate)
{
    if (is_enable) {
        ...
        uartx->RATEFIX_UNION.RATEFIX_CELLS.AUTOBAUD_SAMPLE = 0xd;
...
bool uart_get_auto_baudrate_done(UART_REGISTER_T *uartx)
{
    return (uartx->AUTOBAUD_CON_UNION.AUTOBAUD_CON_CELLS.AUTOBAUD_EN == 0)?true:false;
}

uint32_t uart_get_auto_baudrate_result(UART_REGISTER_T *uartx)
{
    return (uartx->AUTOBAUD_REG_UNION.AUTOBAUD_REG_CELLS.AUTOBAUD_RATE);
}

void uart_send_xoff(UART_REGISTER_T *uartx)
{
    uartx->EFR_UNION.EFR_CELLS.SW_FLOW_CONT = 0x1;
    uartx->EFR_UNION.EFR_CELLS.SEND_XOFF = 0x1;
//    uartx->EFR_UNION.EFR_CELLS.SW_FLOW_CONT = 0x3;
}

void uart_send_xon(UART_REGISTER_T *uartx)
{
    uartx->EFR_UNION.EFR_CELLS.SW_FLOW_CONT = 0x1;
    uartx->EFR_UNION.EFR_CELLS.SEND_XON = 0x1;
    uartx->EFR_UNION.EFR_CELLS.SW_FLOW_CONT = 0x3;
}
