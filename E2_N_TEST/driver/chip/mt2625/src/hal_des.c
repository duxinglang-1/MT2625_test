...
static uint8_t des_sleep_handler = 0;


static void des_operation_done(hal_nvic_irq_t irq_number)
{
    NVIC_DisableIRQ((IRQn_Type)CRYPTO_IRQn);
//    NVIC_ClearPendingIRQ((IRQn_Type)CRYPTO_IRQn);

    g_des_op_done = true;

    NVIC_EnableIRQ((IRQn_Type)CRYPTO_IRQn);
#ifdef HAL_SLEEP_MANAGER_ENABLED
    if (hal_sleep_manager_release_sleeplock(des_sleep_handler, HAL_SLEEP_LOCK_ALL) == HAL_SLEEP_MANAGER_OK){
        log_hal_info("des op-done release sleep lock OK.");
    }else{
        log_hal_error("des op-done release sleep lock Failed.");
    }
#endif
}
                                       ...
                                       uint8_t init_vector[HAL_DES_CBC_IV_LENGTH])
{
    g_des_op_done = false;
#ifdef HAL_SLEEP_MANAGER_ENABLED
    if (des_sleep_handler == 0) {
        des_sleep_handler = hal_sleep_manager_set_sleep_handle("SEC_DES");
    }
    if (hal_sleep_manager_acquire_sleeplock(des_sleep_handler, HAL_SLEEP_LOCK_ALL) == HAL_SLEEP_MANAGER_OK){
        log_hal_info("des encrypt acquire sleep lock OK.");
    }else{
        log_hal_error("des encrypt acquire sleep lock Failed.");
    }
#endif
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
        if (hal_sleep_manager_release_sleeplock(des_sleep_handler, HAL_SLEEP_LOCK_ALL) == HAL_SLEEP_MANAGER_OK){
            log_hal_info("des encrypt release sleep lock OK.");
        }else{
            log_hal_error("des encrypt release sleep lock Failed.");
        }
#endif
                                       ...
                                       uint8_t init_vector[HAL_DES_CBC_IV_LENGTH])
{
    ...
    if (hal_sleep_manager_acquire_sleeplock(des_sleep_handler, HAL_SLEEP_LOCK_ALL) == HAL_SLEEP_MANAGER_OK){
        log_hal_info("des decrypt acquire sleep lock OK.");
    }else{
        log_hal_error("des decrypt acquire sleep lock Failed.");
    }
#endif
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
        if (hal_sleep_manager_release_sleeplock(des_sleep_handler, HAL_SLEEP_LOCK_ALL) == HAL_SLEEP_MANAGER_OK){
            log_hal_info("des decrypt release sleep lock OK.");
        }else{
            log_hal_error("des decrypt release sleep lock Failed.");
        }
#endif
        log_hal_error("des_operate fail.");
        return HAL_DES_STATUS_ERROR;
...
//    NVIC_SetPriority((IRQn_Type)CRYPTO_IRQn, CM4_MTK_CRYPTO_PRI);
//    NVIC_EnableIRQ((IRQn_Type)CRYPTO_IRQn);

    des_set_key(key->length, key->buffer);
    if (DES_TYPE_CBC == mode)  /* DES_TYPE_CBC */
    {
        des_configure(2, 1);  /* 0:efuse and 1:software   0:ECB and 1:CBC */
    } else {  /* DES_TYPE_ECB */
        des_configure(2, 0);  /* 0:efuse and 1:software   0:ECB and 1:CBC */
    }
    if (block_num > 0) {
        ...
        uint8_t data_block[HAL_DES_MIN_SIZE] = {0};
