...
{
    WDT_REGISTER->WDT_SW_RSTSYS = WDT_SYSTEM_SW_RST_KEY | WDT_STANDARD_1_MASK;
    hal_gpt_delay_us(reset_time_us);
    WDT_REGISTER->WDT_SW_RSTSYS = WDT_SYSTEM_SW_RST_KEY;
}

void wdt_set_pmu_mask(uint32_t enable)
{
    if (0 == enable) {
        WDT_REGISTER->WDT_MASK1_UNION.WDT_MASK1 = ((WDT_PMU_RST_KEY) << (WDT_STANDARD_16_OFFSET));
    } else {
        WDT_REGISTER->WDT_MASK1_UNION.WDT_MASK1 = ((WDT_STANDARD_1_MASK | WDT_PMU_RST_KEY) << (WDT_STANDARD_16_OFFSET));
    }
}
