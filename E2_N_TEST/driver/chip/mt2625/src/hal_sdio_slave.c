...
{
    log_hal_info("sdio_slave_backup_all++\r\n");
    //*SPM_SDIO_SLV_SRAM_PD  |= 0x010101;   // it still have some problem, may cause sdio internal sram be reseted, so just send all 0 to host
    //spm_control_mtcmos(SPM_MTCMOS_SDIO_SLV, SPM_MTCMOS_PWR_DISABLE);  // TODO: we may use this method later
    log_hal_info("sdio_slave_backup_all--\r\n");
}

void sdio_slave_restore_all(void *data)
{
    log_hal_info("sdio_slave_restore_all++\r\n");
    //*SPM_SDIO_SLV_SRAM_PD  |= 0x0100;    // it still have some problem, may cause sdio internal sram be reseted, so just send all 0 to host
    //spm_control_mtcmos(SPM_MTCMOS_SDIO_SLV, SPM_MTCMOS_PWR_ENABLE);     // TODO: we may use this method later
    log_hal_info("sdio_slave_restore_all--, SPM_SDIO_SLV_SRAM_PD=%x\r\n", *SPM_SDIO_SLV_SRAM_PD);
}
#endif
...
hal_sdio_slave_status_t hal_sdio_slave_init(hal_sdio_slave_port_t sdio_slave_port)
{
    sdio_slave_init();
#ifdef HAL_SLEEP_MANAGER_ENABLED
    sleep_management_register_suspend_callback(SLEEP_BACKUP_RESTORE_SDIO_SLAVE, (sleep_management_suspend_callback_t)sdio_slave_backup_all, NULL, HAL_SLEEP_MODE_LIGHT_SLEEP);
    sleep_management_register_resume_callback(SLEEP_BACKUP_RESTORE_SDIO_SLAVE, (sleep_management_suspend_callback_t)sdio_slave_restore_all, NULL, HAL_SLEEP_MODE_LIGHT_SLEEP);
#endif
