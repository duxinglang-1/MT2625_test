...
#define IS_SYS_EXCEPTION (((portNVIC_INT_CTRL_REG & 0xFFL) < 16) && ((portNVIC_INT_CTRL_REG & 0xFFL) != 0))
...
ATTR_TEXT_IN_TCM uint32_t sf_get_current_time(void)
{
#if defined(__UBL__)
    uint32_t counter = 0;
...
int SF_DAL_OTPErase_WINBOND(void *MTDData, uint32_t offset);
...
#define TimeStampSet(type) do   {   \
    ...
    timestamp[timestamp_ptr]=(D->DriverStatus!=SFDRV_PROGRAMMING)?(SF_DAL_GetCurrentTime()):(sf_get_current_time()); \
...
#define SF_Suspend_Start() do {   \
    SuspendTime_Start = (D->DriverStatus==SFDRV_PROGRAMMING)?(sf_get_current_time()):(SF_DAL_GetCurrentTime()); \
    TimeStampSet(TIME_STAMP_SUSPEND); \
} while(0)

#define SF_Suspend_End() do {   \
    SuspendTime_End = (D->DriverStatus==SFDRV_PROGRAMMING)?(sf_get_current_time()):(SF_DAL_GetCurrentTime()); \
...
#define SF_Prog_Start() do { \
    ProgTime_Start = sf_get_current_time(); \
} while(0)

#define SF_Prog_End() do { \
    ProgTime_End=sf_get_current_time(); \
...
ATTR_TEXT_IN_RAM void Flash_ReturnReady(void)
{
...
#ifdef SF_DAL_PROG_STATISTICS    
    gNOR_ReturnReadyBegin = sf_get_current_time(); // get start time of Flash_ReturnReady (Only when flash is busy)   
    ...
    if ((!SF_DAL_FLAG_SUS()) && SF_DAL_FLAG_BUSY()) {
        ...
        sf_dal_suspend_ready_start_time = sf_get_current_time();
        ...
        if (0 != (status & D->StatusMap[SF_SR_BUSY])) {
            ...
            sf_dal_polling_start_time = sf_get_current_time();
            ...
            while (1) {
                ...
                sf_dal_polling_end_time = sf_get_current_time();
                ...
                if (QBIT(sf_dal_polling_end_time) > SF_READY_LIMIT / 2)   {
                    ...
                    sf_dal_polling_stage_start_time = sf_get_current_time();
                }
#endif
            }
...
#ifdef SF_DAL_SUSPEND_CHECK
        sf_dal_suspend_ready_end_time = sf_get_current_time();
...
#ifdef SF_DAL_PROG_STATISTICS
    {
        gNOR_ReturnReadyEnd = sf_get_current_time();   // get end time of Flash_ReturnReady    
...
#ifdef __SECURITY_OTP__
    D->OTPLength = 0x200; //0x000~0x1FF
    D->OTPBase = 0x0; // User:0x000~0x1FF; Factory: 0x200~0x3FF
...
int32_t SF_DAL_Init_WINBOND_OTP(NOR_MTD_Driver *driver, SF_MTD_Data *D)
{
    ...
    D->OTPErase = SF_DAL_OTPErase_WINBOND;
    ...
    D->CMD->OTPErase = SF_CMD_WINBOND_ERASE_SECURITY_REG;
...
#endif
#if defined(SF_FIX_WINBOND_PD)
            D->sf_dal_fix |= sf_fix_winbond_pd;
#endif
...
ATTR_TEXT_IN_RAM int SF_DAL_ProgramData(void *MTDData, void *Address, void *Data, uint32_t Length)
{
    ...
    while (0 != Length) {
        ...
        if (D->sf_dal_fix & sf_fix_fidelix_q32a) {
            ...
            SFI_MacEnable(D->CS);
            SFI_MacWaitReady(D->CS);
            sf_dal_resume_time = sf_get_current_time();
        }
#endif
...
ATTR_TEXT_IN_RAM int SF_DAL_EraseBlock(void *MTDData, uint32_t BlockIndex)
{
    ...
    //blockindex only suppurting 0x00FFFFFF
    erase_block = ((BlockIndex >> 24) & 0xFF);
    BlockIndex = (BlockIndex & 0xFFFFFF);
    ...
    if (BlockIndex >> SFC_GENERIC_31_BIT_OFFSET == 0)  {
        ...
        // Caculate D->CurrAddr
        {
            ...
            //cur_address = (void *)((uint32_t)BlockAddress(D, BlockIndex));
            //D->CurrAddr = cur_address;

            D->CurrAddr = (uint8_t *)BlockAddress(D, BlockIndex);
        }
      ...
      if ((erase_block != 0xFA) && (erase_block != 0xFB) && (erase_block != 0xFC)) {
            ...
            switch (eraseunit) {
                ...
                default: ASSERT_RET(0, 0);
            }
      } else {
            //SDK api request erase type is high priority
            switch (erase_block) {
                case 0xFA:  D->CMD->Erase = D->CMD->Erase4K;  break; // 4KB
                case 0xFB:  D->CMD->Erase = D->CMD->Erase32K; break; // 32KB
                case 0xFC:  D->CMD->Erase = D->CMD->Erase64K; break; // 64KB
                default:
                    break;
            }
        }
...
ATTR_TEXT_IN_RAM int SF_DAL_OTPErase_WINBOND(void *MTDData, uint32_t offset)
{
    uint32_t          savedMask;
    uint32_t          cmd;
    ...
    cmd = (SFI_ReverseByteOrder(optr_physical) & SFC_GENERIC_0xFFFFFF00_MASK ) | D->CMD->OTPErase;
    //Save current mode
    ...
    if (D->FlashID[1] != 0x60 && D->FlashID[1] != 0x40 && D->FlashID[1] != 0x70) { //Only switch to SPI for QPI device
    ...
    if (D->FlashID[1] != 0x60 && D->FlashID[1] != 0x40 && D->FlashID[1] != 0x70) { //Only switch to SPI for QPI device, Quad bit is default set to 1 and Wrap setting is not changed by 0xFF command for QPI device
        RestoreIRQMask(savedMask); //restore IRQ here to avoid 60Qbits assert
        savedMask = SaveAndSetIRQMask();
...
ATTR_TEXT_IN_RAM int SF_DAL_OTPWrite_GIGADEVICE(void *MTDData, uint32_t Offset, void *BufferPtr, uint32_t Length)
{
    ...
    for (i = 0; i < Length; i++, bptr++, optr_logical++)    {
        ...
        SFI_Dev_Command(D->CS, D->CMD->OTPExit);  // Switch device to QPI mode
        SFI_SetDevMode(D->CS, save_QPI); // Switch SFC to QPI mode
        SFI_Dev_Command(D->CS, 0xC012); //Set dummy cycle and Wrap
...
extern signed char custom_setSFI_reinit(void);

ATTR_TEXT_IN_RAM void SF_DAL_Dev_ReinitFlah(void *MTDData)
{
    SF_MTD_Data *D = MTDData;
#if defined(SF_DAL_WINBOND) && defined(SF_FIX_WINBOND_PD)
    if (D->sf_dal_fix & sf_fix_winbond_pd) {
        custom_setSFI();
    }
#endif
}
