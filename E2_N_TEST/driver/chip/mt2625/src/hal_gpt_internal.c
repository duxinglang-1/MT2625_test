...
ATTR_TEXT_IN_TCM void gpt_delay_time(GPT_REGISTER_T *gpt, const uint32_t count)
{
    ...
    if (end_count > current) {
        while (gpt_current_count(gpt) >= current
               && gpt_current_count(gpt) < end_count);     /* current time < end_time */
...
{
    gp_gpt[gpt_port]->GPT_IRQ_EN = 0;                       /* disable interrupt */
    gp_gpt[gpt_port]->GPT_CON_UNION.GPT_CON = 0;            /* disable timer     */
    gp_gpt[gpt_port]->GPT_CLR = GPT_COUNT_CLEAR;            /* clear counter value */

    while (gp_gpt_glb->GPT_CLRSTA & (1<<gpt_port));
    gp_gpt[gpt_port]->GPT_CLK     = GPT_CLOCK_32KHZ;  /* default 32Mhz, divide 1 */
    gp_gpt[gpt_port]->GPT_IRQ_ACK = GPT_IRQ_FLAG_ACK; /* clear interrupt status */
    gp_gpt[gpt_port]->GPT_COMPARE = 0xffffffff;       /* set max threshold value */

    if (gpt_port == 5) {
        log_hal_info("set cmph to 0\r\n");
        gp_gpt[gpt_port]->GPT_COMPAREH = 0;
    }
}
...
uint32_t gpt_sw_get_current_time_ms(GPT_REGISTER_T *gpt)
{
    ...
    gpt_sw_context.is_gpt_count_valid = true;
    gp_gpt[HAL_GPT_SW_PORT]->GPT_CLR = GPT_COUNT_CLEAR;
    restore_interrupt_mask(mask);
    
    while (gp_gpt_glb->GPT_CLRSTA & (1 << HAL_GPT_SW_PORT));
