...
int32_t readRawDiskData(NOR_FLASH_DISK_Data *D, uint32_t addr, uint8_t *data_ptr, uint32_t len)
{
    ...
    //retrieve_FDM_lock();
    if (GetFDMLock() != HAL_FLASH_STATUS_OK) {
        return HAL_FLASH_STATUS_ERROR_LOCKED;
    }
...
int32_t eraseRawDiskBlock(NOR_FLASH_DISK_Data *D, uint32_t blkIdx)
{
    ...
    int32_t result;
    uint32_t block_index = (blkIdx & 0xFFFFFF);
    if (!D->is_mount) {
        MountDevice(D);
    }
...
int32_t readOtpDiskData(NOR_FLASH_DISK_Data *D, uint32_t addr, uint8_t *data_ptr, uint32_t len)
{
    //int32_t result;
    uint32_t otpSize;

    D->MTDDriver->OTPQueryLength(D->MTDData, (UINT *)&otpSize);

    if (addr >= otpSize || addr + len > otpSize) {
        return RAW_DISK_ERR_WRONG_ADDRESS;
    }
    ...
    D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_READ, addr, data_ptr, len);
...
int32_t writeOtpDiskData(NOR_FLASH_DISK_Data *D, uint32_t addr, uint8_t *data_ptr, uint32_t len)
{
    uint32_t dest, src;
    uint32_t otpSize;
    int32_t result = FLASH_DISK_DONE;
    ...
    while (DoneLength < len) {
        ...
        // if dest address not word align or write length is one, write one byte at a time
        if ( ((uint32_t)dest & (sizeof(FLASH_CELL) - 1)) || ((len - DoneLength) == 1) ) {
            result = D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_WRITE, (UINT)dest, (void *)src, 1);
            if (result != FLASH_DISK_DONE ) {
                break;
            }
            DoneLength++;
            ...
            if (programBytes == sizeof(FLASH_CELL) &&  (src % sizeof(FLASH_CELL))) {
                FLASH_CELL Cell;
                uint8_t *b = (uint8_t *) &Cell;
                b[0] = *((uint8_t *)src);   // LSB address
                b[1] = *((uint8_t *)src + 1);   // LSB + 1 address
                result = D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_WRITE, (UINT)dest, (void *)Cell, programBytes);
                if (result != FLASH_DISK_DONE ) {
                    break;
                }
            } else {
                if (programBytes & 0x1) { //must be Sibley flash
                    ...
                    result = D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_WRITE, (UINT)dest, (void *)src, programBytes);
                }
                if (result != FLASH_DISK_DONE ) {
                    break;
                }
...
int32_t lockOtpDiskData(NOR_FLASH_DISK_Data *D)
{
    if (!D->is_mount) {
        MountDevice(D);
    }
    ...
    D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_LOCK, 0, NULL, 0);
...
int32_t eraseOtpDiskData(NOR_FLASH_DISK_Data *D, uint32_t addr)
{
    if (!D->is_mount) {
        MountDevice(D);
    }
    ...
    D->MTDDriver->OTPAccess(D->MTDData, FS_OTP_ERASE, 0, NULL, 0);
...
hal_flash_status_t hal_flash_otp_read(uint32_t start_address, uint8_t *buffer, uint32_t length)
{
    ...
    result = readOtpDiskData(D, start_address, buffer, length);
...
hal_flash_status_t hal_flash_otp_write(uint32_t start_address, uint8_t *buffer, uint32_t length)
{
    ...
    result = writeOtpDiskData(D, start_address, buffer, length);
...
hal_flash_status_t hal_flash_otp_erase(uint32_t address)
{
    NOR_FLASH_DISK_Data *D = ENTIRE_DISK_DRIVER_DATA;
    int32_t result;

    result = eraseOtpDiskData(D, address);
...
hal_flash_status_t hal_flash_get_otp_length(uint32_t *length)
{
    ...
    D->MTDDriver->OTPQueryLength(D->MTDData, (UINT *)length);
