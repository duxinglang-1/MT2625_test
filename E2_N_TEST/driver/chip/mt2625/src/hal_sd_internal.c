...
sd_internal_status_t sd_wait_data_ready(hal_sd_port_t sd_port)
{
    ...
    if (count >= MSDC_TIMEOUT_PERIOD_DATA) {
        log_hal_error("wait sd data ready timeout. count >= MSDC_TIMEOUT_PERIOD_DATA\r\n");
        return ERROR_DATA_TIMEOUT;
    }

    if (interrupt_status & MSDC_INT_DATTMO) {
        log_hal_error("wait sd data ready timeout. MSDC_INT_DATTMO, interrupt_status = %x\r\n", (unsigned int)interrupt_status);
        return ERROR_DATA_TIMEOUT;
...
sd_internal_status_t sd_wait_data_ready_dma(hal_sd_port_t sd_port)
{
    ...
    do {
        ...
        hal_gpt_get_free_run_count(HAL_GPT_CLOCK_SOURCE_1M, &end_count);
        hal_gpt_get_duration_count(start_count, end_count, &count);
    } while ((!(interrupt_status & (MSDC_DAT_INTS | MSDC_INT_DXFER_DONE))) && msdc_card_is_present((msdc_port_t)sd_port) && (count < MSDC_TIMEOUT_PERIOD_DATA));


    /*clear msdc interrupt status*/
    msdc_register_base->MSDC_INT = (MSDC_DAT_INTS | MSDC_INT_DXFER_DONE);
...
sd_internal_status_t sd_send_command(hal_sd_port_t sd_port,uint32_t msdc_command, uint32_t argument)
{
    ...
    while (msdc_register_base->SDC_STS & SDC_STS_SDCBUSY);  // DE said wait SDCBUSY instead of CMDBUSY or it may meet R1B wrong. refert to MSDC IP programming guide figure 1.6.10
...
void sd_get_bit_field_value(uint8_t *destination, uint8_t *source, uint16_t start_bit, uint16_t width)
{
    ...
    memset(destination, 0, (width >> 8) + (0 == (width & 0x07) ? 0 : 1));   // the width in here is in bits. the memset is in byte
    for (i = 0; i < width; i++) {
        bytes = (start_bit + i) >> 3;
...
void sd_analysis_csd(uint32_t *csd)
{
    ...
    sd_information.csd.read_bl_len = 1 << sd_information.csd.read_bl_len;   // here we transfer to decimal, refer to spec
...
sd_internal_status_t sd_get_scr(hal_sd_port_t sd_port)
{
    ...
    while (i < 2) {
        if (4 <= (msdc_register_base->MSDC_FIFOCS & MSDC_FIFOCS_RXCNT)) {
...
sd_internal_status_t sd_query_switch_high_speed(hal_sd_port_t sd_port,uint32_t argument)
{
    ...
    read_word_count = COMMAND6_RESPONSE_LENGTH >> 2;    // one word = 4 bytes
...
sd_internal_status_t sd_latch_read_blocks(hal_sd_port_t sd_port,uint32_t *read_buffer,  uint32_t read_address, uint32_t block_number)
{
    ...
    while (read_word_count) {
        fifo_count = ((msdc_register_base->MSDC_FIFOCS & MSDC_FIFOCS_RXCNT) >> 0);
...
static void sd_dma1_interrput_gpt_callback(void *user_data)
{
    sd_internal_status_t status = NO_ERROR;
    uint32_t response = 0;
    static uint32_t count = 0;
    ...
    if (count < 1500) {
        ...
        count = 0;
        /*free gpt timer*/
        hal_gpt_sw_free_timer(sd_dma_interrupt_gpt_timer[MSDC_PORT_1]);
        sd_dma_interrupt_error_handle(HAL_SD_PORT_1,ERROR_STATUS);
        return;
    }

    if (!((MSDC_IS_CMD_BUSY(MSDC1_REG) || MSDC_IS_SDC_BUSY(MSDC1_REG)) && msdc_card_is_present((msdc_port_t)HAL_SD_PORT_1))) {    
        status = sd_send_command(HAL_SD_PORT_1,MSDC_COMMAND13, sd_information.rca << COMMAND_RCA_ARGUMENT_SHIFT);
        if (NO_ERROR != status) {
            sd_dma_interrupt_error_handle(HAL_SD_PORT_1,status);
        }
        response = MSDC1_REG->SDC_RESP0;
        ...
        if ((response & SD_CARD_STATUS_READY_FOR_DATA_BIT_MASK)) {
            ...
            msdc_reset((msdc_port_t)HAL_SD_PORT_1);
            MSDC1_REG->MSDC_INTEN = 0;
            msdc_dma_disable((msdc_port_t)HAL_SD_PORT_1);
            MSDC1_REG->MSDC_INT = MSDC1_REG->MSDC_INT;
            /*call user callback*/
            msdc_config[MSDC_PORT_1].msdc_sd_callback(HAL_SD_EVENT_SUCCESS,NULL);

            sd_dma_interrupt_context.sd_current_write_read_block_num = 0;

            msdc_nvic_set((msdc_port_t)HAL_SD_PORT_1,false);
            sd_information.is_busy = false;
            return;
        }           
    }
...
hal_sd_status_t sd_wait_dma_interrupt_transfer_ready(hal_sd_port_t sd_port)
{
    ...
    if (1 < sd_dma_interrupt_context.sd_current_write_read_block_num) {
        while (30 > retry) {
            status = sd_send_command(sd_port,MSDC_COMMAND12, COMMAND_NO_ARGUMENT);  // it must send stop command in multi block transmission.
            if (status != NO_ERROR) {
                retry++;
            } else {
                break;
    ...
    if ((MSDC_IS_CMD_BUSY(msdc_register_base) || MSDC_IS_SDC_BUSY(msdc_register_base)) && msdc_card_is_present((msdc_port_t)sd_port)) {
         if (HAL_SD_PORT_0 == sd_port) {
             ...
             hal_gpt_sw_get_timer(&(sd_dma_interrupt_gpt_timer[MSDC_PORT_1]));
             hal_gpt_sw_start_timer_ms(sd_dma_interrupt_gpt_timer[MSDC_PORT_1],1,sd_dma1_interrput_gpt_callback,&sd_dma_interrupt_context);
         }
