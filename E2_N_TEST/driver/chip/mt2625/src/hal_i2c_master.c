...
volatile static uint8_t i2c_dma_callback_count[HAL_I2C_MASTER_MAX] = {0, 0, 0};
...
static sleep_management_lock_request_t i2c_lock_sleep_handle[HAL_I2C_MASTER_MAX] = {SLEEP_LOCK_I2C0, SLEEP_LOCK_I2C1, SLEEP_LOCK_I2C2};

#define I2C_LOCK_SLEEP(i2c_port) \
do{ \
    hal_sleep_manager_acquire_sleeplock(i2c_lock_sleep_handle[i2c_port], HAL_SLEEP_LOCK_ALL); \
}while(0)

#define I2C_UNLOCK_SLEEP(i2c_port) \
do{ \
    hal_sleep_manager_release_sleeplock(i2c_lock_sleep_handle[i2c_port], HAL_SLEEP_LOCK_ALL); \
}while(0)
...
void hal_i2c_isr_handler(hal_i2c_port_t i2c_port, hal_i2c_callback_t user_callback, void *user_data)
{
    ...
    if (I2C_TRANSACTION_SUCCESS != transaction_status) {
#ifdef I2C_ERROR_LOG_PRINTF
        log_hal_error("[I2C%d][isr]:status=%d\r\n", i2c_port, transaction_status);
#endif
        /* In case of dummy interrupt */
        if (transaction_status == I2C_TRANSACTION_UNKNOWN_ERROR) {
            return;
        }
    }
