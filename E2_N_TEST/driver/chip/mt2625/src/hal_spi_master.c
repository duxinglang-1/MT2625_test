...
static const uint16_t g_spi_master_pri_code[HAL_SPI_MASTER_MAX] = {SPI_MST0_PRIORITY, SPI_MST1_PRIORITY};
        ...
        hal_spi_master_config_t *spi_config)
{
    ...
    sleep_management_register_suspend_callback(SLEEP_BACKUP_RESTORE_SPI_MASTER, (sleep_management_suspend_callback_t)spi_master_backup_register_callback, NULL, HAL_SLEEP_MODE_LIGHT_SLEEP|HAL_SLEEP_MODE_DEEP_SLEEP|HAL_SLEEP_MODE_DEEPER_SLEEP);
    sleep_management_register_resume_callback(SLEEP_BACKUP_RESTORE_SPI_MASTER, (sleep_management_resume_callback_t)spi_master_restore_register_callback, NULL, HAL_SLEEP_MODE_LIGHT_SLEEP|HAL_SLEEP_MODE_DEEP_SLEEP|HAL_SLEEP_MODE_DEEPER_SLEEP);
#endif
...
hal_spi_master_status_t hal_spi_master_deinit(hal_spi_master_port_t master_port)
{
    ...
    /* disable interrupt */
    NVIC_DisableIRQ(g_spi_master_irq_code[master_port]);
        ...
        void *user_data)
{
    ...
    NVIC_SetPriority(g_spi_master_irq_code[master_port], g_spi_master_pri_code[master_port]);
    NVIC_EnableIRQ(g_spi_master_irq_code[master_port]);
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
        hal_sleep_manager_acquire_sleeplock(SLEEP_LOCK_SPI_MASTER, HAL_SLEEP_LOCK_ALL);
#endif
