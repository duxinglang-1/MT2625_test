...
PDMA_REGISTER_T  *pdma[PDMA_NUMBER] = {PDMA4, PDMA5, PDMA6, PDMA7, PDMA8, PDMA9, PDMA10, PDMA11, PDMA12, PDMA13, PDMA14, PDMA15};

/*peripheral dma global status base address array */
PDMA_REGISTER_GLOBAL_T  *pdma_global[PDMA_NUMBER] = {PDMA_GLOAL1, PDMA_GLOAL1, PDMA_GLOAL1, PDMA_GLOAL1,PDMA_GLOAL2, PDMA_GLOAL2, PDMA_GLOAL2, PDMA_GLOAL2,
                                                      PDMA_GLOAL2, PDMA_GLOAL2, PDMA_GLOAL2, PDMA_GLOAL2 };

/*peripheral dma global status running bit location array */
uint32_t g_pdma_global_running_bit[PDMA_NUMBER] = {PDMA_CHANNEL4_OFFSET, PDMA_CHANNEL5_OFFSET, PDMA_CHANNEL6_OFFSET,PDMA_CHANNEL7_OFFSET, PDMA_CHANNEL8_OFFSET, PDMA_CHANNEL9_OFFSET,
                                                  PDMA_CHANNEL10_OFFSET, PDMA_CHANNEL11_OFFSET, PDMA_CHANNEL12_OFFSET,PDMA_CHANNEL13_OFFSET, PDMA_CHANNEL14_OFFSET, PDMA_CHANNEL15_OFFSET};

/*peripheral dma init status*/
volatile static uint8_t pdma_init_status[PDMA_END_CHANNEL] = {0};

/*virtual fifo dma base address array */
VDMA_REGISTER_T  *vdma[VDMA_NUMBER] = {VDMA16, VDMA17,VDMA18, VDMA19,VDMA20, VDMA21, VDMA22, 
                                       VDMA23, VDMA24, VDMA25, VDMA26, VDMA27
                                      };

/*virtual fifo dma port address array */
VDMA_REGISTER_PORT_T  *vdma_port[VDMA_NUMBER] = {VDMA16_PORT, VDMA17_PORT,VDMA18_PORT, VDMA19_PORT, VDMA20_PORT, VDMA21_PORT, VDMA22_PORT,
                                                 VDMA23_PORT, VDMA24_PORT, VDMA25_PORT, VDMA26_PORT, VDMA27_PORT
                                                };
...
uint32_t g_vdma_global_running_bit[VDMA_NUMBER] = {VDMA_CHANNEL16_OFFSET, VDMA_CHANNEL17_OFFSET,VDMA_CHANNEL18_OFFSET, VDMA_CHANNEL19_OFFSET, 
                                                   VDMA_CHANNEL20_OFFSET, VDMA_CHANNEL21_OFFSET, VDMA_CHANNEL22_OFFSET, VDMA_CHANNEL23_OFFSET, 
                                                   VDMA_CHANNEL24_OFFSET, VDMA_CHANNEL25_OFFSET,VDMA_CHANNEL26_OFFSET, VDMA_CHANNEL27_OFFSET
                                                  };
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
extern uint8_t g_gdma_sleep_handle[GDMA_NUMBER];
extern bool g_gdma_sleep_lock_status[GDMA_NUMBER];
#endif
...
pdma_status_t pdma_init(pdma_channel_t channel)
{
	...
	dma_enable_clock(DMA_PD);
	dma_enable_clock(DMA_AO);
    ...
    hal_nvic_register_isr_handler(DMA_MCU_IRQn, dma_interrupt_hander);
    NVIC_EnableIRQ(DMA_MCU_IRQn);
    hal_nvic_register_isr_handler(DMA_SENSOR_IRQn, sensor_dma_interrupt_hander);
    NVIC_EnableIRQ(DMA_SENSOR_IRQn);
    return  PDMA_OK;
}
...
pdma_status_t pdma_deinit(pdma_channel_t channel)
{
    ...
    dma_disable_clock(DMA_PD);
    dma_disable_clock(DMA_AO);
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
        if(g_gdma_sleep_lock_status[count] == true)
        {
            g_gdma_sleep_lock_status[count] = false;
            hal_sleep_manager_release_sleeplock(g_gdma_sleep_handle[count], HAL_SLEEP_LOCK_ALL);
        }
#endif
...

{
    ...
    /*define tmp callback variable */
    hal_gdma_callback_t callback1;
    pdma_callback_t callback2;
    ...
    /*mask SENSOR DMA interrupt */


    /* step 1: handle gdma in SENSOR DMA domain --> gdma does not exsit, so none handle*/
	 for (count = SENSOR_GDMA_START_NUMBER - 1; count < GDMA_NUMBER; count++) {
        global_status = gdma_global[count]->GDMA_GLBSTA;
        ...
        if (global_status & (0x1 << (g_gdma_global_running_bit[count] + 1))) {

            {
                ...
                gdma[count]->GDMA_START &= ~GDMA_START_BIT_MASK;
                /*call vdma user's callback */
                callback1 = g_gdma_callback[count].func;
                argument = g_gdma_callback[count].argument;
                if (callback1 != NULL) {
                    callback1(HAL_GDMA_EVENT_TRANSACTION_SUCCESS, argument);
                } else {
                    assert(0);
                }
            }
    ...
    /*step 2: handle pdma  in SENSOR DMA domain-->  pdma does not exsit, so none handle*/

	     for (count = SENSOR_PDMA_START_NUMBER - 1; count < PDMA_NUMBER; count++) {
        global_status = pdma_global[count]->PDMA_GLBSTA;
        ...
        if (global_status & (0x1 << (g_pdma_global_running_bit[count] + 1))) {

            {
                ...
                pdma[count]->PDMA_START &= ~PDMA_START_BIT_MASK;
                /*call pdma user's callback */
                callback2 = g_pdma_callback[count].func;
                argument = g_pdma_callback[count].argument;
                if (callback2 != NULL) {
                    callback2(PDMA_EVENT_TRANSACTION_SUCCESS, argument);
                } else {
                    assert(0);
                }
            }
    ...
    /*step 3: handle vdma  in  SENSOR DMA domain */


    /*unmask SENSOR DMA interrupt*/
}
