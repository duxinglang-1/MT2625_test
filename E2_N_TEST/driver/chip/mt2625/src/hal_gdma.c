...
uint32_t g_gdma_global_running_bit[GDMA_NUMBER] = {GDMA_CHANNEL0_OFFSET, GDMA_CHANNEL1_OFFSET,GDMA_CHANNEL2_OFFSET };
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
    if(g_gdma_sleep_lock_status[index] == false)
    {
        g_gdma_sleep_lock_status[index] = true;
        hal_sleep_manager_acquire_sleeplock(g_gdma_sleep_handle[index], HAL_SLEEP_LOCK_ALL);
    }
#endif
    /*start gdma transfer*/
    gdma_start(index);
