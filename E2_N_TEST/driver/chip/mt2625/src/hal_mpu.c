...
{
    uint32_t temp_mpu_size = (uint32_t)region_config->mpu_region_size;
    /* Region is invalid */
    if (region >= HAL_MPU_REGION_MAX) {
        return HAL_MPU_STATUS_ERROR_REGION;
    }
    ...
    if (region_config->mpu_region_address != (region_config->mpu_region_address & (0xFFFFFFFFUL << (HAL_MPU_REGION_SIZE_32B + 1)))) {
        return HAL_MPU_STATUS_ERROR_REGION_ADDRESS;
    }

    while (region_config->mpu_region_address != (region_config->mpu_region_address & (0xFFFFFFFFUL << (temp_mpu_size + 1)))) {
        temp_mpu_size --;
    }
    ...
    MPU->RASR = ((temp_mpu_size << MPU_RASR_SIZE_Pos) | (region_config->mpu_region_access_permission << MPU_RASR_AP_Pos) | (region_config->mpu_subregion_mask << MPU_RASR_SRD_Pos));
