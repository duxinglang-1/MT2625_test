...
#define EMI_SUPPORT_SLEEP_BACKUP
...
#ifdef EMI_SUPPORT_SLEEP_BACKUP
...
//	#endif
   ...
   __EMI_SetSR(0x0,0xb);//this function called by BL, because BL run @104M so,PSRAM setting latency = 2, driving = 1/8
   __EMI_SetSR(0x4,0x8);
    while ((emi_register->EMI_MREG_RW & EMI_MRGE_ACC_IDLE_MASK) == 0);
...
#else
    emi_register->EMI_CONM_UNION.EMI_CONM_CELLS.REQ_MASK = 0x0;
#endif
...
ATTR_TEXT_IN_TCM int32_t hal_emi_dynamic_clock_switch(emi_clock clock,hal_emi_clock_frq target_freq)
{
        ...
        return -1;	
#endif
    if(target_freq == EMI_CLK_13M)
    {
       emi_register->EMI_GENA = 1<<24; //when run @26M with 0.9v, emi need a special setting
       emi_io_register->EMI_IOA = 0x0F010F01;//when run @26M with 0.9v, emi need a special setting
       emi_io_register->EMI_IOB = 0x0F010F01;//when run @26M with 0.9v, emi need a special setting

       __EMI_SetSR(0x0,0xb);//latency = 2, driving = 1/8
    }
    else if(target_freq == EMI_CLK_52M)
    {
