...
#define NOR_BLOCK_SIZE  0x8000      // TODO  (  : i think we can change to 0x10000, but this parameter seems no use)
...
/* File system block size:
    ...
    block configuration        FS size Limit                                               note
    4kB block                  24kB  <= FS Size <= 508kB (127*4kB)                 the file system is limited by 508kB
    32kB block                128kB <= FS Size <= 6064kB (127*32kB)              the FDM overhead is at least two blocks: 32*2 = 64kB
    64kB block                256kB <= FS Size <= 8128kB (127*64kB)              the FDM overhead is at least two blocks: 64*2 = 128kB

*/
#define FS_NOR_BLOCK_SIZE                  (0x8000)
/* File system base address which is a offset from flash start. */
#define ROM_FS_BASE                        (0x00400000) // 0x00100000 (0x00400000: start from 4M)
/*File system size */
#define ROM_FS_LENGHT                      (0x002E0000)	// assume 3M length
