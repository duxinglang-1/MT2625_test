...
void bsp_sensor_peripheral_init(void)
{
...
#ifdef BMI160_I2C
    /* Config I2C2 */ // driver/chip/inc/hal_i2c_master.h
    hal_gpio_init(HAL_GPIO_33);
    hal_gpio_init(HAL_GPIO_34);

    hal_pinmux_set_function(HAL_GPIO_33, HAL_GPIO_33_I2C2_SCL);
    hal_pinmux_set_function(HAL_GPIO_34, HAL_GPIO_34_I2C2_SDA);

    mems_i2c_init(HAL_I2C_MASTER_2, HAL_I2C_FREQUENCY_400K);
#elif defined(BMI160_SPI)
    /* Config SPI0 */
    hal_pinmux_set_function(HAL_GPIO_8, HAL_GPIO_8_SPI_MST0_CS);
    hal_pinmux_set_function(HAL_GPIO_9, HAL_GPIO_9_SPI_MST0_MISO);
    hal_pinmux_set_function(HAL_GPIO_10, HAL_GPIO_10_SPI_MST0_MOSI);
    hal_pinmux_set_function(HAL_GPIO_11, HAL_GPIO_11_SPI_MST0_SCK);
    ...
    mems_spi_init(SPI_SELECT_BMI160, HAL_SPI_MASTER_0, &spi_config, 0);
#endif

#ifdef MTK_SENSOR_ACCELEROMETER_GYROSCOPE_USE_BMI160
    /* Accelerometer, Gyro on the same HW component*/
    /* GPIO[12] function7 for EINT12, BMI160 */
    ...
    eint1_config.firq_enable = false;
  #endif

    hal_eint_init(HAL_EINT_NUMBER_7, &eint1_config);

    bmi160_eint_set(HAL_EINT_NUMBER_7);
    hal_eint_register_callback(HAL_EINT_NUMBER_7, bmi160_eint_handler, NULL);
