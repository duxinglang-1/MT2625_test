...
#include "mt2625_hdk_backlight.h"

#include "hal_gpio.h"
#ifdef HAL_ISINK_MODULE_ENABLED
#include "hal_isink.h"
#endif
#if 1//def HAL_DISPLAY_LCD_MODULE_ENABLED
...
bsp_backlight_status_t bsp_backlight_init_external_isink(void)
{
    ...
    hal_pinmux_set_function(LCD_PWM, 0); // ISINK EN
    hal_gpio_set_direction(LCD_PWM, HAL_GPIO_DIRECTION_OUTPUT);

    hal_gpio_set_output(LCD_PWM, HAL_GPIO_DATA_LOW);
    hal_gpio_set_output(LCD_PWM, HAL_GPIO_DATA_HIGH);
