...
// #define __SR_DEBUG__ 1

#ifdef MTK_SENSOR_USE_BMI160_BUS_I2C
#define BMI160_I2C 1
#elif defined(MTK_SENSOR_USE_BMI160_BUS_SPI)
#define BMI160_SPI 1
#endif
#ifdef BMI160_I2C
...
uint8_t ATTR_ZIDATA_IN_NONCACHED_RAM __attribute__ ((aligned(4))) bmi160_txbuffer[MAX_WRITE_LENGTH*2];
#endif
...
#ifdef __SR_DEBUG__
#define LOGI(fmt,arg...)   printf("BMI160: "fmt,##arg)
#define LOGW(fmt,arg...)   printf("BMI160: "fmt,##arg)
#define LOGE(fmt,arg...)   printf("BMI160: "fmt,##arg)
#else
/* syslog */
...
#ifdef __SR_DEBUG__
#define LOGD(fmt,arg...)   printf("BMI160: "fmt,##arg)
#else
...
struct bmi160_t {
    ...
    unsigned int acc_timeout;
    unsigned int gyro_timeout;
    unsigned int int_en;
...
uint8_t ATTR_ZIDATA_IN_NONCACHED_RAM __attribute__ ((aligned(4))) bmi160_receive_buffer[8];
uint8_t ATTR_ZIDATA_IN_NONCACHED_RAM __attribute__ ((aligned(4))) int_en[3]; /* one byte more for full-duplex */
uint8_t ATTR_ZIDATA_IN_NONCACHED_RAM __attribute__ ((aligned(4))) int_map[3]; /* one byte more for full-duplex */
uint8_t ATTR_ZIDATA_IN_NONCACHED_RAM __attribute__ ((aligned(4))) eint_status[5]; /* one byte more for full-duplex */
...
#else
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t send_buffer[2];
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t bmi160_receive_buffer[8];
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t int_en[3]; /* one byte more for full-duplex */
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t int_map[3]; /* one byte more for full-duplex */
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t eint_status[5]; /* one byte more for full-duplex */
ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t fifo_data_store[MAX_FIFO_BYTE_COUNT + 1]; /* one byte more for full-duplex */
#endif
...
static void mt_device_usleep(int usec)
{
    ...
    hal_gpt_delay_ms((usec + 999) / 1000); // sleep in 1 ms unit
    // LOGI("\nCalled mt_device_usleep=%dusec)", usec);
}

#ifdef BMI160_I2C
static int32_t bmi160_I2C_multi_write(uint8_t addr, uint8_t *value, uint16_t len) // static int32_t bmp280_I2C_multi_write(uint8_t addr, uint8_t *value, uint16_t len)
{
    uint8_t reg_addr = addr;
    uint8_t reg_len = len;
    uint8_t *ptxbuffer = bmi160_txbuffer;
    int32_t ret;
    ...
    ret = mems_i2c_write(BMI160_I2C_ADDRESS, bmi160_txbuffer, (len * 2));
...
static int bmi160_SPI_write(uint8_t addr, uint8_t value)
{
    ...
    ret = mems_spi_write(SPI_SELECT_BMI160, send_buffer, 2U);
#else
    uint8_t send[2];
    ...
    ret = mems_spi_write(SPI_SELECT_BMI160, send, 2U);
#endif /*SPI_DMA_NON_CACHEABLE_ACCESS*/
...
static int32_t bmi160_bus_write(uint8_t addr, uint8_t value)
{
    int32_t ret = 0;

#ifdef BMI160_I2C
    ret = bmi160_I2C_multi_write(addr, &value, 1);
#elif defined(BMI160_SPI)
    ret = bmi160_SPI_write(addr, value);
#endif
...
#ifdef BMI160_I2C
uint8_t ATTR_ZIDATA_IN_NONCACHED_RAM __attribute__ ((aligned(4))) bmi160_reg_addr;
static int32_t bmi160_I2C_multi_read(uint8_t addr, uint8_t *result, uint16_t len)
{
    int32_t ret;
    bmi160_reg_addr = addr;

    ret = mems_i2c_write_read(BMI160_I2C_ADDRESS, bmi160_reg_addr, result, len);

    return ret;
}

static int32_t bmi160_I2C_multi_read_dma(uint8_t addr, uint8_t *result, uint16_t len)
{
    int32_t ret;
    bmi160_reg_addr = addr;

    ret = mems_i2c_write_read_dma(BMI160_I2C_ADDRESS, bmi160_reg_addr, result, len);
...
{
    uint8_t send;
    int32_t ret = HAL_SPI_MASTER_STATUS_OK;

    send = (addr) | SPI_READ_CMD;

#ifdef SPI_DMA_NON_CACHEABLE_ACCESS
     ret = mems_spi_write_read(SPI_SELECT_BMI160, send, bmi160_receive_buffer, 1);
    *result = bmi160_receive_buffer[1];
#else
    uint8_t receive[2] = {0};
    ret = mems_spi_write_read(SPI_SELECT_BMI160, send, receive, 1);
    *result = receive[1];
#endif /*SPI_DMA_NON_CACHEABLE_ACCESS*/
...
#endif
static int32_t bmi160_bus_read(uint8_t addr, uint8_t *result)
{
    int32_t ret = 0;

#ifdef BMI160_I2C
    ret = bmi160_I2C_multi_read(addr, result, 1);
#elif defined(BMI160_SPI)
    ret = bmi160_SPI_read(addr, result);
#endif
...
static int bmi160_SPI_multipe_read(uint8_t addr, uint8_t *result, uint16_t len)
{
    ...
    ret = mems_spi_write_read(SPI_SELECT_BMI160, send, result, len);

    return ret;
}
#endif

static int bmi160_bus_multipe_read(uint8_t addr, uint8_t *result, uint16_t len)
{
    int32_t ret = 0;

#ifdef BMI160_I2C
    ret = bmi160_I2C_multi_read(addr, result, len);
#elif defined(BMI160_SPI)
    ret = bmi160_SPI_multipe_read(addr, result, len);
#endif

    return ret;
}

static int bmi160_bus_multipe_read_dma(uint8_t addr, uint8_t *result, uint16_t len)
{
    int32_t ret = 0;

#ifdef BMI160_I2C
    ret = bmi160_I2C_multi_read_dma(addr, result, len);
#elif defined(BMI160_SPI)
    ret = bmi160_SPI_multipe_read(addr, result, len);
#endif
...
static int bmi160_NVM_Prog(void)
{
    ...
    /* NVM_CONF */
    bmi160_bus_write(BMI160_USER_CONF_ADDR, 0x2);

    /* NVM Programming */
    bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, CMD_PROG_NVM);

    /* read status */
    while ((receive[1] & BMI160_USER_STATUS_NVM_RDY__MSK) != BMI160_USER_STATUS_NVM_RDY__MSK) {
        bmi160_bus_multipe_read(BMI160_USER_STATUS_ADDR, receive,2);
        mt_device_usleep(5000);
        LOGD("[1B]: 0x%X\n", receive[1]);
    }

    /* NVM_CONF */
    bmi160_bus_write(BMI160_USER_CONF_ADDR, 0);
...
static int bmi160_set_acc_pmu_mode(unsigned char mode)
{
    ...
    bmi160_bus_read(BMI160_USER_ACC_CONF_ADDR, receive);

    if (mode == ACC_PMU_LOW1) {
        // undersampling mode must be enabled for low power mode
        bmi160_bus_write(BMI160_USER_ACC_CONF_ADDR, (receive[0] | BMI160_USER_ACC_US__MSK));
    } else if (mode == ACC_PMU_NORMAL) {
        bmi160_bus_write(BMI160_USER_ACC_CONF_ADDR, (receive[0] & (~BMI160_USER_ACC_US__MSK)));
    }

    bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, (CMD_ACC_SET_PMU_MODE | mode));

    /* Average execution time for set accelereometer PMU mode to normal: 0.35 ms.
       another 0.3 ms required. Refer to datasheet. */
    hal_gpt_delay_ms(1U);

    receive[0] = 0;
    while (((receive[0] & ACC_PMU_MASK) >> ACC_PMU_POS) != mode) {
        bmi160_bus_read(BMI160_USER_PMU_STATUS_ADDR, receive);
        LOGD("[0x03] 0x%X\n", receive[0]);
...
static int bmi160_set_gyro_pmu_mode(unsigned char mode)
{
    ...
    bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, (CMD_GYR_SET_PMU_MODE | mode));
...
static void bmi160_FIFO_enable(void)
{
    ...
    bmi160_bus_write(BMI160_USER_FIFO_CONFIG_1_ADDR, setting);

    /* fifo flush */
    bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, CMD_FIFO_FLUSH);

    /* FIFO watermake */
    if (t_bmi160.fifo_watermark > 0) {
        bmi160_bus_write(BMI160_USER_FIFO_CONFIG_0_ADDR, (uint8_t)(t_bmi160.fifo_watermark));
    }
...
void bmi160_pedometer_enable(unsigned int enable)
{
    ...
    bmi160_bus_read(BMI160_USER_STEP_CONF_1, receive);

    if (enable != 0) {
        /* Soft reset */
        bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, CMD_STEM_CNT_CLR);

        // enable
        if ((receive[0] & BMI160_USER_STEP_CNT_EN__MSK) == 0) {
            bmi160_bus_write(BMI160_USER_STEP_CONF_1, receive[0] | BMI160_USER_STEP_CNT_EN__MSK);
        }
    } else {
        // disable
        if ((receive[0] & BMI160_USER_STEP_CNT_EN__MSK) != 0) {
...
unsigned short bmi160_pedometer_cnt(void)
{
    ...
    bmi160_bus_multipe_read(BMI160_USER_STEP_CNT_0, receive, 2);
...
#ifdef SPI_DMA_NON_CACHEABLE_ACCESS
    bmi160_bus_multipe_read(BMI160_USER_FIFO_LENGTH_0_ADDR, bmi160_receive_buffer, 2);

    #ifdef BMI160_I2C
        length = ((((unsigned short)bmi160_receive_buffer[1]) & 0x7) << 8) | bmi160_receive_buffer[0];
    #elif defined(BMI160_SPI)
        length = ((((unsigned short)bmi160_receive_buffer[2]) & 0x7) << 8) | bmi160_receive_buffer[1];
    #endif
#else
    unsigned char receive[3] = {0}; /* one byte more for full-duplex */
    bmi160_bus_multipe_read(BMI160_USER_FIFO_LENGTH_0_ADDR, receive, 2);
...
void bmi160_fifo_data(unsigned char *data, unsigned short length)
{
    ...
    bmi160_bus_multipe_read_dma(BMI160_USER_FIFO_DATA_ADDR, data, length);
...
int bmi160_sensor_Cali_Offset(int *x_offset, int *y_offset, int *z_offset)
{
    ...
    /* FOC_CONF */
    bmi160_bus_write(BMI160_USER_FOC_CONF_ADDR, receive[0]);

    /* Fast Offset Calibration */
    bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, CMD_START_FOC);

    /* read status */
    receive[0] = 0;
    while ((receive[0] & BMI160_USER_STATUS_FOC_RDY__MSK) != BMI160_USER_STATUS_FOC_RDY__MSK) {
        bmi160_bus_read(BMI160_USER_STATUS_ADDR, receive);
        mt_device_usleep(1000);
        LOGD("[1B]: 0x%X\r\n", receive[0]);
    }
    ...
    /* multiple read */
    bmi160_bus_multipe_read(BMI160_USER_OFFSET_0_ADDR, receive, len);
...
int bmi160_read_acc_xyz(struct bmi160acc_t *acc)
{
    ...
    /* multiple read */
    bmi160_bus_multipe_read(BMI160_USER_DATA_14_ADDR, receive, 6);
...
int bmi160_read_gyro_xyz(struct bmi160gyro_t *gyro)
{
    ...
    /* multiple read */
    bmi160_bus_multipe_read(BMI160_USER_DATA_8_ADDR, receive, 6);
...
void bmi160_dump_reg(void)
{
    ...
    bmi160_bus_multipe_read(BMI160_USER_FIFO_DOWNS_ADDR, receive, 3);
    LOGI("Reg_0x45: 0x%X, 0x%X, 0x%X\n", receive[1], receive[2], receive[3]);

    bmi160_bus_multipe_read(BMI160_USER_INT_EN_0_ADDR, receive, 8);
...
static void bmi160_int_enable(unsigned int which)
{
    ...
    if (t_bmi160.int_en == 0) {
        ...
        bmi160_bus_write(addr, value);
        t_bmi160.int_en |= which;
    }

    // INT_EN
    addr = BMI160_USER_INT_EN_0_ADDR;
    bmi160_bus_multipe_read(addr, int_en, 2);

    // INT_MAP
    addr = BMI160_USER_INT_MAP_0_ADDR;
    bmi160_bus_multipe_read(addr, int_map, 2);
    ...
    if (int_en[1] != 0) {
        addr = BMI160_USER_INT_EN_0_ADDR;
        bmi160_bus_write(addr, int_en[1]);
    }
    if (int_en[2] != 0) {
        addr = BMI160_USER_INT_EN_1_ADDR;
        bmi160_bus_write(addr, int_en[2]);
    }
    if (int_map[1] != 0) {
        addr = BMI160_USER_INT_MAP_0_ADDR;
        bmi160_bus_write(addr, int_map[1]);
    }
    if (int_map[2] != 0) {
        addr = BMI160_USER_INT_MAP_1_ADDR;
        bmi160_bus_write(addr, int_map[2]);
    }
...
void bmi160_eint_handler(void *parameter)
{
    ...
    hal_eint_mask((hal_eint_number_t)t_bmi160.eint_num);
    hal_eint_unmask((hal_eint_number_t)t_bmi160.eint_num);
                    ...
                    void *buff_in, int32_t size_in)
{
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                if (setting > 0) {
                    ...
                    bmi160_bus_write(BMI160_USER_ACC_CONF_ADDR, receive[0]);
                } else {
                    err = -1;
        ...
        case SENSOR_GET_DATA: { // output unit is mm/s^2
            ...
            bmi160_bus_multipe_read(BMI160_USER_INT_STATUS_0_ADDR, eint_status, 4);
            #ifdef BMI160_I2C
                t_bmi160.eint_status = (unsigned int) ((eint_status[3] << 24)
                                                       | (eint_status[2] << 16)
                                                       | (eint_status[1] << 8)
                                                       | (eint_status[0]));
            #elif defined(BMI160_SPI)
            ...
            if ((t_bmi160.eint_status & FLAT_INT) && ((t_bmi160.eint_status & FLAT_STATUS) == 0)) {
                ...
                #ifdef BMI160_I2C
                    fifo_time = (unsigned int) ((((unsigned int)fifo_data_store[fifolength+3]) << 16)
                                                             |(((unsigned int)fifo_data_store[fifolength+2]) << 8)
                                                             | (fifo_data_store[fifolength+1]));
                #elif defined(BMI160_SPI)
        ...
        #ifdef BMI160_I2C
            bmi_fifo_analysis_handle(fifo_data_store, fifolength);
        #elif defined(BMI160_SPI)
        ...
        case SENSOR_BATCH:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                ...
                /* 1 ACC data of 7 bytes*/
                t_bmi160.fifo_watermark = ((value/t_bmi160.acc_delay)*7)/4;
                LOGI("acc timeout(%ld), delay(%ld), watermark(%ld)\r\n", value, t_bmi160.acc_delay, t_bmi160.fifo_watermark);

                if ((t_bmi160.fifo_watermark > 0) && (t_bmi160.fifo_watermark <= 256)) {
                    bmi160_FIFO_disable();
                    bmi160_FIFO_enable();
                    t_bmi160.acc_timeout = value;
                } else {
                    err = -1;
                    LOGE("ACC SENSOR_BATCH invalid FIFO watermark (%ld) \r\n", value);
                }
            }
            break;
    ...
    switch (command) {
        case SENSOR_DELAY:
            if ((buff_in == NULL) || (size_in < sizeof(int))) {
                ...
                int setting = valid_odr(value);
                if (setting > 0) {
                    bmi160_bus_read(BMI160_USER_GYR_CONF_ADDR, receive);
                    receive[0] = (receive[0] & (~BMI160_USER_GYR_ODR__MSK)) | (uint8_t)setting;
                    bmi160_bus_write(BMI160_USER_GYR_CONF_ADDR, receive[0]);
                } else {
                    err = -1;
        ...
        case SENSOR_BATCH:
            if ((buff_in == NULL) || (size_in < sizeof(int32_t))) {
                LOGE("acc+gyro SENSOR_BATCH parameter error!\n");
                err = -1;
            } else {
                value = *(uint32_t *)buff_in;

                if (value < t_bmi160.acc_timeout) {
                    value = t_bmi160.acc_timeout;
                }

                /* 1 ACC data of 7 bytes*/
                t_bmi160.fifo_watermark =( ((value/t_bmi160.acc_delay)*7)+(((value/t_bmi160.gyro_delay)*7)))/4;

                LOGI("acc+gyro timeout(%ld), delay(%ld,%ld), watermark(%ld)\r\n", value, t_bmi160.acc_delay, t_bmi160.gyro_delay, t_bmi160.fifo_watermark);
...
void ags_init(void)
{
    ...
    /* Soft reset */
    bmi160_bus_write(BMI160_CMD_COMMANDS_ADDR, CMD_SOFTRESET);
    ...
    while (receive[0] != t_bmi160.chip_id) {
        /* read Chip Id */
        bmi160_bus_read(BMI160_USER_CHIP_ID_ADDR, receive);
        LOGI("chip id == 0x%X\n", receive[0]);
    ...
    bmi160_bus_read(BMI160_USER_ACC_RANGE_ADDR, receive);
    receive[0] = ((receive[0] & (~BMI160_USER_ACC_RANGE__MSK)) | t_bmi160.acc_range);

    hal_gpt_delay_ms(2); // TODO: Try and Error, Need to confirm with FAE
    bmi160_bus_write(BMI160_USER_ACC_RANGE_ADDR, receive[0]);
    ...
    fail_cout = 1;
    while (receive[0] != t_bmi160.acc_range) {        
        LOGE("acc_range = %d\r\n", receive[0]);        
        receive[0] = ((receive[0] & (~BMI160_USER_ACC_RANGE__MSK)) | t_bmi160.acc_range);
        ...
        hal_gpt_delay_ms(2); // TODO: Try and Error, Need to confirm with FAE
        bmi160_bus_read(BMI160_USER_ACC_RANGE_ADDR, receive);
        if (receive[0] == t_bmi160.acc_range) {
            break;
        }        
        fail_cout++;
        if(fail_cout > 5) {
    ...
    bmi160_bus_read(BMI160_USER_GYR_RANGE_ADDR, &(t_bmi160.gyro_range));

    //+++
    //LOGD("acc gyro offset en \r\n");
    //bmi160_bus_read(BMI160_USER_OFFSET_6_ADDR, receive);
    ...
    obj_gyro.polling = 0; // polling mode
