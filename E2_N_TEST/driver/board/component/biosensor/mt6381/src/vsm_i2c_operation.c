...
#define LOGE(fmt,arg...)   printf("[vsm_i2c]"fmt,##arg)
#define LOGW(fmt,arg...)   printf("[vsm_i2c]"fmt,##arg)
#define LOGI(fmt,arg...)   printf("[vsm_i2c]"fmt,##arg)
//#define LOGD(fmt,arg...)   printf("[vsm_i2c]"fmt,##arg)
#else
#include "syslog.h"

log_create_module(vsm_i2c, PRINT_LEVEL_INFO);

#define LOGE(fmt,arg...)   LOG_E(vsm_i2c, fmt,##arg)
#define LOGW(fmt,arg...)   LOG_W(vsm_i2c, fmt,##arg)
#define LOGI(fmt,arg...)   LOG_I(vsm_i2c, fmt,##arg)
#endif
...
void i2c_init(uint8_t port, uint32_t i2c_freq)
{
    vsm_i2c_work_port = port;
...
{
    vsm_i2c_work_port = HAL_I2C_MASTER_MAX;
    vsm_i2c_work_freq = HAL_I2C_FREQUENCY_MAX;
#ifdef VSM_BUS_USE_DMA
    g_vsm_i2c_timeout_tick = 0;
    vSemaphoreDelete(g_vsm_i2c_dma_semaphore);
#endif
}
...
vsm_status_t vsm_i2c_write(uint8_t slave_addr, uint8_t *data, uint16_t len)
{
    ...
    #ifdef VSM_BUS_USE_DMA /*DMA mode*/
    int i = 0;
    ...
    if(status != HAL_I2C_STATUS_OK) {
        LOGE("status = %d, port %d, freq %d\r\n", status, vsm_i2c_work_port, vsm_i2c_work_freq);
        return VSM_STATUS_ERROR;
    } else {
...
vsm_status_t vsm_i2c_write_read(uint8_t slave_addr, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    #ifndef VSM_BUS_USE_DMA /* Polling mode */
