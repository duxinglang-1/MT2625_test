 ...
 * |\b MT6381                     | MT6381 is a 2-in-1 bio-sensing AFE (analog front-end) to facilitate bio-signal acquisition. The AFE comprises of low noise voltage and current sensing channels and is capable of sensing EKG (Electrocardiography) and PPG (Photoplethysmography) simultaneously.|
 ...
 *
 * - \b EKG \b Channel. \n
 *   MT6381 integrates a high-resolution DC-coupled front end. It can endure wide input offset, large interference and different electrode impedance. Besides right leg drive mode, it supports a two-electrode mode for reducing electrode cost. Note that there is larger interference during the two-electrode mode. \n
 *   \n
 * - \b PPG \b Channel. \n
 *   MT6381 consists of LED drivers, a high-end DC-coupled current-mode front end and a flexible timing control for the PPG channel. The flexible timing control enables the users to control the device timing for different applications and to power down the device for power saving operations. The PPG front-end can sense up to four channels (two for signal and two for ambient) by time multiplexing. \n
 *
...
/** @brief This structure defines the calibration data structure. */
typedef struct {
    uint32_t pga6;       /**< Analog Gain.*/
    uint32_t ambdac5_5;  /**< Analog Offset.*/
} biometric_cali;
 ...
 * @param[in] data_buf_size is the size of data_buf.
 * @param[out] amb_buf is the ambient data buffer to receive.
 ...
 * @param[in] addr is the address of the register of mt6381. The first byte is the register offset. The second byte is the device address.
...
/**
 * @brief This function do calibration.
 * @param[out] cali is the calibration results.
 * @return #VSM_STATUS_ERROR, if the operation failed. \n
 *         #VSM_STATUS_OK, if the operation completed successfully. \n
 */
...
/**
 * @brief This function write calibration data.
 * @param[in] cali is the calibration data.
 * @return #VSM_STATUS_OK, if the operation completed successfully. \n
 */
...
/**
 * @brief This function read calibration data.
 * @param[out] cali is the calibration data.
 * @return #VSM_STATUS_OK, if the operation completed successfully. \n
 */
