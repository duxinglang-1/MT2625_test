...
typedef struct
{
    int32_t *input;           /**< A pointer to the input PPG signal. */
    int32_t *input_amb;       /**< A pointer to the input AMB signal. */
    int32_t input_fs;         /**< The sampling frequency (fs) of the input data. (supports 125Hz only). */
    int32_t input_length;     /**< The number of input samples. The maximum length = input_fs * 2= 250.*/
    int32_t input_bit_width;  /**< The bit width of the input data. If the data is already converted to signed representation, set bit width to 32. */
    int32_t input_config;     /**< Configuration of the input signal (Please set to 1). */
    int32_t input_source;     /**< 1: this is the data from MT2511 PPG channel 1 (PPG1). 2: this is the data from MT2511 PPG channel 2 (PPG2). */
} ppg_control_t;
...
*/
uint32_t ppg_control_get_version(void);
 ...
 *  
*/
int32_t ppg_control_process(
    ppg_control_t *ppg_control_input,    // The input signal data.
    int32_t ppg_control_mode,              // The PPG control mode.
    int32_t *ppg_control_output            // The output signal data.
);
...
*/
int32_t ppg_control_get_status(int32_t ppg_control_internal_config);
