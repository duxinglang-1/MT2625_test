...
**************************************************************************
 ** @file
 * GKI header file for the Rhino implementation of the GKI OS
 * abstraction macros.
 **************************************************************************/
 ...
 * @ingroup MdBlock
 */
typedef struct MdBlock * pMdBlock;

typedef struct MdBlockPool {
  uint32_t count;
  pMdBlock freeBlockList;
  uint8_t pool_number;
  uint32_t block_size;
} MdBlockPool, *pMdBlockPool;
...
extern unsigned MdBlockCreatePool (pMdBlockPool pool, /**< Pointer to memory pool. */
...
extern void * MdBlockAlloc (pMdBlockPool pool, uint32_t address);
extern void * MdBlockTryAlloc (pMdBlockPool pool, uint32_t address);
extern void MdBlockRelease (void * ptr);
extern int MdBlockIsAllocated (const void * ptr);
...
#define KI_OS_MEM_BLOCK_OVERHEAD    (sizeof(MdBlock) + KI_OS_MEM_BLOCK_TAIL_SIZE)
...
#define M_KiOsGetMemBlock(bLOCKpTR, pOOLnUM, AddRess)                                \
    {                                                                       \
        bLOCKpTR = MdBlockAlloc (&kiMemoryPoolIdTable[pOOLnUM], AddRess);      \
    }
#else 
#define M_KiOsGetMemBlock(bLOCKpTR, pOOLnUM)                           \
    {                                                                  \
        bLOCKpTR = MdBlockAlloc (&kiMemoryPoolIdTable[pOOLnUM]);      \
...
#define M_KiOsGetMemBlockPoll(bLOCKpTR, pOOLnUM, AddRess)                       \
    {                                                                  \
        bLOCKpTR = MdBlockTryAlloc (&kiMemoryPoolIdTable[pOOLnUM], AddRess);   \
    }
...
#define M_KiOsCreateMemPool(pOOLnUM)                                            \
    {                                                                           \
        ...
        MdBlockCreatePool (&kiMemoryPoolIdTable[pOOLnUM],            \
 ...
 * can be referenced.
 *
 *@param puxStackBuffer Used to pass a static task stack address
 * 
...
#define M_KiOsWaitSemaphoreEx(sEMAdEF)          \
    {                                           \
        xSemaphoreTake ((sEMAdEF).osHandle,portMAX_DELAY); \
    }
...
*                   or blocking should occur as a result of calling this macro.
*******************************************************************************/
#define uxSemaphoreGetCountFromISR uxSemaphoreGetCount
...
typedef MdBlockPool KiOsPoolHandle;
