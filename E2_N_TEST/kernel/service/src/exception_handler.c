...
bool IsDuplicatedMiniDump = false;
bool IsMiniDumpEnabled    = false;

/*Signal Send param*/
ATTR_NONINIT_DATA_IN_RAM MiniDump_SignalSendT MiniDump_SignalSend[MINIDUMP_SIGNAL_SEND_HISTORY_MAX_NUM];
uint32_t MiniDump_SignalSendIndex = 0;

/*Context Switch param*/
ATTR_NONINIT_DATA_IN_RAM MiniDump_ContextSwitchT MiniDump_ContextSwitch[MINIDUMP_CONTEXT_SWITCH_HISTORY_MAX_NUM];
uint32_t MiniDump_ContextSwitchIndex = 0;

/*Task info param*/
ATTR_NONINIT_DATA_IN_RAM MiniDump_TaskInfoHistoryT MiniDump_TaskInfoHistory[KI_NUM_OF_REAL_TASKS];
uint32_t MiniDump_TaskTimeStamp[KI_NUM_OF_REAL_TASKS];
ATTR_NONINIT_DATA_IN_RAM MiniDump_AllocateEmptyT MiniDump_AllocateEmpty[MINIDUMP_BLOCK_MAX_NUM];
...
ATTR_NONINIT_DATA_IN_RAM MiniDump_ModemT MiniDump_ModemDump[(MAX_USERID <= MINIDUMP_MODEM_MAX_USER_ID)?MAX_USERID:-1];
...
ATTR_NONINIT_DATA_IN_RAM static unsigned int xExceptionStack[EXCEPTION_STACK_WORDS] = {0}; /* reserved as exception handler's stack */
...
/* members must be add to the end!!!, because member's offset maybe used in  CommonFault_Handler*/
...
#ifdef EXCEPTION_AUTO_REBOOT
static int auto_reboot_flag = EXCEPTION_AUTO_REBOOT_MAGIC;
#else
...
#if defined(MTK_PORT_SERVICE_ENABLE)
serial_port_dev_t at_cmd_port = SERIAL_PORT_DEV_UART_0;
hal_uart_baudrate_t at_cmd_port_baudrate = HAL_UART_BAUDRATE_115200;
#endif

#if defined(MTK_NBIOT_TARGET_BUILD)
extern bool IsGenieConnected(void);
#else
#define IsGenieConnected() true
#endif

static bool auto_reboot_check(void);
static bool reboot_check(void);
void exception_reboot(void);
#ifdef OPTIMIZED_GKI_LOG_ENABLE
extern void KiLogExceptionHandshake(void);
#endif

#ifdef HAL_WDT_MODULE_ENABLED
static void exception_config_wdt(void)
{
...
bool exception_occured(void)
{
   return (IsSystemEnterException > 0);
}
...
void exception_get_at_port_config(void)
{
#if defined(MTK_PORT_SERVICE_ENABLE)
   serial_port_dev_setting_t dev;
   serial_port_dev_t serial_port;
#ifdef __NB_IOT_GSM_CCCI__
    serial_port_config_read_dev_number("connl2", &serial_port);
#else
    serial_port_config_read_dev_number("connl", &serial_port);
#endif
   at_cmd_port = serial_port;
   if(serial_port >= SERIAL_PORT_DEV_UART_TYPE_BEGIN && serial_port <= SERIAL_PORT_DEV_UART_TYPE_END)
   {
      serial_port_config_read_dev_setting(serial_port, &dev);
      at_cmd_port_baudrate =  dev.port_setting_uart.baudrate;
   }
#endif
}

static void print_assert_info_2_connl(void)
{
#if defined(MTK_PORT_SERVICE_ENABLE)
    char dbuf[200];
    int dbl=0;
    ...
    int line;
    hal_uart_port_t connl_uart_port;
    hal_uart_config_t uart_config;


    if(at_cmd_port >= SERIAL_PORT_DEV_UART_TYPE_BEGIN && at_cmd_port <= SERIAL_PORT_DEV_UART_TYPE_END)
    {
         connl_uart_port = (hal_uart_port_t)(at_cmd_port - SERIAL_PORT_DEV_UART_TYPE_BEGIN);
         //re-init connl uart as polling mode
         hal_uart_deinit(connl_uart_port);
         uart_config.word_length = HAL_UART_WORD_LENGTH_8;
         uart_config.stop_bit = HAL_UART_STOP_BIT_1;
         uart_config.parity = HAL_UART_PARITY_NONE;
         uart_config.baudrate = at_cmd_port_baudrate;
         hal_uart_init(connl_uart_port, &uart_config);
         
         dbl = snprintf(dbuf, 50, "\r\n ------------- Exception called ------------- ");
         hal_uart_send_polling(connl_uart_port, (uint8_t *)dbuf, dbl);
         exception_get_assert_expr(&expr, &file, &line);
         if (file)
         {
             dbl = snprintf(dbuf, 50, "\r\n assert failed: %s", expr);
             hal_uart_send_polling(connl_uart_port, (uint8_t *)dbuf, dbl);
             dbl = snprintf(dbuf, 200, "\r\n  file: %s", file);
             hal_uart_send_polling(connl_uart_port, (uint8_t *)dbuf, dbl);
             dbl = snprintf(dbuf, 20, "\r\n  line: %d\n\r", line);
             hal_uart_send_polling(connl_uart_port, (uint8_t *)dbuf, dbl);
         }
         dbl = snprintf(dbuf, 50, "\r\n genie connected: %s\n\r", (IsGenieConnected() ? "Yes": "No"));
         hal_uart_send_polling(connl_uart_port, (uint8_t *)dbuf, dbl);
         dbl = snprintf(dbuf, 50, "\r\n auto-reboot: %s\n\r", (auto_reboot_check()? "Yes": "No"));
         hal_uart_send_polling(connl_uart_port, (uint8_t *)dbuf, dbl);
    }
#endif    
}

static void save_cm4_registers(uint32_t stack[])
{
    ...
    taskContext.basepri = __get_BASEPRI();    
}
...
extern uint32_t g_uart_reg_dump[HAL_UART_MAX][32];

ATTR_TEXT_IN_RAM void exception_init(uint32_t stack[])
{
    int i;
    uint32_t* uart_base;
    ...
    // save UART register information
    for(i = 0; i < 4; i++)
    {
        uart_base = (uint32_t*)g_uart_regbase[i];
        memcpy(&g_uart_reg_dump[i], uart_base, 128);
    }

    //print assert info to AT port.
    print_assert_info_2_connl();

    // save cm4 registers into taskContext
    save_cm4_registers(stack);

    // do mini-dump
#if defined(MTK_MINI_DUMP_ENABLE)
    if(IsMiniDumpEnabled)
    {
        coredump_entry(pTaskContext, memory_regions);
    }
#endif

    // reboot if no need dump no matter genie connect or not.
    if (reboot_check() == true) {
        exception_reboot();
    }
    
    // reboot if auto reboot is true and genie is not connected
    if(auto_reboot_check() && (false == IsGenieConnected()))
        exception_reboot();

#ifdef OPTIMIZED_GKI_LOG_ENABLE
    KiLogExceptionHandshake();
#endif
...
{
    uint8_t Result = HISTORY_INFO_ERROR, index = 0;
...
{
    uint8_t TaskState = 0, TskIndexId = 0;
    ...
    TskIndexId = M_KiTaskGetIndex(kiTaskInitTable[TaskID].taskId);

    if(TskIndexId >= KI_NUM_UNIQUE_TASK_IDS_DEFINED)
    {
        return MINIDUMP_NO_GKI_SEMA;
    }  
    
    TCB = kiTaskSplitTable[TskIndexId].osTaskId;
...
{
    uint32_t TcbSema = 0, QueueSema = 0, TskIndexId = 0;
    ...
    TskIndexId = (uint32_t)M_KiTaskGetIndex(kiTaskInitTable[TaskID].taskId);
    ...
    TaskTCB = (void *)kiTaskSplitTable[TskIndexId].osTaskId;
...
#ifndef __H10_TEMP__
#include "h10_mmi.h"
void convert_build_time()
{
    populate_build_info(temp_info.build_time,MAX_BUILD_INFO_LEN+1);
}
#endif
...
void exception_reboot(void)
{
    ...
    hal_wdt_disable(HAL_WDT_DISABLE_MAGIC);
    hal_wdt_init(&wdt_config);
    rtc_set_sys_oper_mode(RTC_SYS_WDT_EE);
...
void stackDump(void)
{
    ...
    for(i = 0; i <= 12; i++)
    {
        printf("r%-2d = 0x%08x\n\r", i, *(&taskContext.r0 + i));
#if defined (KI_RLG_ENABLE_EVENT_OVER_HSL)
        snprintf(trace_buffer, 80, "r%-2d = 0x%08x\n\r", i, *(&taskContext.r0 + i));
        FrHslString (0x000A, trace_buffer);
#endif
    }    
...
void Debug_Monitor_Handler(uint32_t stack[])
{
    ...
    /* is task stack overflow? */
    {
        ...
        printf("Task stack overflow:%c, stack end:0x%x \r\n",((is_match)? 'Y':'N'),(unsigned int)stack_end);
#if defined (KI_RLG_ENABLE_EVENT_OVER_HSL)
		char trace_buffer[80];
