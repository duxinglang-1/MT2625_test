...
extern void MiniDump_RecordGkiTaskSwitchHistory(uint32_t pRio);
extern void MiniDump_RecordAPTaskSwitchHistory(char *NameAddr);
#endif
...
ATTR_TEXT_IN_TCM void vTraceTaskSwitchIn(uint32_t pRio)
{
    ...
    #ifdef MTK_MINI_DUMP_ENABLE
        if(pRio > MIN_GKI_TASK_PRIORITY - 1)
            MiniDump_RecordGkiTaskSwitchHistory(pRio); /*Record the gki task priority*/
        else
            MiniDump_RecordAPTaskSwitchHistory((char *)pxTaskName); /*reocrd the ap task name*/
