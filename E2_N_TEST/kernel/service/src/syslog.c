...
/* buffer pool items */
#ifndef OPTIMIZED_GKI_LOG_ENABLE
#define LOG_POOL_ITEMS SYSLOG_QUEUE_LENGTH
#else
#define LOG_POOL_ITEMS 8
#endif
/* maximum delay to wait for available log queue */
...
/* syslog buffer pool */
ATTR_NONINIT_DATA_IN_RAM static syslog_buffer_t syslog_buffer_pool[LOG_POOL_ITEMS];
...
#define SYSLOG_FLUSH_LOG_DONE 0x20

#if defined(USE_KIPRINTF_AS_PRINTF)
/* The feature is only applicable to OS-enabled system. */
#define MTK_BUFFER_EARLY_SYSLOG
#endif

#define SYSLOG_BUFFER_ALLOC         1  /* log buffer allocated for normal use */
#define SYSLOG_BUFFER_EARLY_ALLOC   3  /* log buffer allocated for early logging */
...
static void syslog_buffer_dump(void)
{
...
#if defined(MTK_BUFFER_EARLY_SYSLOG)
        /* Consider the early crash case as well. */
        if (syslog_buffer_pool[index].occupied == SYSLOG_BUFFER_ALLOC ||
            syslog_buffer_pool[index].occupied == SYSLOG_BUFFER_EARLY_ALLOC) {
#else
        if (syslog_buffer_pool[index].occupied == SYSLOG_BUFFER_ALLOC) {
#endif
    ...
    while (pHead) {
        ...
        pCurr->occupied = SYSLOG_BUFFER_ALLOC;
...
syslog_buffer_t *buffer_pool_alloc(void)
{
    ...
    if (syslog_free_list) {
        ...
        syslog_buffer->occupied = SYSLOG_BUFFER_ALLOC;
...
void print_normal_log(const normal_log_message_t *normal_log_message)
{
    ...
    syslog_printf("[T: %u M: %s C: %s F: %s L: %d]: %s\r\n",
...
{
#if defined(MTK_BUFFER_EARLY_SYSLOG)
    /* Defer drop count report as other task may call log api during log flush process. */
    if (syslog_drop_count && (syslog_init_done & SYSLOG_FLUSH_LOG_DONE)) {
#else
...
#ifdef MTK_SYSTEM_HANG_CHECK_ENABLE
static void syslog_feed_wdt(void)
{
...
static void request_syslog_service(syslog_buffer_t *log_buffer)
{
    if (syslog_task_ready) {
        /* Delegate the log to syslog task. */
        log_queue_send(&log_buffer);
    } else {
#if defined(MTK_BUFFER_EARLY_SYSLOG)
        /* Buffer the log to be flushed later. Set a flag instead of calling OS API. */
        log_buffer->occupied = SYSLOG_BUFFER_EARLY_ALLOC;
#else
        /* Directly output the log in plain text. */
...
#if defined(MTK_BUFFER_EARLY_SYSLOG)
static void syslog_buffer_flush(void)
{
    uint32_t index;

    for (index = 0; index < LOG_POOL_ITEMS; index++) {
#ifdef MTK_SYSTEM_HANG_CHECK_ENABLE
        /* feed watchdog during AP output logs, in case AP output too many logs and watchdog timeout */
        syslog_feed_wdt();
#endif
        /* Early log buffer is allocated from the start of syslog buffer pool in sequence.
         * When the first non-early log is found, the flush operation is done.
         */
        if (syslog_buffer_pool[index].occupied != SYSLOG_BUFFER_EARLY_ALLOC) {
            break;
        }
        process_log_message((log_message_t *)(syslog_buffer_pool[index].buffer));
        buffer_pool_release (&syslog_buffer_pool[index]);
    }
}
#endif
...
#if defined(MTK_BUFFER_EARLY_SYSLOG)
    /* Flush buffered syslog. */
    syslog_buffer_flush();
    syslog_init_done |= SYSLOG_FLUSH_LOG_DONE;
#endif
                       ...
                       va_list list)
{
    ...
    if ((context->log_switch == DEBUG_LOG_ON) && (context->print_level <= level)) {
#ifdef OPTIMIZED_GKI_LOG_ENABLE
        char strbuff[MAX_LOG_SIZE];
        uint32_t nr_of_chars;
        nr_of_chars = snprintf(strbuff, MAX_LOG_SIZE, "[T: %u M: %s C: %s  L: %d]: ", get_current_time_in_ms(), func, change_level_to_string(level), line);
        nr_of_chars += vsnprintf(strbuff + nr_of_chars, MAX_LOG_SIZE - nr_of_chars, message, list);
        if (nr_of_chars >= MAX_LOG_SIZE)
        {
            nr_of_chars = MAX_LOG_SIZE;
        }
        else
        {
            nr_of_chars += 1;
        }
        syslog_output(nr_of_chars, strbuff);
#else
        if (xTaskGetSchedulerState() == taskSCHEDULER_SUSPENDED && (portNVIC_INT_CTRL_REG & 0xff) == 0) {
            if (syslog_init_done > 0) {
                UBaseType_t uxSavedInterruptStatus;
            ...
            request_syslog_service(syslog_buffer);
        }
#endif /*OPTIMIZED_GKI_LOG_ENABLE*/
    }
}
