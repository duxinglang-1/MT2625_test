...
#ifdef SYSTEM_HANG_CHECK_ENABLE_WDT_DURING_LIGHT_SLEEP
    if(sleep_time > 1020000)
        sleep_time = 1020000;
#endif
    ret = hal_sleep_manager_set_sleep_time(sleep_time);
    if (ret == HAL_SLEEP_MANAGER_ERROR) {
...
#ifdef SYSTEM_HANG_CHECK_ENABLE_WDT_DURING_LIGHT_SLEEP
    //make sure watchdog timeout is longer than sleep_time about 1~2 second
    hal_wdt_reinit_length((sleep_time/1000) + 2);
#else
...
#ifdef MTK_SYSTEM_HANG_CHECK_ENABLE
#ifdef SYSTEM_HANG_CHECK_ENABLE_WDT_DURING_LIGHT_SLEEP
    hal_wdt_reinit_length(SYSTEM_HANG_CHECK_TIMEOUT_DURATION);
#else
